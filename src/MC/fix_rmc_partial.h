...
class FixRMCPartial : public Fix {
 public:
  ...
  int determine_charge_state(Mol *, double);
  void calculateMoleculeCOM(double *, Mol *);
  double calculateColoumbSelf(double **, int);
  void calculateSpecialBondMatrix(double **, Mol *, int);
  double calDistance(double *, double *);
  void calculateColoumbCross(double **, int, double **, int, double *);
  void bringMoleculeTogether(Mol *, double **, int);
  double getSpecialBondCoefficient(int, int);
  void modify_charge(Mol *, double *);
  void restore_charge(Mol *);
