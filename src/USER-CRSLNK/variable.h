...
public:
	...
	PriorityList* get_type_detector(char*);
	void add_type_detector(CoordinateType, int, char**);
...
private:
	...
	std::vector<PriorityList*> type_dectors;
	...
	int group_function(char *, char *, Tree **, Tree **, int &, double *,
			int &);
	int region_function(char *);
	int special_function(char *, char *, Tree **, Tree **, int &, double *,
			int &);
	void peratom2global(int, char *, double *, int, tagint, Tree **, Tree **,
			int &, double *, int &);
