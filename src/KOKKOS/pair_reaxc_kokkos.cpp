...
PairReaxCKokkos<Space>::PairReaxCKokkos(LAMMPS *lmp) : PairReaxC(lmp)
{
  respa_enable = 0;
  ...
  d_resize_bo = DualViewHelper<Space>::view(k_resize_bo);

  k_resize_hb = DAT::tdual_int_scalar("pair:resize_hb");
  d_resize_hb = DualViewHelper<Space>::view(k_resize_hb);
...
template<ExecutionSpace Space>
PairReaxCKokkos<Space>::~PairReaxCKokkos()
{
  if (copymode) return;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::init_style()
{
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::setup()
{
  int i,j,k,m;
  int n = atom->ntypes;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::init_md()
{
  // init_taper()
  KK_FLOAT d1, d7, swa, swa2, swa3, swb, swb2, swb3;
  ...
  k_tap.modify_host();
  DualViewHelper<Space>::sync(k_tap);
  ...
  if ( control->tabulate ) {
    ...
    d_LR = DualViewHelper<Space>::view(k_LR);
    ...
    for (int i = 1; i <= ntypes; ++i) {
      for (int j = i; j <= ntypes; ++j) {
        ...
        typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d k_vdW    = typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d("lookup:LR[i,j].vdW",n);
        typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d k_CEvd   = typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d("lookup:LR[i,j].CEvd",n);
        typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d k_ele    = typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d("lookup:LR[i,j].ele",n);
        typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d k_CEclmb = typename LR_lookup_table_kk<Space>::tdual_cubic_spline_coef_1d("lookup:LR[i,j].CEclmb",n);

        k_LR.h_view(i,j).d_vdW    = DualViewHelper<Space>::view(k_vdW);
        k_LR.h_view(i,j).d_CEvd   = DualViewHelper<Space>::view(k_CEvd);
        k_LR.h_view(i,j).d_ele    = DualViewHelper<Space>::view(k_ele);
        k_LR.h_view(i,j).d_CEclmb = DualViewHelper<Space>::view(k_CEclmb);
        ...
        DualViewHelper<Space>::sync(k_vdW);
        DualViewHelper<Space>::sync(k_CEvd);
        DualViewHelper<Space>::sync(k_ele);
        DualViewHelper<Space>::sync(k_CEclmb);
      }
    }
    k_LR.modify_host();
    DualViewHelper<Space>::sync(k_LR);
...
template<ExecutionSpace Space>
int PairReaxCKokkos<Space>::Init_Lookup_Tables()
{
  int i, j, r;
  int num_atom_types;
  KK_FLOAT dr;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::Deallocate_Lookup_Tables()
{
  int i, j;
  int ntypes;
  LR_lookup_table ** & LR = system->LR;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::LR_vdW_Coulomb( int i, int j, KK_FLOAT r_ij, LR_data *lr )
{
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::compute(int eflag_in, int vflag_in)
{
  copymode = 1;
  ...
  while (resize) {
    ...
    DualViewHelper<Space>::sync(k_resize_bo);

    k_resize_hb.h_view() = 0;
    k_resize_hb.modify_host();
    DualViewHelper<Space>::sync(k_resize_hb);
    ...
    DualViewHelper<Space>::modify(k_resize_bo);
    k_resize_bo.sync_host();
    int resize_bo = k_resize_bo.h_view();
    if (resize_bo) maxbo++;

    DualViewHelper<Space>::modify(k_resize_hb);
...
void PairReaxCKokkos<Space>::operator()(PairReaxComputePolar<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
  ...
  const KK_FLOAT epol = KCALpMOL_to_EV*(chi*qi+(eta/2.0)*qi*qi);
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputePolar<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeLJCoulomb<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    const KK_FLOAT denom3 = pow(denom1,0.3333333333333);
    const KK_FLOAT ecoul = C_ele * qi*qj*Tap/denom3;
    const KK_FLOAT fcoul = C_ele * qi*qj*(dTap-Tap*rij/denom1)/denom3;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeLJCoulomb<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeTabulatedLJCoulomb<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    const LR_lookup_table_kk<Space>& t = d_LR(tmin,tmax);
    ...
    const KK_FLOAT ecoul = (((ele.d*dif + ele.c)*dif + ele.b)*dif +
      ele.a)*qi*qj;

    const KK_FLOAT fvdwl = ((CEvd.d*dif + CEvd.c)*dif + CEvd.b)*dif +
      CEvd.a;

    const KK_FLOAT fcoul = (((CEclmb.d*dif+CEclmb.c)*dif+CEclmb.b)*dif +
      CEclmb.a)*qi*qj;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeTabulatedLJCoulomb<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::allocate_array()
{
  if (cut_hbsq > 0.0) {
  ...
  d_BO = typename AT::t_float_2d_dl("reax/c/kk:BO",nmax,maxbo);
  d_BO_s = typename AT::t_float_2d_dl("reax/c/kk:BO",nmax,maxbo);
  d_BO_pi = typename AT::t_float_2d_dl("reax/c/kk:BO_pi",nmax,maxbo);
  d_BO_pi2 = typename AT::t_float_2d_dl("reax/c/kk:BO_pi2",nmax,maxbo);

  d_dln_BOp_pix = typename AT::t_float_2d_dl("reax/c/kk:d_dln_BOp_pix",nmax,maxbo);
  d_dln_BOp_piy = typename AT::t_float_2d_dl("reax/c/kk:d_dln_BOp_piy",nmax,maxbo);
  d_dln_BOp_piz = typename AT::t_float_2d_dl("reax/c/kk:d_dln_BOp_piz",nmax,maxbo);

  d_dln_BOp_pi2x = typename AT::t_float_2d_dl("reax/c/kk:d_dln_BOp_pi2x",nmax,maxbo);
  d_dln_BOp_pi2y = typename AT::t_float_2d_dl("reax/c/kk:d_dln_BOp_pi2y",nmax,maxbo);
  d_dln_BOp_pi2z = typename AT::t_float_2d_dl("reax/c/kk:d_dln_BOp_pi2z",nmax,maxbo);

  d_C1dbo = typename AT::t_float_2d_dl("reax/c/kk:d_C1dbo",nmax,maxbo);
  d_C2dbo = typename AT::t_float_2d_dl("reax/c/kk:d_C2dbo",nmax,maxbo);
  d_C3dbo = typename AT::t_float_2d_dl("reax/c/kk:d_C3dbo",nmax,maxbo);

  d_C1dbopi = typename AT::t_float_2d_dl("reax/c/kk:d_C1dbopi",nmax,maxbo);
  d_C2dbopi = typename AT::t_float_2d_dl("reax/c/kk:d_C2dbopi",nmax,maxbo);
  d_C3dbopi = typename AT::t_float_2d_dl("reax/c/kk:d_C3dbopi",nmax,maxbo);
  d_C4dbopi = typename AT::t_float_2d_dl("reax/c/kk:d_C4dbopi",nmax,maxbo);

  d_C1dbopi2 = typename AT::t_float_2d_dl("reax/c/kk:d_C1dbopi2",nmax,maxbo);
  d_C2dbopi2 = typename AT::t_float_2d_dl("reax/c/kk:d_C2dbopi2",nmax,maxbo);
  d_C3dbopi2 = typename AT::t_float_2d_dl("reax/c/kk:d_C3dbopi2",nmax,maxbo);
  d_C4dbopi2 = typename AT::t_float_2d_dl("reax/c/kk:d_C4dbopi2",nmax,maxbo);

  d_dBOpx = typename AT::t_float_2d_dl("reax/c/kk:dBOpx",nmax,maxbo);
  d_dBOpy = typename AT::t_float_2d_dl("reax/c/kk:dBOpy",nmax,maxbo);
  d_dBOpz = typename AT::t_float_2d_dl("reax/c/kk:dBOpz",nmax,maxbo);

  d_dDeltap_self = typename AT::t_float_2d_dl("reax/c/kk:dDeltap_self",nmax,3);
  d_Deltap_boc = typename AT::t_float_1d("reax/c/kk:Deltap_boc",nmax);
  d_Deltap = typename AT::t_float_1d("reax/c/kk:Deltap",nmax);
  d_total_bo = typename AT::t_float_1d("reax/c/kk:total_bo",nmax);

  d_Cdbo = typename AT::t_float_2d_dl("reax/c/kk:Cdbo",nmax,3*maxbo);
  d_Cdbopi = typename AT::t_float_2d_dl("reax/c/kk:Cdbopi",nmax,3*maxbo);
  d_Cdbopi2 = typename AT::t_float_2d_dl("reax/c/kk:Cdbopi2",nmax,3*maxbo);

  d_Delta = typename AT::t_float_1d("reax/c/kk:Delta",nmax);
  d_Delta_boc = typename AT::t_float_1d("reax/c/kk:Delta_boc",nmax);
  d_dDelta_lp = typename AT::t_float_1d("reax/c/kk:dDelta_lp",nmax);
  d_Delta_lp = typename AT::t_float_1d("reax/c/kk:Delta_lp",nmax);
  d_Delta_lp_temp = typename AT::t_float_1d("reax/c/kk:Delta_lp_temp",nmax);
  d_CdDelta = typename AT::t_float_1d("reax/c/kk:CdDelta",nmax);
  d_sum_ovun = typename AT::t_float_2d_dl("reax/c/kk:sum_ovun",nmax,3);

  // FixReaxCBonds
  d_abo = typename AT::t_float_2d("reax/c/kk:abo",nmax,maxbo);
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxZero, const int &n) const {
  d_total_bo(n) = 0.0;
  d_CdDelta(n) = 0.0;
  d_bo_num(n) = 0.0;
  d_hb_num(n) = 0.0;
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxZeroEAtom, const int &i) const {
  d_eatom(i) = 0.0;
}

template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxZeroVAtom, const int &i) const {
  d_vatom(i,0) = 0.0;
  d_vatom(i,1) = 0.0;
  d_vatom(i,2) = 0.0;
  d_vatom(i,3) = 0.0;
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxBuildListsFull, const int &ii) const {
  ...
  KK_FLOAT C12, C34, C56, BO_s, BO_pi, BO_pi2, BO, delij[3], dBOp_i[3], dln_BOp_pi_i[3], dln_BOp_pi2_i[3];
...
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxBuildListsHalf<NEIGHFLAG>, const int &ii) const {
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxBondOrder1, const int &ii) const {
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxBondOrder2, const int &ii) const {
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxBondOrder3, const int &ii) const {
// bot part of BO()
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeMulti1<NEIGHFLAG,EVFLAG>, const int &ii) const {
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeMulti2<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeMulti2<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeAngular<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {

  auto v_f = ScatterViewHelper<NeedDup<NEIGHFLAG,DeviceType>::value,decltype(dup_f),decltype(ndup_f)>::get(dup_f,ndup_f);
  auto a_f = v_f.template access<AtomicDup<NEIGHFLAG,DeviceType>::value>();
  Kokkos::View<typename AT::t_float_2d_dl::data_type, typename AT::t_float_2d_dl::array_layout,typename KKDevice<DeviceType>::value,Kokkos::MemoryTraits<AtomicF<NEIGHFLAG>::value> > a_Cdbo = d_Cdbo;
  ...
  KK_FLOAT p_val6, p_val7, p_val8, p_val9, p_val10;
  ...
  KK_FLOAT dSBO1, dSBO2, SBO, SBO2, CSBO2, SBOp, prod_SBO, vlpadj;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeAngular<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeTorsion<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
  ...
  KK_FLOAT fi_tmp[3], fj_tmp[3], fk_tmp[3], fl_tmp[3];
  KK_FLOAT dcos_omega_di[3], dcos_omega_dj[3], dcos_omega_dk[3], dcos_omega_dl[3];
  ...
  KK_FLOAT fitmp[3], fjtmp[3], fktmp[3];
  for(int j = 0; j < 3; j++) fitmp[j] = 0.0;
  KK_FLOAT CdDelta_i = 0.0;
  ...
  for (int jj = j_start; jj < j_end; jj++) {
    ...
    for (int kk = j_start; kk < j_end; kk++) {
      ...
      for (int ll = l_start; ll < l_end; ll++) {
        ...
        const KK_FLOAT inv_distjl3 = pow( inv_distjl, 3.0 );
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeTorsion<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeHydrogen<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeHydrogen<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxUpdateBond<NEIGHFLAG>, const int &ii) const {

  Kokkos::View<typename AT::t_float_2d_dl::data_type, typename AT::t_float_2d_dl::array_layout,typename KKDevice<DeviceType>::value,Kokkos::MemoryTraits<AtomicF<NEIGHFLAG>::value> > a_Cdbo = d_Cdbo;
  Kokkos::View<typename AT::t_float_2d_dl::data_type, typename AT::t_float_2d_dl::array_layout,typename KKDevice<DeviceType>::value,Kokkos::MemoryTraits<AtomicF<NEIGHFLAG>::value> > a_Cdbopi = d_Cdbopi;
  Kokkos::View<typename AT::t_float_2d_dl::data_type, typename AT::t_float_2d_dl::array_layout,typename KKDevice<DeviceType>::value,Kokkos::MemoryTraits<AtomicF<NEIGHFLAG>::value> > a_Cdbopi2 = d_Cdbopi2;
  //auto a_Cdbo = dup_Cdbo.template access<AtomicDup<NEIGHFLAG,DeviceType>::value>();
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeBond1<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeBond1<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeBond2<NEIGHFLAG,EVFLAG>, const int &ii, EV_FLOAT_REAX& ev) const {
...
template<int NEIGHFLAG, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxComputeBond2<NEIGHFLAG,EVFLAG>, const int &ii) const {
  EV_FLOAT_REAX ev;
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::ev_tally(EV_FLOAT_REAX &ev, const int &i, const int &j,
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::e_tally(EV_FLOAT_REAX &ev, const int &i, const int &j,
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::e_tally_single(EV_FLOAT_REAX &ev, const int &i,
      const KK_FLOAT &epair) const
{
  // The eatom array is duplicated for OpenMP, atomic for CUDA, and neither for Serial
  auto v_eatom = ScatterViewHelper<NeedDup<NEIGHFLAG,DeviceType>::value,decltype(dup_eatom),decltype(ndup_eatom)>::get(dup_eatom,ndup_eatom);
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::v_tally(EV_FLOAT_REAX &ev, const int &i,
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::v_tally3(EV_FLOAT_REAX &ev, const int &i, const int &j, const int &k,
...
template<int NEIGHFLAG>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::v_tally4(EV_FLOAT_REAX &ev, const int &i, const int &j, const int &k,
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::v_tally3_atom(EV_FLOAT_REAX &ev, const int &i, const int &j, const int &k,
...
template<ExecutionSpace Space>
void *PairReaxCKokkos<Space>::extract(const char *str, int &dim)
{
  dim = 1;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::ev_setup(int eflag, int vflag, int)
{
  int i;
...
template<ExecutionSpace Space>
double PairReaxCKokkos<Space>::memory_usage()
{
  double bytes = 0.0;
  ...
  bytes += nmax*17*sizeof(KK_FLOAT);
  bytes += maxbo*nmax*34*sizeof(KK_FLOAT);

  // FixReaxCSpecies
  if (fixspecies_flag) {
    bytes += MAXSPECBOND*nmax*sizeof(tagint);
    bytes += MAXSPECBOND*nmax*sizeof(KK_FLOAT);
  }

  // FixReaxCBonds
  bytes += maxbo*nmax*sizeof(tagint);
  bytes += maxbo*nmax*sizeof(KK_FLOAT);
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::FindBond(int &numbonds)
{
  copymode = 1;
  ...
  numbonds = 0;
  PairReaxCKokkosFindBondFunctor<Space> find_bond_functor(this);
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxFindBondZero, const int &i) const {
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::calculate_find_bond_item(int ii, int &numbonds) const
{
  const int i = d_ilist[ii];
  int nj = 0;
...
template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::PackBondBuffer(DAT::tdual_float_1d k_buf, int &nbuf_local)
{
  ...
  q = DualViewHelper<Space>::view(atomKK->k_q);
  if (atom->molecule)
    molecule = DualViewHelper<Space>::view(atomKK->k_molecule);

  copymode = 1;
  nlocal = atomKK->nlocal;
  PairReaxCKokkosPackBondBufferFunctor<Space> pack_bond_buffer_functor(this);
  Kokkos::parallel_scan(nlocal,pack_bond_buffer_functor);
  copymode = 0;

  DualViewHelper<Space>::modify(k_buf);
  DualViewHelper<Space>::modify(k_nbuf_local);
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::pack_bond_buffer_item(int i, int &j, const bool &final) const
{
  if (i == 0)
    j += 2;
  ...
  if (final && i == nlocal-1)
    DualViewHelper<Space>::view(k_nbuf_local)() = j - 1;
}

/* ---------------------------------------------------------------------- */

template<ExecutionSpace Space>
void PairReaxCKokkos<Space>::FindBondSpecies()
{
  ...
  DualViewHelper<Space>::modify(k_tmpbo);
  DualViewHelper<Space>::modify(k_tmpid);
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxFindBondSpeciesZero, const int &i) const {
  for (int j = 0; j < MAXSPECBOND; j++) {
    DualViewHelper<Space>::view(k_tmpbo)(i,j) = 0.0;
    DualViewHelper<Space>::view(k_tmpid)(i,j) = 0;
  }
}

template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairReaxCKokkos<Space>::operator()(PairReaxFindBondSpecies, const int &i) const {
  int nj = 0;
  ...
  for (int jj = j_start; jj < j_end; jj++) {
    ...
    if (bo_tmp >= 0.10 ) { // Why is this a hardcoded value?
      DualViewHelper<Space>::view(k_tmpid)(i,nj) = j;
      DualViewHelper<Space>::view(k_tmpbo)(i,nj) = bo_tmp;
      nj++;
      if (nj > MAXSPECBOND) DualViewHelper<Space>::view(k_error_flag)() = 1;
    }
  }
}

template class PairReaxCKokkos<Device>;
template class PairReaxCKokkos<Host>;
}
