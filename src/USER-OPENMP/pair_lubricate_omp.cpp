...
PairLubricateOMP::PairLubricateOMP(LAMMPS *lmp) : PairOMP(lmp)
{
  single_enable = 0;
...
void PairLubricateOMP::compute(int eflag, int vflag)
{
  ...
  if (omega_flag) {
    ...
    if (evflag) {
      if (force->newton_pair) return eval<0,1,1>();
      else return eval<0,1,0>();
    } else {
...
template <int OMEGA_FLAG, int EVFLAG, int NEWTON_PAIR> 
void PairLubricateOMP::eval() 
{
...
#pragma omp parallel default(shared)
#endif
  {
    ...
    double **torque = atom->torque + tid*nall;  
...
void PairLubricateOMP::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLubricateOMP::settings(int narg, char **arg)
{
...
void PairLubricateOMP::coeff(int narg, char **arg)
{
  if (narg != 2 && narg != 4)
...
void PairLubricateOMP::init_style()
{
...
double PairLubricateOMP::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLubricateOMP::write_restart(FILE *fp)
{
...
void PairLubricateOMP::read_restart(FILE *fp)
{
...
void PairLubricateOMP::write_restart_settings(FILE *fp)
{
...
void PairLubricateOMP::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
