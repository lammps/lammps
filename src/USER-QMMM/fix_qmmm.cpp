 ...
 *
 *                     */
int FixQMMM::match_element(double mass, int search_isotopes, double *delta)
{
  int i;
  int bestcandidate;
  double diff, mindiff;
...
int *FixQMMM::match_atom_elements()
{
  double *masses;
  int *tmp = NULL;
  int *elements = NULL;
  ...
  if(tmp == NULL) {
    char emsg[150];
    snprintf(emsg, 150, "FixQMMM: unable to allocate memory in %s:%d\n",__FILE__, __LINE__);
  ...
  for(i=1;i<=atom->ntypes;i++) {
    ...
    snprintf(loginfo, 200, "FixQMMM: type %2d (mass: %8g) matches a %2s with: Z = %-3d A = %-3d (error = %-8.2g -> %-.2g%%)\n",
             i, atom->mass[i], FixQMMM_EL[tmp[i]], FixQMMM_Z[tmp[i]], FixQMMM_A[tmp[i]],
  ...
  for(i=0;i<atom->natoms;i++)
    elements[i] = FixQMMM_Z[tmp[atom->type[i]]];
  free(tmp);
  return elements;
}
