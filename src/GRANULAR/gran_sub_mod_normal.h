...
namespace LAMMPS_NS::Granular_NS {
  ...
  class GranSubModNormal : public GranSubMod {
   public:
    ...
    // coefficients needed by the shared kernels; only the cohesive models
    // define them, so the base returns zero
    [[nodiscard]] virtual double get_k() const { return 0.0; }
    [[nodiscard]] virtual double get_cohesion() const { return 0.0; }
    [[nodiscard]] virtual double get_emix() const { return 0.0; }

    void fill_kernel_params(GranKernel::GranNormalParams<double> &p) const
    {
      p.model = GRAN_NORMAL_NONE;    // host classes call the models directly
      p.k = get_k();
      p.cohesion = get_cohesion();
      p.Emix = get_emix();
    }
  ...
  class GranSubModNormalDMT : public GranSubModNormal {
   public:
    ...
    [[nodiscard]] double get_k() const override { return k; }
    [[nodiscard]] double get_cohesion() const override { return cohesion; }
  ...
  class GranSubModNormalJKR : public GranSubModNormal {
   public:
    ...
    [[nodiscard]] double get_k() const override { return k; }
    [[nodiscard]] double get_cohesion() const override { return cohesion; }
    [[nodiscard]] double get_emix() const override { return Emix; }
