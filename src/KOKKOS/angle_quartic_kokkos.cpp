...
#include "angle_quartic_kokkos.h"
...
AngleQuarticKokkos<DeviceType>::AngleQuarticKokkos(LAMMPS *lmp) : AngleQuartic(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
AngleQuarticKokkos<DeviceType>::~AngleQuarticKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void AngleQuarticKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleQuarticCompute<1,1> >(0,nanglelist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleQuarticCompute<0,1> >(0,nanglelist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleQuarticCompute<1,0> >(0,nanglelist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleQuarticCompute<0,0> >(0,nanglelist),*this);
    }
  }
...
void AngleQuarticKokkos<DeviceType>::operator()(TagAngleQuarticCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT tk = static_cast<KK_FLOAT>(2.0)*d_k2[type]*dtheta
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleQuarticKokkos<DeviceType>::operator()(TagAngleQuarticCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagAngleQuarticCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void AngleQuarticKokkos<DeviceType>::allocate()
{
  AngleQuartic::allocate();

  int n = atom->nangletypes;
  k_k2     = DAT::tdual_kkfloat_1d("AngleQuartic::k2",n+1);
  k_k3     = DAT::tdual_kkfloat_1d("AngleQuartic::k3",n+1);
  k_k4     = DAT::tdual_kkfloat_1d("AngleQuartic::k4",n+1);
  k_theta0 = DAT::tdual_kkfloat_1d("AngleQuartic::theta0",n+1);
...
template<class DeviceType>
void AngleQuarticKokkos<DeviceType>::coeff(int narg, char **arg)
{
  AngleQuartic::coeff(narg, arg);
...
template<class DeviceType>
void AngleQuarticKokkos<DeviceType>::read_restart(FILE *fp)
{
  AngleQuartic::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleQuarticKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
