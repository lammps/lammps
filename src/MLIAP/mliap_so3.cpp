...
void MLIAP_SO3::init()
{
	...
	for(int l=0;l<m_lmax+2;l++){
		for(int m=-l;m<l+1;m++){
			m_pfac[l*m_pfac_l2+m]=sqrt((2.0*l+1.0)*pfac1)*pow(-1,m);
		}
	}
    m_dfac_l1=m_lmax+1;
	...
	for(int l=1;l<m_lmax+1;l++){
		 for(int m=-l;m<l+1;m++){
			 m_dfac0[l*m_dfac_l2+m]=-sqrt(((l+1.0)*(l+1.0)-m*m)/(2.0*l+1.0)/(2.0*l+3.0))*l;
			 m_dfac1[l*m_dfac_l2+m]=sqrt((l*l-m*m)/(2.0*l-1.0)/(2.0*l+1.0))*(l+1.0);
			 m_dfac2[l*m_dfac_l2+m]=-sqrt((l+m+1.0)*(l+m+2.0)/2.0/(2.0*l+1.0)/(2.0*l+3.0))*l;
			 m_dfac3[l*m_dfac_l2+m]=sqrt((l-m-1.0)*(l-m)/2.0/(2.0*l-1.0)/(2.0*l+1.0))*(l+1.0);
			 m_dfac4[l*m_dfac_l2+m]=-sqrt((l-m+1.0)*(l-m+2.0)/2.0/(2.0*l+1.0)/(2.0*l+3.0))*l;
			 m_dfac5[l*m_dfac_l2+m]=sqrt((l+m-1.0)*(l+m)/2.0/(2.0*l-1.0)/(2.0*l+1.0))*(l+1.0);
		 }
	}
    ...
    init_garray(m_nmax, m_lmax, m_rcut, m_alpha,
    		m_w,m_nmax,m_nmax, m_g_array,m_nmax,m_Nmax);
...
double MLIAP_SO3::memory_usage()
{
  ...
  bytes = 0;
  bytes += (double)m_nmax*m_nmax * sizeof(double);  //m_w
...
int MLIAP_SO3::partition (double arr[], int low, int high, double arrv[],int n)
{
...
void MLIAP_SO3::quickSort(double arr[], int low, int high, double arrv[], int n)
{
    if (low < high)
    {
...
void MLIAP_SO3::compute_pi(int nmax,int lmax, double *clisttot_r, double *clisttot_i, int lcl1,int lcl2,
			...
			for(l=0;l<lmax+1;l++){
			     ...
			     for(m=-l;m<l+1;m++){
				      ...
				      plist_r[lpl2*indpl+i] += ( clisttot_r[lcl2*n1+j] * clisttot_r[lcl2*n2+j] +
				    		                     clisttot_i[lcl2*n1+j] * clisttot_i[lcl2*n2+j] )*norm;
				      plist_i[lpl2*indpl+i] += ( -clisttot_r[lcl2*n1+j] * clisttot_i[lcl2*n2+j] +
				      				    		 clisttot_i[lcl2*n1+j] * clisttot_r[lcl2*n2+j] )*norm;
...
double MLIAP_SO3::g(double r,int n,int nmax,double rcut,double *w,int lw1, int lw2){
...
void MLIAP_SO3::compute_dpidrj(int nmax, int lmax, double *clisttot_r,double *clisttot_i,int lctot1,int lctot2,
...
void MLIAP_SO3::compute_uarray_recursive(double x, double y, double z, double r,
...
void MLIAP_SO3::init_garray(int nmax, int lmax, double rcut, double alpha, double *w, int lw1, int lw2,
	...
	for (i=1; i<Nmax+1;i++){
	        ...
	        xi = rcut/2*(x+1);
	        for(n=1;n<nmax+1;n++){ //g(r, n, nmax, rcut, w,lw1,lw2)
	            // r**2*g(n)(r)*e^(-alpha*r**2)
	            g_array[(n-1)*lg2+i-1] = rcut/2*M_PI/Nmax*sqrt(1-x*x)*xi*xi*g(xi, n, nmax, rcut, w,lw1,lw2)*exp(-alpha*xi*xi);
	        }
	}
}

void MLIAP_SO3::get_sbes_array(int natoms,int* numneighs,int* jelems,double* wjelem,
                              double** rij,int nmax, int lmax,double rcut, double alpha, int ncoefs){
    ...
    for (int ii = 0; ii < natoms; ii++) {
	  ...
	  for(neighbor=0;neighbor<numneighs[ii];neighbor++){
		  ...
		  for(i=1;i<m_Nmax+1;i++){
				...
				for(j=2;j<lmax+1;j++){
					m_sbes_array[gindex+(i-1)*mindex+j] = m_sbes_array[gindex+(i-1)*mindex+j-2] - (2*j-1)/rb*m_sbes_array[gindex+(i-1)*mindex+j-1];
				}
				exts=m_sbes_array[gindex+(i-1)*mindex+j-2] - (2*j-1)/rb*m_sbes_array[gindex+(i-1)*mindex+j-1];


				m_sbes_darray[gindex+(i-1)*mindex+0]=sb;

				for(j=1;j<lmax;j++){
					m_sbes_darray[gindex+(i-1)*mindex+j]=xi*(j*m_sbes_array[gindex+(i-1)*mindex+j-1] + (j+1)*m_sbes_array[gindex+(i-1)*mindex+j+1]) / (2*j+1);
				}
				m_sbes_darray[gindex+(i-1)*mindex+j]=xi*(j*m_sbes_array[gindex+(i-1)*mindex+j-1] + (j+1)*exts) / (2*j+1);
...
void MLIAP_SO3::get_rip_array(int natoms,int* numneighs,int* jelems,double* wjelem,
		                      double** rij,int nmax, int lmax,double rcut, double alpha, int ncoefs){
	  ...
	  for(neighbor=0;neighbor<numneighs[ii];neighbor++){
		  ...
		  for(n=1;n<nmax+1;n++){
				for(l=0;l<lmax+1;l++){
					...
					integrald=0.0;
					for(i=0;i<m_Nmax;i++){
							integral += m_g_array[(n-1)*m_Nmax+i]*m_sbes_array[(ipair-1)*m_Nmax*(m_lmax+1)+i*(m_lmax+1)+l];
							integrald += m_g_array[(n-1)*m_Nmax+i]*m_sbes_darray[(ipair-1)*m_Nmax*(m_lmax+1)+i*(m_lmax+1)+l];
					}

					m_rip_array[(ipair-1)*m_nmax*(m_lmax+1)+(n-1)*(m_lmax+1)+l]=integral*expfac;
					m_rip_darray[(ipair-1)*m_nmax*(m_lmax+1)+(n-1)*(m_lmax+1)+l]=integrald*expfac;
...
void MLIAP_SO3::spectrum(int natoms,int* numneighs,int* jelems,double* wjelem,double** rij,
	...
	if(m_init_arrays==0) init_arrays(natoms,numneighs,ncoefs);
	...
	get_sbes_array(natoms,numneighs,jelems,wjelem,
			 rij,
			nmax, lmax,rcut, alpha,
			ncoefs);

	get_rip_array(natoms,numneighs,jelems,wjelem,
			 rij,
			nmax, lmax,rcut, alpha,
			ncoefs);
    ...
    for (int ii = 0; ii < natoms; ii++) {
	  ...
	  for(neighbor=0;neighbor<numneighs[ii];neighbor++){
		  ...
		  compute_uarray_recursive(x,y,z,r,twolmax,m_ulist_r,m_ulist_i,m_idxu_block,m_rootpq,m_ldim,m_ldim);
	  ...
	  compute_pi(nmax,lmax,m_clisttot_r,m_clisttot_i,nmax,m_numYlms,m_plist_r,m_plist_i,natoms,ncoefs,ii);
...
void MLIAP_SO3::spectrum_dxdr(int natoms,int* numneighs,int* jelems,double* wjelem,
		         double** rij,int nmax, int lmax,double rcut, double alpha,int npairs, int ncoefs){
	...
	get_sbes_array(natoms,numneighs,jelems,wjelem,
			 rij,nmax, lmax,rcut, alpha,ncoefs);
...
////////////////  compute_carray_wD  ////////
          {
			...
			for(int n=1;n<nmax+1;n++){
				...
				for(int l=0;l<lmax+1;l++){
					  ...
					  r_int_temp=m_rip_darray[(idpair-1)*m_nmax*(m_lmax+1)+(n-1)*(m_lmax+1)+l];
					...
					for(int m=-l;m<l+1;m++){
        				...
        				m_dclist_r[((n-1)*m_numYlms+i)*3+0] +=r_int*m_Ylms_r[i]*dexpfac[0] + dr_int[0]*m_Ylms_r[i] + r_int*m_dYlm_r[i*3+0];
						m_dclist_r[((n-1)*m_numYlms+i)*3+1] +=r_int*m_Ylms_r[i]*dexpfac[1] + dr_int[1]*m_Ylms_r[i] + r_int*m_dYlm_r[i*3+1];
						m_dclist_r[((n-1)*m_numYlms+i)*3+2] +=r_int*m_Ylms_r[i]*dexpfac[2] + dr_int[2]*m_Ylms_r[i] + r_int*m_dYlm_r[i*3+2];


						m_dclist_i[((n-1)*m_numYlms+i)*3+0] +=r_int*m_Ylms_i[i]*dexpfac[0] + dr_int[0]*m_Ylms_i[i] + r_int*m_dYlm_i[i*3+0];
						m_dclist_i[((n-1)*m_numYlms+i)*3+1] +=r_int*m_Ylms_i[i]*dexpfac[1] + dr_int[1]*m_Ylms_i[i] + r_int*m_dYlm_i[i*3+1];
						m_dclist_i[((n-1)*m_numYlms+i)*3+2] +=r_int*m_Ylms_i[i]*dexpfac[2] + dr_int[2]*m_Ylms_i[i] + r_int*m_dYlm_i[i*3+2];
...
////////////// end compute_carray_wD //////////////////
	      ...
	      compute_dpidrj(nmax, lmax, m_clisttot_r,m_clisttot_i,nmax,m_numYlms, m_dclist_r,m_dclist_i,nmax,m_numYlms,3,
	    		  m_tempdp_r,numps,3);
