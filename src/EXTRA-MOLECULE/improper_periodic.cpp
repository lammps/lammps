...
ImproperPeriodic::ImproperPeriodic(LAMMPS *_lmp) : Improper(_lmp)
{
  writedata = 1;
...
ImproperPeriodic::~ImproperPeriodic()
{
  if (copymode) return;
...
void ImproperPeriodic::compute(int eflag, int vflag)
{
...
void ImproperPeriodic::allocate()
{
  allocated = 1;
  ...
  memory->create(delta, np1, "improper:delta");
...
void ImproperPeriodic::coeff(int narg, char **arg)
{
...
void ImproperPeriodic::write_restart(FILE *fp)
{
  fwrite(&k[1], sizeof(double), atom->nimpropertypes, fp);
  fwrite(&multiplicity[1], sizeof(int), atom->nimpropertypes, fp);
  fwrite(&delta[1], sizeof(double), atom->nimpropertypes, fp);
}
...
void ImproperPeriodic::read_restart(FILE *fp)
{
  allocate();
  ...
  if (comm->me == 0) {
    ...
    utils::sfread(FLERR, &delta[1], sizeof(double), atom->nimpropertypes, fp, nullptr, error);
  }
  MPI_Bcast(&k[1], atom->nimpropertypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&multiplicity[1], atom->nimpropertypes, MPI_INT, 0, world);
  MPI_Bcast(&delta[1], atom->nimpropertypes, MPI_DOUBLE, 0, world);
...
void ImproperPeriodic::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nimpropertypes; i++)
    fprintf(fp, "%d %g %d %g\n", i, k[i], multiplicity[i], delta[i] * 180.0 / MY_PI);
}
...
void *ImproperPeriodic::extract(const char *str, int &dim)
{
  dim = 1;
