...
PairStyle(gmu/pinn, PairGMUPINN)
...
class PairGMUPINN : public Pair {
 public:
  PairGMUPINN(class LAMMPS *lmp);
  ~PairGMUPINN();
  ...
  double pinn_fs(const double /*r*/, const int /*gaussian position*/);
  double ann_fs_d(double, int, Param *);
  double pinn_fs_d(const double /*r*/, const int /*gaussian position*/);
  void compute_atomic_LSP(const int, double *);
  //void write_potential(); // only for debugging
  ...
  void force_Sijk_ri(double /*prefactor*/, double */*vector of primary bond*/,
                     double */*vector of other bond*/, double /*length of primary bond*/,
                     double /*length of other bond*/, double /*r12*/,
                     double /*angle between the bonds*/, double */*force on center atom*/,
                     double */*force on primary neighbor*/, double */*force on other neighbor*/);
  void dcostheta_ijk_dri(double cstheta, double *drj,
                         double *drk, double *fj, double *fk);

  void unpack_vec_to_bop_sets(const double */*array*/, const int /*array size*/);

  void force_costheta_ijk_ri(double /*prefactor*/, double /*CosineTheta*/,
                             double */*vector r_ij*/, double */*vector r_ik*/,
                             double */*force_on_i*/, double */*force_on_j*/,
                             double */*force_on_k*/);
  void iderivs_wrt_bops(const int /*myid*/, const double /*eng*/);
  void pack_derivs_wrt_bop_sets_to_vec(double */*array*/, const int /*size*/);

  inline double signof(const double a)
  {
    if (a >= 0.0) return 1.0;
    else return -1.0;
  }
