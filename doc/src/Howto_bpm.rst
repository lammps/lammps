...
between the two particles, but can also include history data such as the
bond orientation for rotational models. This produces a stress-free initial
...
In this implementation, bonds store their reference state and set any
other history data when they are first computed in the setup of the first
simulation run. Data is then preserved across run commands and is written
to :doc:`binary restart files <restart>` such that restarting the system
will not reset the reference state of a bond. Bonds that are created midway
into a run, such as those created by pouring grains using :doc:`fix pour
...
Each BPM bond stores history data (such as a reference state, see above)
typically set from the initial configuration and is preserved only in :doc:`binary
restart files <restart>`.  The *write/history* and *read/history*
keywords, available for all BPM bond styles, provides an additional route to
save this data to a text file and later restore it.  This is useful for
restoring bonds' history states after reading a :doc:`data file <read_data>`
(which does not preserve it), or for initializing a simulation with a
reference state generated by a previous run, such as a pre-strained or
relaxed state.

The *write/history* keyword takes the ID of an internal fix and an
interval *N*, e.g. ``write/history myfix 1000``.  Every *N* timesteps the
current bond data, the IDs of the two bonded atoms followed by that
bond's stored history, is transferred to the internal fix *myfix*.  This
data can then be written to a file with the :doc:`dump local <dump>` command,

.. code-block:: LAMMPS

   bond_style bpm/spring write/history myfix 1000
   dump 1 all local 1000 bond*.ref f_myfix[*]

Here ``f_myfix[*]`` outputs all columns of the internal fix and the
``bond*.ref`` wildcard writes one file per dump step.  Unlike the
broken-bond output described above, the standard :doc:`dump local <dump>`
header must be retained (do *not* use :doc:`dump_modify <dump_modify>`
*header no*): the reader locates the bond data using the ``ITEM: NUMBER OF
ENTRIES`` and ``ITEM: ENTRIES`` lines that the header provides.

The *read/history* keyword takes the name of a file to read, e.g.
``read/history bond.ref``.  The file is read when the bond style is defined.
Note that history file data overrides the history data that would otherwise
...
The expected format for a history file is an ordinary
...
.. code-block:: none
   ...
   0.0000000000000000e+00 1.0000000000000000e+01
   0.0000000000000000e+00 1.0000000000000000e+01
   ITEM: ENTRIES f_myfix[1] f_myfix[2] f_myfix[3]
   1 2 1.0025
   2 3 0.9981
   3 4 1.0004
   4 5 0.9972
...
The columns following the two atom IDs correspond to the internal bond
history in the exact order it is stored by each bond style.  The table
below lists this order so that a history file can be constructed
correctly.  Column 1 and column 2 are always the IDs of the two
atoms in the bond; columns 3 and beyond are the bond history:

+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| Bond style                                                         | Col 1 | Col 2 | Col 3       | Col 4                      | Col 5                                  | Col 6                 | Col 7          | Col 8          | Col 9        |
+====================================================================+=======+=======+=============+============================+========================================+=======================+================+================+==============+
| :doc:`bpm/spring <bond_bpm_spring>`                                | id1   | id2   | :math:`r_0` |                            |                                        |                       |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/spring/plastic <bond_bpm_spring_plastic>`                | id1   | id2   | :math:`r_0` | :math:`e_p`                |                                        |                       |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/zero <bond_bpm_zero>`                                    | id1   | id2   | :math:`r_0` |                            |                                        |                       |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/rotational <bond_bpm_rotational>` (*damping derivative*) | id1   | id2   | :math:`r_0` | :math:`\hat{n}_{0,x}`      | :math:`\hat{n}_{0,y}`                  | :math:`\hat{n}_{0,z}` | :math:`\gamma` | :math:`\theta` | :math:`\psi` |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/rotational <bond_bpm_rotational>` (*damping dem*)        | id1   | id2   | :math:`r_0` | :math:`\hat{n}_{0,x}`      | :math:`\hat{n}_{0,y}`                  | :math:`\hat{n}_{0,z}` |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/peri <bond_bpm_peri>` (*pmb*, *lps*)                     | id1   | id2   | :math:`r_0` |                            |                                        |                       |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/peri <bond_bpm_peri>` (*eps*)                            | id1   | id2   | :math:`r_0` | :math:`e^{\mathrm{dev}}_p` |                                        |                       |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+
| :doc:`bpm/peri <bond_bpm_peri>` (*ves*)                            | id1   | id2   | :math:`r_0` | :math:`e^{\mathrm{dev}}`   | :math:`e^{\mathrm{dev}}_\mathrm{back}` |                       |                |                |              |
+--------------------------------------------------------------------+-------+-------+-------------+----------------------------+----------------------------------------+-----------------------+----------------+----------------+--------------+

where the history variables are

* :math:`r_0` --- the reference (initial) bond length
* :math:`\hat{n}_{0,x}, \hat{n}_{0,y}, \hat{n}_{0,z}` --- components of the
  initial unit bond vector (stored pointing from the lower to the higher
  atom ID)
* :math:`\gamma, \theta, \psi` --- the previous-step shear, bending, and
  twisting angles, stored only for *damping derivative* and used to form
  the rate-dependent damping
* :math:`e_p` --- the accumulated plastic strain, so that the bond's
  current equilibrium length is :math:`(1 + e_p) r_0`
* :math:`e^{\mathrm{dev}}_p` --- the accumulated plastic deviatoric
  extension (*eps* model)
* :math:`e^{\mathrm{dev}}` and :math:`e^{\mathrm{dev}}_\mathrm{back}` ---
...
in the history file, depends on the *damping* setting and the constitutive
model, respectively.  When a :doc:`bpm/peri <bond_bpm_peri>` simulation mixes
bond types with different models, the history size is that of the largest model
in use: three columns of history if any *ves* bonds are present, otherwise two if
any *eps* bonds are present, otherwise one.
