  ...
  FixRigidNHSmallKokkos<DeviceType>(lmp, narg, arg)
{
  ...
  if (this->tstat_flag == 0 || this->pstat_flag == 0)
    this->error->all(FLERR,"Did not set temp or press for fix rigid/npt/small/kk");
  if (this->t_start <= 0.0 || this->t_stop <= 0.0)
    this->error->all(FLERR,"Target temperature for fix rigid/npt/small/kk cannot be 0.0");
  if (this->p_start[0] < 0.0 || this->p_start[1] < 0.0 || this->p_start[2] < 0.0 ||
      this->p_stop[0] < 0.0 || this->p_stop[1] < 0.0 || this->p_stop[2] < 0.0)
    this->error->all(FLERR,"Target pressure for fix rigid/npt/small/kk cannot be < 0.0");

  if (this->t_period <= 0.0)
    this->error->all(FLERR,"Fix rigid/npt/small/kk period must be > 0.0");

  if (this->t_chain < 1)
    this->error->all(FLERR,"Fix rigid npt/small/kk t_chain should not be less than 1");
  if (this->t_iter < 1)
    this->error->all(FLERR,"Fix rigid npt/small/kk t_iter should not be less than 1");
  if (this->t_order != 3 && this->t_order != 5)
    this->error->all(FLERR,"Fix rigid npt/small/kk t_order must be 3 or 5");
