...
FixNVECACLimit::FixNVECACLimit(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixNVECACLimit::initial_integrate(int vflag)
{
  ...
  for (int poly_counter = 0; poly_counter < poly_count[i];poly_counter++) {	
      for(int k=0; k<nodes_per_element; k++){	
        ...
        vsq = nodal_velocities[i][poly_counter][k][0]*nodal_velocities[i][poly_counter][k][0] + 
              nodal_velocities[i][poly_counter][k][1]*nodal_velocities[i][poly_counter][k][1] + 
              nodal_velocities[i][poly_counter][k][2]*nodal_velocities[i][poly_counter][k][2];
        if (vsq > vlimitsq) {
          scale = sqrt(vlimitsq/vsq);
          nodal_velocities[i][poly_counter][k][0] *= scale;
          nodal_velocities[i][poly_counter][k][1] *= scale;
          nodal_velocities[i][poly_counter][k][2] *= scale;
        }
...
void FixNVECACLimit::final_integrate()
{
  double dtfm,vsq,scale;
