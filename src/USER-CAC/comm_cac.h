...
class CommCAC : public CommTiled {
 public:
  ...
  virtual void forward_comm_fix(class Fix *, int size);    // forward comm from a Fix
  virtual void forward_comm_npair(class NPair *, int size);    // forward comm from a NPair
 ...
 private:
  ...
  int **sendsize,**recvsize, **other_sendsize, **other_recvsize; // size of buffer sent by each overlap proc per swap
  int **sendoffset,**recvoffset, **other_sendoffset, **other_recvoffset; // offset of buffer for each overlap proc per swap
