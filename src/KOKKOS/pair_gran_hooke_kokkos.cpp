...
{
  // gran/hooke keeps no shear history, so unlike gran/hooke/history it
  // does not have to swap the dummy fix for a NEIGH_HISTORY/KK one

  PairGranHooke::init_style();
  ...
  if (neighflag == FULL)
    error->all(FLERR,"Must use half neighbor list with gran/hooke/kk");
}
...
void PairGranHookeKokkos<DeviceType>::operator()(TagPairGranHookeCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG>, const int ii, EV_FLOAT &ev) const {
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    // force normalization: gran/hooke keeps no shear history, so the
    // tangential force is pure velocity damping capped by Coulomb friction

    const KK_FLOAT vrel = Kokkos::sqrt(vtr1*vtr1 + vtr2*vtr2 + vtr3*vtr3);

    const KK_FLOAT fn = xmu_kk * Kokkos::fabs(ccel*r);
    const KK_FLOAT fs = meff*gammat_kk*vrel;
    KK_FLOAT ft = static_cast<KK_FLOAT>(0.0);
    if (vrel != static_cast<KK_FLOAT>(0.0)) ft = Kokkos::fmin(fn,fs) / vrel;

    // tangential force due to tangential velocity damping

    const KK_FLOAT fs1 = -ft*vtr1;
    const KK_FLOAT fs2 = -ft*vtr2;
    const KK_FLOAT fs3 = -ft*vtr3;
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void PairGranHookeKokkos<DeviceType>::operator()(TagPairGranHookeCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG>, const int ii) const {
  EV_FLOAT ev;
  this->template operator()<NEIGHFLAG,NEWTON_PAIR,VFLAG>(TagPairGranHookeCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG>(), ii, ev);
}
