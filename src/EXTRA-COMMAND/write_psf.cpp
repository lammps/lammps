...
void WritePsf::write(const std::string &file)
{
  ...
  if (atom->molecular == Atom::MOLECULAR && (atom->nbonds || atom->nbondtypes)) {
    nbonds_local = pack_bond(nullptr);
  ...
  if (atom->molecular == Atom::MOLECULAR && (atom->nangles || atom->nangletypes)) {
    nangles_local = pack_angle(nullptr);
...
int WritePsf::pack_bond(tagint **buf)
{
  tagint *tag = atom->tag;
  int *mask = atom->mask;
...
int WritePsf::pack_angle(tagint **buf)
{
  tagint *tag = atom->tag;
  int *mask = atom->mask;
  ...
  for (i = 0; i < nlocal; i++)
    for (j = 0; j < num_angle[i]; j++) {
      if (angle_type[i][j] == 0 || !( mask[i] & groupbit) || !( mask[j] & groupbit) ) continue;
      if (buf) {
