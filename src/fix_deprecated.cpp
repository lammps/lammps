  ...
  Fix(lmp, narg, arg)
{
  ...
  if (my_style == "DEPRECATED") {
    ...
    utils::logmesg(lmp,"\nFix style 'lb/pc' is part of the old LATBOLTZ "
                     " package and is now defunct.\n  LAMMPS standard "
		     " fix NVE can be used in its place.\n\n");
  }
  else if (my_style == "lb/rigid/pc/sphere") {
    utils::logmesg(lmp,"\nFix style 'lb/rigid/pc/sphere' is part of the old "
		     " LATBOLTZ package and is now defunct.\n  LAMMPS standard "
		     " fix rigid can be used in its place.\n\n");
  }
