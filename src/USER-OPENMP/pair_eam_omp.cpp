...
void PairEAMOMP::compute(int eflag, int vflag)
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->sfree(fp);
    nmax = atom->nmax;
    rho = (double *) memory->smalloc(comm->nthreads*nmax*
				     sizeof(double),"pair:rho");
...
#endif
  {
    int i,j,ii,jj,m,inum,jnum,itype,jtype,tid;
    ...
    // set up loop ranges and thread info
    int iifrom, iito;
    f = loop_setup_thr(atom->f,iifrom,iito,tid,inum,nall,nthreads);
  
    // zero out density. rho_thr points to per thread storage
    double *rho_thr = rho + nmax*tid;
    if (NEWTON_PAIR) {
	...
	if (rsq < cutforcesq) {
	  ...
	  rho_thr[i] += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
	  if (NEWTON_PAIR || j < nlocal) {
	    coeff = rhor_spline[type2rhor[itype][jtype]][m];
	    rho_thr[j] += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
	  }
	}
      }
    }
...
#pragma omp master
      ...
      rho_reduce_thr(rho,nmax,nlocal,nthreads,tid);
    }
...
void PairEAMOMP::settings(int narg, char **arg)
{
...
void PairEAMOMP::coeff(int narg, char **arg)
{
...
double PairEAMOMP::init_one(int i, int j)
{
...
void PairEAMOMP::read_file(char *filename)
{
...
void PairEAMOMP::interpolate(int n, double delta, double *f, double **spline)
{
...
void PairEAMOMP::grab(FILE *fptr, int n, double *list)
{
  char *ptr;
  char line[MAXLINE];
...
double PairEAMOMP::single(int i, int j, int itype, int jtype,
...
int PairEAMOMP::pack_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
...
void PairEAMOMP::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int PairEAMOMP::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairEAMOMP::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void PairEAMOMP::swap_eam(double *fp_caller, double **fp_caller_hold)
{
  double *tmp = fp;
  fp = fp_caller;
  *fp_caller_hold = tmp;
