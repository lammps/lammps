...
#include "pair_lennard_mdf_kokkos.h"
...
PairLennardMDFKokkos<DeviceType>::PairLennardMDFKokkos(LAMMPS *lmp) : PairLennardMDF(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairLennardMDFKokkos<DeviceType>::~PairLennardMDFKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLennardMDFKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairLennardMDFKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairLennardMDFKokkos<DeviceType>::
compute_fpair(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const KK_FLOAT cut_inner_sq_val = (STACKPARAMS?m_params[itype][jtype].cut_inner_sq:params(itype,jtype).cut_inner_sq);
  KK_FLOAT forcelj = r6inv * (lj1_val*r6inv - lj2_val);

  if (rsq > cut_inner_sq_val) {
    const KK_FLOAT philj = r6inv * (lj3_val*r6inv - lj4_val);
    const KK_FLOAT rr = Kokkos::sqrt(rsq);
    const KK_FLOAT cut_val = (STACKPARAMS?m_params[itype][jtype].cut:params(itype,jtype).cut);
    ...
    const KK_FLOAT d = (rr - cut_inner_val) / dp;
    ...
    const KK_FLOAT dt = static_cast<KK_FLOAT>(30.0) * d*d * dd*dd * rr / dp;
...
compute_evdwl(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const KK_FLOAT lj4_val = (STACKPARAMS?m_params[itype][jtype].lj4:params(itype,jtype).lj4);
  const KK_FLOAT cut_inner_sq_val = (STACKPARAMS?m_params[itype][jtype].cut_inner_sq:params(itype,jtype).cut_inner_sq);
  KK_FLOAT evdwl = r6inv * (lj3_val*r6inv - lj4_val);
...
template<class DeviceType>
void PairLennardMDFKokkos<DeviceType>::allocate()
{
  PairLennardMDF::allocate();

  int n = atom->ntypes;
  memory->destroy(cutsq);
  memoryKK->create_kokkos(k_cutsq,cutsq,n+1,n+1,"pair:cutsq");
  d_cutsq = k_cutsq.template view<DeviceType>();
  k_params = Kokkos::DualView<params_lennard_mdf**,Kokkos::LayoutRight,DeviceType>("PairLennardMDF::params",n+1,n+1);
...
template<class DeviceType>
void PairLennardMDFKokkos<DeviceType>::init_style()
{
  PairLennardMDF::init_style();
...
template<class DeviceType>
double PairLennardMDFKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLennardMDF::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLennardMDFKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLennardMDFKokkos<LMPHostType>;
#endif
}
