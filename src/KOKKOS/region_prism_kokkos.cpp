...
RegPrismKokkos<DeviceType>::RegPrismKokkos(LAMMPS *lmp, int narg, char **arg)
  : RegPrism(lmp, narg, arg)
{
...
template<class DeviceType>
void RegPrismKokkos<DeviceType>::match_all_kokkos(int groupbit_in, DAT::tdual_int_1d k_match_in)
{
  ...
  copymode = 1;
  Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagRegPrismMatchAll>(0,nlocal),*this);
  copymode = 0;
...
void RegPrismKokkos<DeviceType>::operator()(TagRegPrismMatchAll, const int &i) const {
