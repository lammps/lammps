...
.. note::
   ...
   Kokkos with CUDA currently implicitly assumes that the MPI
   ...
   GPU-direct support.  These can be avoided by adding the flags :doc:`-pk kokkos gpu/direct off <Run_options>` to the LAMMPS command line or
   by using the command :doc:`package kokkos gpu/direct off <package>` in
   the input file.
...
avoided by using :doc:`-pk kokkos gpu/direct no <package>`. As above for 
