...
FixBocs::FixBocs(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg), rfix(NULL), id_dilate(NULL), irregular(NULL), id_temp(NULL),
    id_press(NULL), eta(NULL), eta_dot(NULL), eta_dotdot(NULL), eta_mass(NULL), etap(NULL),
    etap_dot(NULL), etap_dotdot(NULL), etap_mass(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "temp") == 0) {
      ...
      if (strcmp(arg[iarg], "analytic") == 0) {
        ...
        iarg += 4;
        if (iarg + N_p_match > narg) error->all(FLERR, "Illegal fix bocs command. Missing coeffs.");
        ...
        if (iarg + 2 > narg)
          error->all(FLERR, "Illegal fix bocs command. "
        ...
        std::string errmsg = fmt::format("CG basis type {} is not recognized\nSupported "
                                         "basis types: analytic linear_spline cubic_spline",
                                         arg[iarg]);
      ...
      std::string errmsg =
          fmt::format("Illegal fix bocs command: unrecognized keyword {}", arg[iarg]);
      error->all(FLERR, errmsg);
    }
  }
  // error checks
  ...
  if (dipole_flag) {
    ...
    if (!atom->mu_flag) error->all(FLERR, "Using update dipole flag requires atom attribute mu");
  }

  if ((tstat_flag && t_period <= 0.0) || (p_flag[0] && p_period[0] <= 0.0) ||
      (p_flag[1] && p_period[1] <= 0.0) || (p_flag[2] && p_period[2] <= 0.0) ||
      (p_flag[3] && p_period[3] <= 0.0) || (p_flag[4] && p_period[4] <= 0.0) ||
  ...
  if (!tstat_flag) error->all(FLERR, "Temperature control must be used with fix bocs");
  if (!pstat_flag) error->all(FLERR, "Pressure control must be used with fix bocs");
...
void FixBocs::init()
{
  ...
  if (allremap == 0) {
    ...
    if (idilate == -1) error->all(FLERR, "Fix bocs dilate group ID does not exist");
  ...
  if (icompute < 0) error->all(FLERR, "Temperature ID for fix bocs does not exist");
  ...
  if (pstat_flag) {
    ...
    if (icompute < 0) error->all(FLERR, "Pressure ID for fix bocs does not exist");
    ...
    if (p_match_flag) // MRD NJD
    {
      if (pressure) {
        if (p_basis_type == BASIS_ANALYTIC) {
          ((ComputePressureBocs *)pressure)
              ->send_cg_info(p_basis_type, N_p_match, p_match_coeffs, N_mol, vavg);
        }
  ...
  if (tstat_flag) tdrag_factor = 1.0 - (update->dt * t_freq * drag / nc_tchain);
...
int FixBocs::read_F_table(char *filename, int p_basis_type)
{
  ...
  if (fpi) {
    ...
    for (int i = 0; i < inputLines.size(); ++i) {
      ...
      if (test_sscanf == 2) {
        ...
        if (i == 1) {
          ...
          // if (comm->me == 0) {
          //    error->message(FLERR, fmt::format("INFO: standard volume interval computed: {}",
          //    stdVolumeInterval));
          //}
        }
        else if (i > 1) {
          ...
          // if (comm->me == 0) {
          //    error->message(FLERR, fmt::format("INFO: current volume interval: {}",
          //    currVolumeInterval));
          //}
    ...
    if (numBadVolumeIntervals > 0 && comm->me == 0) {
      error->message(FLERR, fmt::format("INFO: total number bad volume intervals = {}",
                                        numBadVolumeIntervals));
    }
  }
  else {
  ...
  if (badInput && comm->me == 0) {
    error->warning(
        FLERR,
        fmt::format("Bad volume / pressure-correction data: {}\nSee details above", filename));
  }
  ...
  if (p_basis_type == BASIS_LINEAR_SPLINE) {
    ...
    error->all(FLERR, fmt::format("ERROR: invalid p_basis_type value of {} in read_F_table",
                                  p_basis_type));
  }
...
{
  // if (comm->me == 0) {
  // error->message(FLERR, fmt::format("INFO: entering build_linear_splines, spline_length = {}",
  // spline_length));
  //}
  ...
  if (comm->me == 0) {
    error->message(FLERR, fmt::format("INFO: leaving build_linear_splines, spline_length = {}",
                                      spline_length));
  }
...
{
  // if (comm->me == 0) {
  // error->message(FLERR, fmt::format("INFO: entering build_cubic_splines, spline_length = {}",
  // spline_length));
  //}
  int n = spline_length;
  ...
  for (int i = 0; i < n; i++) {
    ...
    double alpha_i;
    if (i > 1 && i < (n - 1)) {
      alpha_i = (3.0 / h[i]) * (data[1][i + 1] - data[1][i]) -
                (3.0 / h[i - 1]) * (data[1][i] - data[1][i - 1]);
      alpha[i - 1] = alpha_i;
    }
  }
  l[0]  = 1.0;
  ...
  if (comm->me == 0) {
    error->message(FLERR,
                   fmt::format("INFO: leaving build_cubic_splines, numSplines = {}", numSplines));
  }
...
void FixBocs::setup(int /*vflag*/)
{
  ...
  if (tstat_flag) {
    ...
    for (int ich = 1; ich < mtchain; ich++) {
      eta_dotdot[ich] =
          (eta_mass[ich - 1] * eta_dot[ich - 1] * eta_dot[ich - 1] - boltz * t_target) /
          eta_mass[ich];
    }
  }
  ...
  if (pstat_flag) {
    ...
    if (mpchain) {
      ...
      for (ich = 1; ich < mpchain; ich++)
        energy += kt * etap[ich] + 0.5 * etap_mass[ich] * etap_dot[ich] * etap_dot[ich];
    }
...
double FixBocs::compute_vector(int n)
{
  ...
  if (pstat_flag) {
    if (pstyle == ISO) {
      ilen = 1;
      if (n < ilen) return p_hydro * (volume - vol0) / nktv2p;
      n -= ilen;
    }
    else if (pstyle == ANISO) {
      ilen = 3;
    ...
    if (pstyle == ISO) {
      ilen = 1;
      if (n < ilen) return pdim * 0.5 * omega_dot[n] * omega_dot[n] * omega_mass[n];
      n -= ilen;
    }
    else if (pstyle == ANISO) {
      ilen = 3;
...
void FixBocs::reset_dt()
{
  ...
  if (strstr(update->integrate_style, "respa")) dto = 0.5 * step_respa[0];

  if (pstat_flag) pdrag_factor = 1.0 - (update->dt * p_freq_max * drag / nc_pchain);
...
void FixBocs::compute_sigma()
{
  ...
  sigma[0] = vol0 * (h0_inv[0] * ((p_target[0] - p_hydro) * h0_inv[0] + p_target[5] * h0_inv[5] +
                                  p_target[4] * h0_inv[4]) +
                     h0_inv[5] * (p_target[5] * h0_inv[0] + (p_target[1] - p_hydro) * h0_inv[5] +
                                  p_target[3] * h0_inv[4]) +
                     h0_inv[4] * (p_target[4] * h0_inv[0] + p_target[3] * h0_inv[5] +
                                  (p_target[2] - p_hydro) * h0_inv[4]));
  sigma[1] = vol0 * (h0_inv[1] * ((p_target[1] - p_hydro) * h0_inv[1] + p_target[3] * h0_inv[3]) +
                     h0_inv[3] * (p_target[3] * h0_inv[1] + (p_target[2] - p_hydro) * h0_inv[3]));
  sigma[2] = vol0 * (h0_inv[2] * ((p_target[2] - p_hydro) * h0_inv[2]));
  sigma[3] = vol0 * (h0_inv[1] * (p_target[3] * h0_inv[2]) +
                     h0_inv[3] * ((p_target[2] - p_hydro) * h0_inv[2]));
  sigma[4] = vol0 * (h0_inv[0] * (p_target[4] * h0_inv[2]) + h0_inv[5] * (p_target[3] * h0_inv[2]) +
                     h0_inv[4] * ((p_target[2] - p_hydro) * h0_inv[2]));
  sigma[5] = vol0 * (h0_inv[0] * (p_target[5] * h0_inv[1] + p_target[4] * h0_inv[3]) +
...
double FixBocs::compute_strain_energy()
{
  ...
  d0 = sigma[0] * (h[0] * h[0] + h[5] * h[5] + h[4] * h[4]) +
       sigma[5] * (h[1] * h[5] + h[3] * h[4]) + sigma[4] * (h[2] * h[4]);
  d1 = sigma[5] * (h[5] * h[1] + h[4] * h[3]) + sigma[1] * (h[1] * h[1] + h[3] * h[3]) +
       sigma[3] * (h[2] * h[3]);
  d2 = sigma[4] * (h[4] * h[2]) + sigma[3] * (h[3] * h[2]) + sigma[2] * (h[2] * h[2]);
...
void FixBocs::compute_deviatoric()
{
  ...
  fdev[0] = h[0] * (sigma[0] * h[0] + sigma[5] * h[5] + sigma[4] * h[4]) +
            h[5] * (sigma[5] * h[0] + sigma[1] * h[5] + sigma[3] * h[4]) +
            h[4] * (sigma[4] * h[0] + sigma[3] * h[5] + sigma[2] * h[4]);
  fdev[1] = h[1] * (sigma[1] * h[1] + sigma[3] * h[3]) + h[3] * (sigma[3] * h[1] + sigma[2] * h[3]);
  fdev[2] = h[2] * (sigma[2] * h[2]);
  fdev[3] = h[1] * (sigma[3] * h[2]) + h[3] * (sigma[2] * h[2]);
  fdev[4] = h[0] * (sigma[4] * h[2]) + h[5] * (sigma[3] * h[2]) + h[4] * (sigma[2] * h[2]);
  fdev[5] = h[0] * (sigma[5] * h[1] + sigma[4] * h[3]) +
            h[5] * (sigma[1] * h[1] + sigma[3] * h[3]) + h[4] * (sigma[3] * h[1] + sigma[2] * h[3]);
}
...
void FixBocs::nh_omega_dot()
{
  ...
  for (int i = 0; i < 3; i++)
    if (p_flag[i]) {
      f_omega =
          (p_current[i] - p_hydro) * volume / (omega_mass[i] * nktv2p) + mtk_term1 / omega_mass[i];
