...
#include "angle_fourier_simple_kokkos.h"
...
AngleFourierSimpleKokkos<DeviceType>::AngleFourierSimpleKokkos(LAMMPS *lmp) : AngleFourierSimple(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
AngleFourierSimpleKokkos<DeviceType>::~AngleFourierSimpleKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void AngleFourierSimpleKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleFourierSimpleCompute<1,1> >(0,nanglelist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleFourierSimpleCompute<0,1> >(0,nanglelist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleFourierSimpleCompute<1,0> >(0,nanglelist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleFourierSimpleCompute<0,0> >(0,nanglelist),*this);
    }
  }
...
void AngleFourierSimpleKokkos<DeviceType>::operator()(TagAngleFourierSimpleCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  if (fabs(c) - static_cast<KK_FLOAT>(1.0) > static_cast<KK_FLOAT>(SMALL)) {
    ...
    a = d_k[type] * d_C[type] * d_N[type] * sgn * a;
  }
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleFourierSimpleKokkos<DeviceType>::operator()(TagAngleFourierSimpleCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagAngleFourierSimpleCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void AngleFourierSimpleKokkos<DeviceType>::allocate()
{
  AngleFourierSimple::allocate();

  int n = atom->nangletypes;
  k_k = DAT::tdual_kkfloat_1d("AngleFourierSimple::k",n+1);
  k_C = DAT::tdual_kkfloat_1d("AngleFourierSimple::C",n+1);
  k_N = DAT::tdual_kkfloat_1d("AngleFourierSimple::N",n+1);

  d_k = k_k.template view<DeviceType>();
  d_C = k_C.template view<DeviceType>();
  d_N = k_N.template view<DeviceType>();
}
...
template<class DeviceType>
void AngleFourierSimpleKokkos<DeviceType>::coeff(int narg, char **arg)
{
  AngleFourierSimple::coeff(narg, arg);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    k_N.view_host()[i] = static_cast<KK_FLOAT>(N[i]);
  }
...
template<class DeviceType>
void AngleFourierSimpleKokkos<DeviceType>::read_restart(FILE *fp)
{
  AngleFourierSimple::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleFourierSimpleKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
