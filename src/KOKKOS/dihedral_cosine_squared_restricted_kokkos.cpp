...
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::operator()(TagDihedralCosineSquaredRestrictedCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT b1mag2 = 1.0 / sb1;
  const KK_FLOAT b1mag = sqrt(b1mag2);
  const KK_FLOAT b2mag2 = 1.0 / sb2;
  const KK_FLOAT b2mag = sqrt(b2mag2);
  const KK_FLOAT b3mag2 = 1.0 / sb3;
  ...
  const KK_FLOAT p0 = cos(d_phi0[type]);
  const KK_FLOAT sq_sin = 1.0 - c*c;

  const KK_FLOAT pd = 2.0*d_k[type]*(c - p0)*(1.0 - c*p0) / (sq_sin*sq_sin);
  ...
  const KK_FLOAT a22 = -sb2*(2.0*c0*s12a - cs*(s1+s2));
