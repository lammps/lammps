...
#include "pair_lj96_cut_kokkos.h"
...
PairLJ96CutKokkos<DeviceType>::PairLJ96CutKokkos(LAMMPS *lmp) : PairLJ96Cut(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairLJ96CutKokkos<DeviceType>::~PairLJ96CutKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLJ96CutKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairLJ96CutKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairLJ96CutKokkos<DeviceType>::
...
template<class DeviceType>
void PairLJ96CutKokkos<DeviceType>::allocate()
{
  PairLJ96Cut::allocate();

  int n = atom->ntypes;
  memory->destroy(cutsq);
  memoryKK->create_kokkos(k_cutsq,cutsq,n+1,n+1,"pair:cutsq");
  d_cutsq = k_cutsq.template view<DeviceType>();
  k_params = Kokkos::DualView<params_lj**,Kokkos::LayoutRight,DeviceType>("PairLJ96Cut::params",n+1,n+1);
...
template<class DeviceType>
void PairLJ96CutKokkos<DeviceType>::init_style()
{
  PairLJ96Cut::init_style();
...
template<class DeviceType>
double PairLJ96CutKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLJ96Cut::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLJ96CutKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLJ96CutKokkos<LMPHostType>;
#endif
}
