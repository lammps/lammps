...
public:
  ...
  void spectrum(int natoms,int* numneighs,int* jelems,double* wjelem,double** rij,
...
private:
  ...
  void get_sbes_array(int natoms,int* numneighs,int* jelems,double* wjelem,double** rij,
  		int nmax, int lmax,double rcut, double alpha,int ncoefs);
  void get_rip_array(int natoms,int* numneighs,int* jelems,double* wjelem,double** rij,
  		int nmax, int lmax,double rcut, double alpha,int ncoefs);
  void init_arrays(int natoms,int* numneighs,int ncoefs);
  void init_garray(int nmax, int lmax, double rcut, double alpha, double *w, int lw1, int lw2,
  		              double *g_array, int lg1, int lg2);

  void compute_uarray_recursive(double x, double y, double z, double r,
  		int twol, double *ulist_r, double *ulist_i, int *idxu_block, double *rootpqarray,
  		int roi1,int roi2);
  void compute_ncoeff();

  int get_sum(int istart,int iend, int id,int imult);

  void compute_dpidrj(int nmax, int lmax, double *clisttot_r,double *clisttot_i,int lctot1,int lctot2,
  		double *dclist_r,double *dclist_i,int ldcli1,int ldcli2,int ldcli3,
  		double *dplist_r,int dpli1,int dpli2);


  double g(double r,int n,int nmax,double rcut,double *w,int lw1, int lw2);
  double phi(double r,int alpha,double rcut);


  void compute_pi(int nmax,int lmax, double *clisttot_r, double *clisttot_i, int lcl1,int lcl2,
  		double *plist_r, double *plist_i, int lpl1,int lpl2,int indpl);
