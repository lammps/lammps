...
DumpCAC_Kinetic::DumpCAC_Kinetic(LAMMPS *lmp, int narg, char **arg) : Dump(lmp, narg, arg)
{
...
DumpCAC_Kinetic::~DumpCAC_Kinetic()
{
...
void DumpCAC_Kinetic::init_style()
{
  ...
  if (buffer_flag == 1) write_choice = &DumpCAC_Kinetic::write_string;
  else write_choice = &DumpCAC_Kinetic::write_lines;
...
int DumpCAC_Kinetic::modify_param(int narg, char **arg)
{
...
void DumpCAC_Kinetic::pack(tagint *ids)
{
  int m,n;
...
int DumpCAC_Kinetic::convert_string(int n, double *mybuf)
{
  int offset = 0;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCAC_Kinetic::write_data(int n, double *mybuf)
{
...
void DumpCAC_Kinetic::write_string(int n, double *mybuf)
{
...
void DumpCAC_Kinetic::write_lines(int n, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,format,
