...
#include "angle_fourier_kokkos.h"
...
AngleFourierKokkos<DeviceType>::AngleFourierKokkos(LAMMPS *lmp) : AngleFourier(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
AngleFourierKokkos<DeviceType>::~AngleFourierKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void AngleFourierKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleFourierCompute<1,1> >(0,nanglelist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleFourierCompute<0,1> >(0,nanglelist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleFourierCompute<1,0> >(0,nanglelist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleFourierCompute<0,0> >(0,nanglelist),*this);
    }
  }
...
void AngleFourierKokkos<DeviceType>::operator()(TagAngleFourierCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT c2 = static_cast<KK_FLOAT>(2.0)*c*c - static_cast<KK_FLOAT>(1.0);

  KK_FLOAT eangle = static_cast<KK_FLOAT>(0.0);
  if (eflag) eangle = d_k[type]*(d_C0[type] + d_C1[type]*c + d_C2[type]*c2);

  const KK_FLOAT a = d_k[type]*(d_C1[type] + static_cast<KK_FLOAT>(4.0)*d_C2[type]*c);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleFourierKokkos<DeviceType>::operator()(TagAngleFourierCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagAngleFourierCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void AngleFourierKokkos<DeviceType>::allocate()
{
  AngleFourier::allocate();

  int n = atom->nangletypes;
  k_k  = DAT::tdual_kkfloat_1d("AngleFourier::k",n+1);
  k_C0 = DAT::tdual_kkfloat_1d("AngleFourier::C0",n+1);
  k_C1 = DAT::tdual_kkfloat_1d("AngleFourier::C1",n+1);
  k_C2 = DAT::tdual_kkfloat_1d("AngleFourier::C2",n+1);
...
template<class DeviceType>
void AngleFourierKokkos<DeviceType>::coeff(int narg, char **arg)
{
  AngleFourier::coeff(narg, arg);
...
template<class DeviceType>
void AngleFourierKokkos<DeviceType>::read_restart(FILE *fp)
{
  AngleFourier::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleFourierKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
...
namespace LAMMPS_NS {
template class AngleFourierKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class AngleFourierKokkos<LMPHostType>;
#endif
}
