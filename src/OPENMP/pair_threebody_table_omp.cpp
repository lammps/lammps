...
#include "pair_threebody_table_omp.h"
...
PairThreebodyTableOMP::PairThreebodyTableOMP(LAMMPS *lmp) :
  PairThreebodyTable(lmp), ThrOMP(lmp, THR_PAIR)
{
...
void PairThreebodyTableOMP::compute(int eflag, int vflag)
{
  ev_init(eflag,vflag);
...
void PairThreebodyTableOMP::eval(int iifrom, int iito, ThrData * const thr)
{
  ...
  for (ii = iifrom; ii < iito; ++ii) {
    ...
    // two-body interactions: only used to build short neighbor list
    // (threebody/table has no two-body forces)
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      // two-body interactions are not computed for threebody/table
    }
...
double PairThreebodyTableOMP::memory_usage()
{
  double bytes = memory_usage_thr();
  bytes += PairThreebodyTable::memory_usage();
