...
#include "pair_lj_cut_soft_kokkos.h"
...
PairLJCutSoftKokkos<DeviceType>::PairLJCutSoftKokkos(LAMMPS *lmp) : PairLJCutSoft(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairLJCutSoftKokkos<DeviceType>::~PairLJCutSoftKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLJCutSoftKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairLJCutSoftKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairLJCutSoftKokkos<DeviceType>::
...
template<class DeviceType>
void PairLJCutSoftKokkos<DeviceType>::allocate()
{
  PairLJCutSoft::allocate();

  int n = atom->ntypes;
  memory->destroy(cutsq);
  memoryKK->create_kokkos(k_cutsq,cutsq,n+1,n+1,"pair:cutsq");
  d_cutsq = k_cutsq.template view<DeviceType>();
  k_params = Kokkos::DualView<params_lj**,Kokkos::LayoutRight,DeviceType>("PairLJCutSoft::params",n+1,n+1);
...
template<class DeviceType>
void PairLJCutSoftKokkos<DeviceType>::init_style()
{
  PairLJCutSoft::init_style();
...
template<class DeviceType>
double PairLJCutSoftKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLJCutSoft::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLJCutSoftKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLJCutSoftKokkos<LMPHostType>;
#endif
}
