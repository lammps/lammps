...
LbalanceSimple::LbalanceSimple(class LAMMPS *lmp,int narg, char **arg)
  : Lbalance(lmp,narg,arg) {

  if (narg < iarg+2)
    error->all(FLERR,"Loadbalance simple: Not enough arguments");

  if (strcmp(arg[iarg++],"ntry"))
    error->all(FLERR,"Loadbalance nlocal/simple: Expecting 'ntry'");

  ntry_simple = atoi(arg[iarg++]);
  if (ntry_simple < 1)
    error->all(FLERR,"Loadbalance max: ntry too small");

  if (ntry_simple > 10)
    error->warning(FLERR,"Loadbalance max: ntry >10 might result in high comm cost");
}
...
void LbalanceSimple::loadbalance_local_boxes() {
  ...
  if (domain->triclinic)
    error->all(FLERR,"Load balancing not implemented for triclinic boxes");

  if (strstr(update->integrate_style,"respa") != NULL)
    error->all(FLERR,"Cannot use r-RESPA with load balancing");
...
void LbalanceSimple::loadbalance_local_boxes_simple() {
  ...
  for (idim = 0; idim < 3; idim++) {
    ...
    for(idim_proc = lodim[idim]; idim_proc <= hidim[idim]; idim_proc++) {
      ...
      //NP this ensures that no proc is skipped - 
      //   i.e. particles are always communicated to a neigh proc
	...
	bal_sublo[idim] = boxlo[idim];
      ...
      if (idim_proc == procgrid[idim]-1)
	bal_subhi[idim] = boxhi[idim];
      else {
...
double LbalanceSimple::calc_border(int ntry,int dim,int ncount_ideal,
				   double *border,int *ncount) {
  double btemp;
  int ntemp;
  ...
  if(ntry > 0) {
    if (ncount[0] > ncount_ideal || ncount[2] < ncount_ideal)
      error->all(FLERR,"Illegal situation in LbalanceSimple::calc_border");
  ...
  for(int i = 0; i < 3; i++)
    rel_dev[i] = fabs(static_cast<double>(ncount[i]-ncount_ideal) 
		      / static_cast<double>(ncount_ideal)) ;

  //NP return the border with minimum deviation
  if (rel_dev[0] < rel_dev[1] && rel_dev[0] < rel_dev[2])
    return border[0];
  else 
    if(rel_dev[1] < rel_dev[2])
      return border[1];
    else
      return border[2];
}
...
void LbalanceSimple::calc_max_shift(int idim,int idim_proc) {
  ...
  for(int i = 0; i < 2; i++) {
    if (myloc[idim] == idim_proc+i)
      max_shift[i] = subhi[idim] - sublo[idim];
  ...
  if (idim_proc == comm->procgrid[idim]-1)
    max_shift[1] = 0.;
}
...
void LbalanceSimple::apply_border(double *bal_sublo, double *bal_subhi,
				  int idim,int idim_proc) {

  double subhi_final,subhi_final_all, boxhi_stencil;
  double boxhi_stencil_all,bal_subhi_max;

  //NP get boxhi for the stencil
  boxhi_stencil = - BIG;
  if (myloc[idim] <= hidim[idim])
    boxhi_stencil = subhi[idim];
  ...
  if (bal_subhi[idim] > bal_subhi_max)
    bal_subhi[idim] = bal_subhi_max;
  ...
  if (myloc[idim] == idim_proc) {
    ...
    if (bal_subhi[idim] - bal_sublo[idim] < minextent)
      bal_subhi[idim] = bal_sublo[idim] + minextent;
    ...
    if (subhi[idim] - sublo[idim] < minextent)
      error->warning(FLERR,"Minimum sub-domain extent could not be obeyed because particles\n"
		     "would have been lost (did you insert large particles?). Inaccuracies may result.");
