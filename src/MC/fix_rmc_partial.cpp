    ...
    rng_type_destination(device_type_destination()), Fix(lmp, narg, arg)
{
  ...
  if (rmcdata.contains("barrier")) {
    ...
    if (rmcdata["barrier"].size() != num_charge_states) {
      error->all(FLERR, 3,
                 "Number of barriers provided inconsistent with number of charge states.");
    }
    for (int i = 0; i < num_charge_states; i++) delta_g_list[i] = rmcdata["barrier"][i];
  } else {
  ...
  if (rmcdata.contains("semiconductor_charges")) {
    ...
    if (rmcdata["semiconductor_charges"].contains("neutral") &&
        rmcdata["semiconductor_charges"].contains("charged")) {
      for (int i = 0; i < semiconductor_size; i++) {
        semiconductor_charges[0][i] = rmcdata["semiconductor_charges"]["neutral"][i];
        semiconductor_charges[num_charge_states - 1][i] =
            rmcdata["semiconductor_charges"]["charged"][i];
      }
    } else {
      error->all(
          FLERR, 3,
          "The neutral and/or charged values missing from JSON input semiconductor_charges field");
    }
  } else {
  ...
  if (rmcdata.contains("dopant_charges")) {
    if (rmcdata["dopant_charges"].contains("neutral") &&
        rmcdata["dopant_charges"].contains("charged")) {
      ...
      error->all(FLERR, 3,
                 "The neutral and/or charged values missing from JSON input dopant_charges field");
    }
  } else {
  ...
  for (int i = 0; i < semiconductor_size; i++) {
    temp_delta_semiconductor =
        (semiconductor_charges[num_charge_states - 1][i] - semiconductor_charges[0][i]) /
        (num_charge_states - 1);
  ...
  for (int i = 0; i < dopant_size; i++) {
    temp_delta_dopant =
        (dopant_charges[num_charge_states - 1][i] - dopant_charges[0][i]) / (num_charge_states - 1);
    for (int j = 0; j < num_charge_states - 1; j++) {
  ...
  charges[0] = 0;
  for (int i = 0; i < num_charge_states - 1; i++) charges[i + 1] = charges[i] + deltaQ;
  ...
  if (rmcdata.contains("dihedral_modification")) {
    if (std::string(rmcdata["dihedral_modification"]) == "y") {
      ...
      if (!rmcdata.contains("dihedral_types") || !rmcdata.contains("dihedral_list")) {
        error->all(
            FLERR, 3,
            "The dihedral_types and/or dihedral_list field are missing in the JSON input file");
      }
    } else {
      do_dihedral = 0;
    }
  ...
  if (rmcdata.contains("angle_modification")) {
    if (std::string(rmcdata["angle_modification"]) == "y") {
      ...
      if (!rmcdata.contains("angle_types") || !rmcdata.contains("angle_list")) {
        error->all(FLERR, 3,
                   "The angle_types and/or angle_list field are missing in the JSON input file");
      }
    } else {
      do_angle = 0;
    }
  ...
  if (rmcdata.contains("bond_modification")) {
    if (std::string(rmcdata["bond_modification"]) == "y") {
      ...
      if (!rmcdata.contains("bond_types") || !rmcdata.contains("bond_list")) {
        error->all(FLERR, 3,
                   "The bond_types and/or bond_list field are missing in the JSON input file");
      }
    } else {
      do_bond = 0;
    }
  ...
  if (do_dihedral == 1) {
    ...
    for (int i = 0; i < num_dihedrals; i++) {
      ...
      for (int j = 0; j < 4; j++) dihedral_list[i][j] = rmcdata["dihedral_list"][i][j];
  ...
  if (do_angle == 1) {
    ...
    for (int i = 0; i < num_charge_states; i++) angle_types[i] = rmcdata["angle_types"][i];
  ...
  if (do_bond == 1) {
    ...
    for (int i = 0; i < num_charge_states; i++) bond_types[i] = rmcdata["bond_types"][i];
    bond_list = new int *[num_bonds];
    for (int i = 0; i < num_bonds; i++) {
      bond_list[i] = new int[3];
      for (int j = 0; j < 2; j++) bond_list[i][j] = rmcdata["bond_list"][i][j];
  ...
  for (int i = 0; i < semiconductor_size; i++) {
    ...
    if (i < dopant_size) dopant_mol_c[i] = new double[6];
  }
  ...
  if (comm->me == 0) {
    ...
    mesg += "Charges: ";
    for (int i = 0; i < num_charge_states; i++) mesg += fmt::format("{} ", charges[i]);
    ...
    mesg += "\nReaction Energies: ";
    for (int i = 0; i < num_charge_states; i++) mesg += fmt::format("{} ", delta_g_list[i]);
    mesg += "\nbetaDelta_G: ";
    for (int i = 0; i < num_charge_states; i++) {
    ...
    if (do_bond == 1) {
      mesg += "Bond types: ";
      for (int i = 0; i < num_charge_states; i++) mesg += fmt::format("{} ", bond_types[i]);
      mesg += fmt::format("\nNumber of bonds: {}\n", num_bonds);
    }
...
int FixRMCPartial::determine_molecule(int global_id)
{
  ...
  for (int i = 0; i < atom->nlocal; i++) {
    if (atom->tag[i] == global_id) mol_id = atom->molecule[i];
  }
...
void FixRMCPartial::calculateMoleculeCOM(double *com, struct Mol *molecule)
{
  ...
  for (int i = 0; i < molecule->local_atoms; i++) {
    ...
    for (int j = 0; j < 3; j++) local_com[j] = local_com[j] + molecule->mass[i] * unwrap[j];
  ...
  for (int j = 0; j < 3; j++) com[j] = com[j] * (1.0 / global_mass);
...
void FixRMCPartial::calculateColoumbCross(double **osc_struct, int osc_size, double **dopant_struct,
                                          int dopant_size, double *result)
{
  double cpair_old = 0.0;
  double cpair_new = 0.0;
  result[0] = 0.0;
  ...
  for (int i = 0; i < atom->nlocal; i++) {
    if (atom->tag[i] == iglobal) {
      ...
      for (int j = 0; j < atom->nspecial[i][0]; j++) {    // check if 1-2
        if (atom->special[i][j] == jglobal) factor[comm->me] = force->special_coul[1] - 1;
      }
      for (int j = atom->nspecial[i][0]; j < atom->nspecial[i][1]; j++) {    // check if 1-3
        if (atom->special[i][j] == jglobal) factor[comm->me] = force->special_coul[2] - 1;
      }
      for (int j = atom->nspecial[i][1]; j < atom->nspecial[i][2]; j++) {    // check if 1-4
        if (atom->special[i][j] == jglobal) factor[comm->me] = force->special_coul[3] - 1;
      }
      if (factor[comm->me] != force->special_coul[1] - 1 &&
                                          ...
                                          int molsize)
{
  ...
  for (int i = 0; i < molsize; i++) {
    ...
    for (int j = 0; j < 6; j++) local_struct[i][j] = 0.0;
  }
  ...
  for (int i = 0; i < molecule->local_atoms; i++) {
    index = base + i;
    for (int j = 0; j < 3; j++) local_struct[index][j] = molecule->pos[i][j];
  ...
  for (int i = 0; i < molsize; i++) delete[] local_struct[i];
  delete[] local_struct;
...
int FixRMCPartial::determine_dopant_or_semiconductor(int mol_id)
{
  ...
  for (int procs = 0; procs < comm->nprocs; procs++) {
    if (global_indicator[procs] != -1) final_indicator = global_indicator[procs];
  }
  delete[] indicator;
...
FixRMCPartial::Mol FixRMCPartial::get_molecule(int mol_id, int num_atoms_max)
{
  ...
  for (int i = 0; i < comm->nprocs; i++) {
    ...
    if (gmin_tag[i] != INT_MAX) {
      ...
      if (gmin_tag[i] < gmintagval) gmintagval = gmin_tag[i];
    }
  }
...
void FixRMCPartial::make_move()
{
  ...
  for (int i = 0; i < semiconductor_size; i++) {
    for (int j = 0; j < 6; j++) osc_mol_c[i][j] = 0.0;
  }
  for (int i = 0; i < dopant_size; i++) {
    for (int j = 0; j < 6; j++) dopant_mol_c[i][j] = 0.0;
  }
  ...
  if (comm->me == 0) {
    ...
    while (indicator != 0) {
      ...
      if (molecule_type[rand_semi - 1] == 0 &&
          fabs(molecule_charge_states[rand_semi - 1] - charges[rand_charge]) < 0.001) {
        indicator = 0;
      }
      tries = tries + 1;
      if (tries > 10000) {
        error->one(FLERR, Error::NOLASTLINE,
                   "Too many attemps at finding a semiconductor, something is going wrong!");
      }
    }
  }
  ...
  if (comm->me == 0) {
    while (indicator != 1) {
      ...
      if (molecule_type[rand_dope - 1] == 1) {
        if (fabs(molecule_charge_states[rand_dope - 1] + molecule_charge_states[rand_semi - 1]) <
            0.001) {
          utils::logmesg(lmp,
                         "FOUND EQUALITY\n"
      ...
      if (tries > 10000) {
        error->one(FLERR, Error::NOLASTLINE,
                   "Max tries reached to find dopant of same charge state");
      }
    }
  }
  ...
  if (do_dihedral == 1) edihedral = change_dihedral_parameters(rand_semi, destination_charge_state);
  if (do_angle == 1) eangle = change_angle_parameters(rand_semi, destination_charge_state);
  ...
  calculateColoumbCross(osc_mol_c, semiconductor_size, dopant_mol_c, dopant_size,
                        interaction_energy);

  if (comm->me == 0) {
    std::string mesg;
    if (do_dihedral == 1) mesg += fmt::format("The dihedral energy change is {}\n", edihedral);
    if (do_angle == 1) mesg += fmt::format("The angle energy change is {}\n", eangle);
    if (do_bond == 1) mesg += fmt::format("The bond energy change is {}\n", ebond);
  ...
  if (do_dihedral == 1) energy_diff = energy_diff - edihedral;
  if (do_angle == 1) energy_diff = energy_diff - eangle;
  if (do_bond == 1) energy_diff = energy_diff - ebond;
  ...
  if (comm->me == 0) rand_number = acc_dist(rng_acc);
  ...
  if (comm->me == 0) {
    ...
    for (int i = 0; i < num_charge_states; i++) mesg += fmt::format("{} ", dde[i]);
    mesg += "\n";
    utils::logmesg(lmp, mesg);
  }
}
...
void FixRMCPartial::post_mortem()
{
  ...
  if (comm->me == 0) {
    ...
    fprintf(fcom, "%d\n", n_molecules);
    for (int j = 0; j < 3; j++) fprintf(fcom, "%f %f\n", domain->boxlo[j], domain->boxhi[j]);
...
FixRMCPartial::~FixRMCPartial()
{
  ...
  for (int i = 0; i < semiconductor_size; i++) delete[] osc_mol_c[i];
  for (int i = 0; i < dopant_size; i++) delete[] dopant_mol_c[i];
  ...
  if (do_dihedral == 1) {
    ...
    for (int i = 0; i < num_dihedrals; i++) delete[] dihedral_list[i];
    delete[] dihedral_list;
  }
  if (do_angle == 1) {
    delete[] angle_types;
    for (int i = 0; i < num_angles; i++) delete[] angle_list[i];
    delete[] angle_list;
  }
  if (do_bond == 1) {
    delete[] bond_types;
    for (int i = 0; i < num_bonds; i++) delete[] bond_list[i];
    delete[] bond_list;
  }
