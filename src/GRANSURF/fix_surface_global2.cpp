...
void FixSurfaceGlobal2::hooke(int i, int j, double radi, double meff,
...
void FixSurfaceGlobal2::hooke_history(int i, int j, double radi, double meff,
...
int FixSurfaceGlobal2::overlap_sphere_line(int i, int j, double *pt, 
                                          double *r, double &rsq)
{
  double a[3],b[3];
...
int FixSurfaceGlobal2::endpt_neigh_check(int i, int j, int jflag)
{
  int k;
  double rsq;
  double dr[3],contact[3];
...
int FixSurfaceGlobal2::overlap_sphere_tri(int i, int j, double *pt, 
                                         double *r, double &rsq)
{
...
int FixSurfaceGlobal2::nearest_point_line(double *x, double *p1, double *p2, 
                                         double *pt)
{
  double a[3],b[3];
...
int FixSurfaceGlobal2::edge_neigh_check(int i, int j, int jflag)
{
  int k;
  double rsq;
  double dr[3],contact[3];
...
int FixSurfaceGlobal2::corner_neigh_check(int i, int j, int jflag)
{
  int k,n,kflag;
  tagint idconnect;
  double rsq;
...
void FixSurfaceGlobal2::extract_from_molecules(char *molID)
{
  ...
  for (int m = 0; m < nmol; m++) {
    ...
    if (dimension == 2) {
      ...
      // NOTE: this is bogus
      double **epts = (double **) onemols[m]->lines;
      iline = nlines - nl;
      ipoint = npoints - np;
      for (int i = 0; i < nl; i++) {
        lines[iline].mol = molline[i];
        lines[iline].type = typeline[i];
        lines[iline].p1 = ipoint;
        lines[iline].p2 = ipoint + 1;
        points[ipoint].x[0] = epts[i][0];
        points[ipoint].x[1] = epts[i][1];
        points[ipoint].x[2] = 0.0;
        points[ipoint+1].x[0] = epts[i][2];
        points[ipoint+1].x[1] = epts[i][3];
        points[ipoint+1].x[2] = 0.0;
        iline++;
        ipoint += 2;
      }
    }
    ...
    if (dimension == 3) {
      ...
      // NOTE: this is bogus
      double **cpts = (double **) onemols[m]->tris;
      itri = ntris - nt;
      ipoint = npoints - np;
      for (int i = 0; i < nt; i++) {
        ...
        tris[itri].p1 = ipoint;
        tris[itri].p2 = ipoint + 1;
        tris[itri].p3 = ipoint + 2;
        points[ipoint].x[0] = cpts[i][0];
        points[ipoint].x[1] = cpts[i][1];
        points[ipoint].x[2] = cpts[i][2];
        points[ipoint+1].x[0] = cpts[i][3];
        points[ipoint+1].x[1] = cpts[i][4];
        points[ipoint+1].x[2] = cpts[i][5];
        points[ipoint+2].x[0] = cpts[i][6];
        points[ipoint+2].x[1] = cpts[i][7];
        points[ipoint+2].x[2] = cpts[i][8];
        itri++;
        ipoint += 3;
      }
    }
...
void FixSurfaceGlobal2::connectivity2d_global()
{
  int j,p1,p2;
...
void FixSurfaceGlobal2::connectivity3d_global()
{
...
void FixSurfaceGlobal2::set_attributes()
{
...
void FixSurfaceGlobal2::move_init()
{
...
void FixSurfaceGlobal2::move_clear()
{
