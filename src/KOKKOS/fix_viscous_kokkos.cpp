...
void FixViscousKokkos<DeviceType>::init()
{
  ...
  for (int i = 1; i <= atom->ntypes; i++) k_gamma.view_host()(i) = static_cast<KK_FLOAT>(gamma[i]);
...
void FixViscousKokkos<DeviceType>::operator()(TagFixViscous, const int &i) const {
  if (mask[i] & groupbit) {
    ...
    f(i,0) -= static_cast<KK_ACC_FLOAT>(drag*v(i,0));
    f(i,1) -= static_cast<KK_ACC_FLOAT>(drag*v(i,1));
    f(i,2) -= static_cast<KK_ACC_FLOAT>(drag*v(i,2));
  }
}
