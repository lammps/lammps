...
FixWallACL::FixWallACL(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  nwall(0)
{
  int dir;
  if (narg < 4) error->all(FLERR,"Illegal fix wall/ACL command");
  ...
  while (iarg < narg) {
    if ((strcmp(arg[iarg],"xlo") == 0) || (strcmp(arg[iarg],"xhi") == 0) ||
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix wall/ACL command");
      ...
      for (int m = 0; (m < nwall) && (m < 6); m++)
        if (newwall == wallwhich[m])
          error->all(FLERR,"Wall defined twice in fix wall/ACL command");
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal wall/ACL command");
      if (strcmp(arg[iarg+1],"box") == 0) scaleflag = 0;
      else if (strcmp(arg[iarg+1],"lattice") == 0) scaleflag = 1;
      else error->all(FLERR,"Illegal fix wall/ACL command");
      iarg += 2;
    } else error->all(FLERR,"Illegal fix wall/ACL command");
  }
  ...
  for (int m = 0; m < nwall; m++)
    if ((wallwhich[m] == ZLO || wallwhich[m] == ZHI) && domain->dimension == 2)
      error->all(FLERR,
                 "Cannot use fix wall/ACL zlo/zhi for a 2d simulation");
...
void FixWallACL::init()
{
  for (int m = 0; m < nwall; m++) {
    if (wallstyle[m] != VARIABLE) continue;
    varindex[m] = input->variable->find(varstr[m]);
    if (varindex[m] < 0)
      error->all(FLERR,"Variable name for fix wall/ACL does not exist");
    if (!input->variable->equalstyle(varindex[m]))
      error->all(FLERR,"Variable for fix wall/ACL is invalid style");
  }
...
void FixWallACL::post_integrate()
{
  int i,dim,side,dir;
