...
// Modify parameters from user
  ...
  fft1 = std::unique_ptr< heffte::fft3d<backend_tag> > ( new heffte::fft3d< backend_tag > (
  ...
  fft2 = std::unique_ptr< heffte::fft3d<backend_tag> > ( new heffte::fft3d< backend_tag > (
                          heffte::box3d({nxlo_fft, nylo_fft, nzlo_fft}, {nxhi_fft, nyhi_fft, nzhi_fft}),
...
/* ----------------------------------------------------------------------
   FFT-based Poisson solver for ik - Using heFFTe for FFT computation
------------------------------------------------------------------------- */
...
void PPPM::poisson_ik()
{
  ...
  // transform charge density (r -> k)
   ...
   // Input-output vectors management to be moved to the allocation function, here just for simplicity and clarity.
  std::vector< std::complex<FFT_SCALAR> > input;

  for (i = 0; i < nfft; i++) {
    input.push_back({density_fft[i], ZEROF});
  }

  std::vector< std::complex<FFT_SCALAR> > output(nfft);
  std::copy(input.begin(), input.end(), output.begin());
  std::complex<FFT_SCALAR> *output_array = output.data();

  // Real-to-complex transform in heFFTe
  #ifdef Heffte_ENABLE_GPU
    heffte::gpu::vector<std::complex<FFT_SCALAR>> gpu_output;
    gpu_output = gpu::transfer::load(output);
    output_array = gpu_output.data();
  #endif

  fft1->forward(output_array, output_array, heffte::scale::none);

  #ifdef Heffte_ENABLE_GPU
  heffte::gpu::synchronize_default_stream();
  work1 = static_cast<double>(gpu::transfer::unload(gpu_output));
  heffte::gpu::synchronize_default_stream();
  #else
  work1 = static_cast<double>(output_array);
  #endif
  ...
  for (k = nzlo_fft; k <= nzhi_fft; k++)
    for (j = nylo_fft; j <= nyhi_fft; j++)
      for (i = nxlo_fft; i <= nxhi_fft; i++) {
        output[cc] = {fkx[i]*work1[n+1], -fkx[i]*work1[n]};
        n += 2;
        cc += 1;
      }
  ...
  fft2->backward(output_array, output_array, heffte::scale::none);
  
  #ifdef Heffte_ENABLE_GPU
  heffte::gpu::synchronize_default_stream();
  work2 = static_cast<double>(gpu::transfer::unload(gpu_output));
  heffte::gpu::synchronize_default_stream();
  #else
  work2 = static_cast<double>(output_array);
  #endif
  ...
  for (k = nzlo_fft; k <= nzhi_fft; k++)
    for (j = nylo_fft; j <= nyhi_fft; j++)
      for (i = nxlo_fft; i <= nxhi_fft; i++) {
        output[cc] = {fky[j]*work1[n+1], -fky[j]*work1[n]};
        n += 2;
        cc += 1;
      }
  ...
  for (k = nzlo_fft; k <= nzhi_fft; k++)
    for (j = nylo_fft; j <= nyhi_fft; j++)
      for (i = nxlo_fft; i <= nxhi_fft; i++) {
        output[cc] = {fkz[k]*work1[n+1], -fkz[k]*work1[n]};
        n +=2;
        cc +=1;
      }
...
int PPPM::timing_3d(int n, double &time3d)
{
  ...
  std::vector< std::complex<FFT_SCALAR> > output(nfft_both);
  ...
  for (int i = 0; i < n; i++) {
    fft1->forward(output_array, output_array, heffte::scale::none);
    fft2->backward(output_array, output_array);
  }
