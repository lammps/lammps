...
PairLJSDKSoft::PairLJSDKSoft(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 0;
  single_enable = 1;
  writedata = 1;
...
void PairLJSDKSoft::compute(int eflag, int vflag)
{
  if (eflag || vflag) {
        ...
        if (ljt == LJ12_4) {
          ...
          if (EFLAG)
            evdwl = r4inv*(lj3[itype][jtype]*r4inv*r4inv
                           - alphalj[itype][jtype]) - offset[itype][jtype];
...
void PairLJSDKSoft::settings(int narg, char **arg)
{
...
void PairLJSDKSoft::coeff(int narg, char **arg)
{
...
double PairLJSDKSoft::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
  ...
  alphalj[i][j] = lj_prefact[ljt] * epsilon[i][j] * pow(sigma[i][j],lj_pow2[ljt]);
...
void PairLJSDKSoft::write_restart(FILE *fp)
{
...
void PairLJSDKSoft::read_restart(FILE *fp)
{
...
void PairLJSDKSoft::write_restart_settings(FILE *fp)
{
...
void PairLJSDKSoft::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairLJSDKSoft::write_data(FILE *)
{
...
void PairLJSDKSoft::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %s %g %g %g %g\n",i,j,lj_type_list[lj_type[i][j]],
              epsilon[i][j],sigma[i][j],lambda[i][j],cut[i][j]);
}

/* ---------------------------------------------------------------------- */

double PairLJSDKSoft::single(int, int, int itype, int jtype, double rsq,
...
void *PairLJSDKSoft::extract(const char *str, int &dim)
{
  dim = 2;
...
double PairLJSDKSoft::memory_usage()
{
  ...
  // cut/cutsq/epsilon/sigma/lambda/offset/lj1/lj2/lj3/rminsq/emin
