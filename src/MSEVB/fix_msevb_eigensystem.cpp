...
void FixMSEVB::compute_excess_states() {
  ...
  for (int batch = 0; batch < nbatches; batch++) {
    ...
    for (int b = 0; b < batch_count; b++) {
      ...
      if (sites[b_sk].n_components > 0) {
        for (int ci = 0; ci < sites[b_sk].n_components; ci++) {
          const int par_i = sites[b_sk].components[ci] + 1;
  ...
  for (int k = 0; k < nsites_parallel; k++) {
    ...
    if (sites[k].n_components > 0) {
      for (int ci = 0; ci < sites[k].n_components; ci++) {
        ...
        fill_hamiltonian_edge(par_i, state_k, sites[comp_j]);
      }
    } else {
  ...
  for (int k = 0; k < nsites_parallel; k++) {
    ...
    if (sites[k].n_components > 0) {
      for (int ci = 0; ci < sites[k].n_components; ci++) {
        int par_i = sites[k].components[ci] + 1;
        int comp_j = sites[k].components[1 - ci];
        bcast_edge(par_i, state_k, sites[comp_j]);
      }
    } else {
...
/* ----------------------------------------------------------------------
   ...
   accumulate coupling_virial.  Called from both
   weight_based_hellmann_feynman_forces() and simple_force_mixing().
---------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------
   Simple (Python-like) force mixing for debugging.

   Replaces weight_based_hellmann_feynman_forces() when grimme_simple is set.
   Instead of decomposing Grimme off-diagonal coupling into scalar weight
   adjustments, this explicitly gathers per-state forces from all partitions
   and computes the full Hellmann-Feynman contraction:

     f_mixed[a] = sum_{i,j} rho[i][j] * dH[i][j][a]

   where:
     dH[k][k][a] = f_k[a]                           (diagonal = per-state forces)
     dH[p][c][a] = -g * (f_p[a] - f_c[a])           (Grimme off-diagonal)
     dH[p][c][a] = sparse fH/fX/fY                  (position-based off-diagonal)

   Maps directly to the Python reference:
     coupling.py: cpl_forces = -(-2*b*dE*V * (iF - nF))
     msevb.py:    mixed_forces = einsum("ijkl,i,j->kl", dmatrix, evec, evec)

   This is O(nstates * natoms) per Grimme edge vs O(1) for the weight
   decomposition, but is useful for verifying correctness.
---------------------------------------------------------------------- */

void FixMSEVB::simple_force_mixing() {
  sync_forces_to_host();
  const int ns = nstates;
  const int nlocal = atom->nlocal;
  double **f = atom->f;
  ...
  // Build density matrix rho from eigenvectors
  // Python: mixed_forces = einsum("ijkl,i,j->kl", dmatrix, evec, evec)
  //         which is sum_{i,j} evec[i]*evec[j] * dH[i][j][a]
  //         = sum_{i,j} rho[i][j] * dH[i][j][a]
  std::vector<double> rho(ns * ns, 0.0);
  if (fermi_dirac_enabled && ns > 1) {
    for (int k = 0; k < ns; k++) {
      if (fd_occ[k] < 1.0e-15) continue;
  ...
  // 1. Gather per-state forces from all partitions via samerank
  //    After Allgather: all_f[k * nlocal * 3 + a * 3 + d] = forces for
  //    state k on atom a, component d
  std::vector<double> my_f(nlocal * 3, 0.0);
  if (ipartition < nstates) {
    for (int a = 0; a < nlocal; a++)
      for (int d = 0; d < 3; d++)
        my_f[a * 3 + d] = f[a][d];
  }

  std::vector<double> all_f(npartitions * nlocal * 3, 0.0);
  MPI_Allgather(my_f.data(), nlocal * 3, MPI_DOUBLE,
                all_f.data(), nlocal * 3, MPI_DOUBLE, samerank);

  // Helper: get per-state force for a given state, atom, component
  // Parallel states: from all_f (gathered from partitions)
  // Excess states: from excess_forces buffer (filled by compute_excess_states)
  auto get_force = [&](int state, int a, int d) -> double {
    if (state < npartitions) {
      return all_f[state * nlocal * 3 + a * 3 + d];
    } else {
      // Excess state: index b = state - npartitions
      int b = state - npartitions;
      if (b >= 0 && b < nsites_serial && excess_forces) {
        return excess_forces[b * excess_forces_nmax * 3 + a * 3 + d];
      }
      return 0.0;
    }
  };

  // 2. Zero output forces
  for (int a = 0; a < nlocal; a++)
    f[a][0] = f[a][1] = f[a][2] = 0.0;

  // 3. Diagonal contribution: f[a] += rho[k][k] * f_k[a]
  //    Python: dmatrix[k,k] = forces_k
  for (int k = 0; k < ns; k++) {
    double w = rho[k * ns + k];
    if (std::fabs(w) < 1.0e-30) continue;
    for (int a = 0; a < nlocal; a++)
      for (int d = 0; d < 3; d++)
        f[a][d] += w * get_force(k, a, d);
  }

  // 4. Grimme off-diagonal contribution (explicit, all atoms)
  //    dH[p][c][a] = -g * (f_p[a] - f_c[a])
  //
  //    Python equivalent (coupling.py:297-307):
  //      dE = energies["new"] - energies["initial"]   # E_child - E_parent
  //      cpl = a * exp(-b * dE**2)
  //      dF = iF - nF                                 # f_parent - f_child
  //      dCddE = -2 * b * dE * cpl * dF
  //      cpl_forces = -dCddE                          # = -g * dF
  //
  //    Contraction: 2 * rho[p][c] * dH[p][c][a]
  //      = 2 * rho_pc * (-g) * (f_p[a] - f_c[a])
  for (int k = 0; k < nsites_parallel + nsites_serial; k++) {
    int state_k = k + 1;

    auto process_grimme = [&](int parent, int child,
                              const ReactiveSite &site) {
      const ReactionDef &rxn = rxndefs[site.rxn_idx];
      if (rxn.coupling_type != COUPLING_GRIMME2015) return;

      auto cr = evaluate_coupling(rxn, epot[parent], epot[child],
                                  site.tag_H, site.tag_X, site.tag_Y);

      double rho_pc = rho[parent * ns + child];
      double coeff = 2.0 * rho_pc * (-cr.g);

      for (int a = 0; a < nlocal; a++)
        for (int d = 0; d < 3; d++)
          f[a][d] += coeff * (get_force(parent, a, d) -
                              get_force(child, a, d));
    };
    ...
    if (sites[k].n_components > 0) {
      for (int ci = 0; ci < sites[k].n_components; ci++) {
        int par_i = sites[k].components[ci] + 1;
        int comp_j = sites[k].components[1 - ci];
        process_grimme(par_i, state_k, sites[comp_j]);
      }
    } else {
      process_grimme(sites[k].parent_state, state_k, sites[k]);
    }
  }

  // 5. Apply sparse position-based coupling corrections and virial
  apply_coupling_corrections();
  modified_forces_on_host();
}
