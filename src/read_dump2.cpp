...
ReadDump2::ReadDump2(LAMMPS *lmp) : Pointers(lmp)
{
...
void ReadDump2::command(int narg, char **arg)
{
...
void ReadDump2::store_files(int nstr, char **str)
{
  nfile = nstr;
...
void ReadDump2::setup_reader(int narg, char **arg)
{
...
bigint ReadDump2::seek(bigint nrequest, int exact)
{
  int ifile,eofflag;
  bigint ntimestep;
...
bigint ReadDump2::next(bigint ncurrent, bigint nlast, int nevery, int nskip)
{
  int ifile,eofflag;
  bigint ntimestep;
  ...
  if (me == 0) {
    ...
    for (ifile = currentfile; ifile < nfile; ifile++) {
      ...
      while (1) {
        ...
        // new code logic to match new parallel read_dump
        if (eofflag) break;
...
void ReadDump2::header(int fieldinfo)
{
  int triclinic_snap;
...
int ReadDump2::fields_and_keywords(int narg, char **arg)
{
...
int ReadDump2::whichtype(char *str)
{
  int type = -1;
...
void ReadDump2::process_atoms(int n)
{
...
double ReadDump2::xfield(int i, int j)
{
...
double ReadDump2::yfield(int i, int j)
{
  if (!scaled) return fields[i][j];
...
double ReadDump2::zfield(int i, int j)
{
  if (!scaled) return fields[i][j];
