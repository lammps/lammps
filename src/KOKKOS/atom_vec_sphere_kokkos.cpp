...
struct AtomVecSphereKokkos_PackComm {
  ...
  AtomVecSphereKokkos_PackComm(
    ...
    _rmass(DualViewHelper<Space>::view(rmass)),
  ...
  const int* const pbc)
{
  ...
  if(commKK->forward_comm_on_host) {
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackComm<Host,1,1> f(
          atomKK->k_x,
        ...
        struct AtomVecSphereKokkos_PackComm<Host,1,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
      ...
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackComm<Host,0,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackComm<Host,0,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackComm<Device,1,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackComm<Device,1,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
      ...
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackComm<Device,0,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackComm<Device,0,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          buf,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
...
template<ExecutionSpace Space,int RADVARY,int PBC_FLAG,int TRICLINIC,int DEFORM_VREMAP>
struct AtomVecSphereKokkos_PackCommVel {
  ...
  AtomVecSphereKokkos_PackCommVel(
    ...
    _omega(DualViewHelper<Space>::view(omega)),
    _list(DualViewHelper<Space>::view(list)),_iswap(iswap),
    _xprd(xprd),_yprd(yprd),_zprd(zprd),
    _xy(xy),_xz(xz),_yz(yz),
  ...
  if(commKK->forward_comm_on_host) {
    ...
    if(pbc_flag) {
      if(deform_vremap) {
        if(domain->triclinic) {
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Host,0,1,1,1> f(
            ...
            struct AtomVecSphereKokkos_PackCommVel<Host,1,1,1,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
          ...
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Host,0,1,0,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Host,1,1,0,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
        ...
        if(domain->triclinic) {
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Host,0,1,1,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Host,1,1,1,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
          ...
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Host,0,1,0,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Host,1,1,0,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
      ...
      if(domain->triclinic) {
        if (radvary == 0) {
          struct AtomVecSphereKokkos_PackCommVel<Host,0,0,1,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
          ...
          struct AtomVecSphereKokkos_PackCommVel<Host,1,0,1,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
        ...
        if (radvary == 0) {
          struct AtomVecSphereKokkos_PackCommVel<Host,0,0,0,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
          ...
          struct AtomVecSphereKokkos_PackCommVel<Host,1,0,0,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
    ...
    if(pbc_flag) {
      if(deform_vremap) {
        if(domain->triclinic) {
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Device,0,1,1,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Device,1,1,1,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
          ...
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Device,0,1,0,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Device,1,1,0,1> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
        ...
        if(domain->triclinic) {
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Device,0,1,1,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Device,1,1,1,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
          ...
          if (radvary == 0) {
            struct AtomVecSphereKokkos_PackCommVel<Device,0,1,0,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
            ...
            struct AtomVecSphereKokkos_PackCommVel<Device,1,1,0,0> f(
              atomKK->k_x,atomKK->k_mask,
              atomKK->k_radius,atomKK->k_rmass,
              atomKK->k_v,atomKK->k_omega,
              buf,list,iswap,
      ...
      if(domain->triclinic) {
        if (radvary == 0) {
          struct AtomVecSphereKokkos_PackCommVel<Device,0,0,1,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
          ...
          struct AtomVecSphereKokkos_PackCommVel<Device,1,0,1,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
        ...
        if (radvary == 0) {
          struct AtomVecSphereKokkos_PackCommVel<Device,0,0,0,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
          ...
          struct AtomVecSphereKokkos_PackCommVel<Device,1,0,0,0> f(
            atomKK->k_x,atomKK->k_mask,
            atomKK->k_radius,atomKK->k_rmass,
            atomKK->k_v,atomKK->k_omega,
            buf,list,iswap,
...
struct AtomVecSphereKokkos_PackCommSelf {
  ...
  AtomVecSphereKokkos_PackCommSelf(
    ...
    _radius(DualViewHelper<Space>::view(radius)),
    _rmass(DualViewHelper<Space>::view(rmass)),
    _nfirst(nfirst),_list(DualViewHelper<Space>::view(list)),_iswap(iswap),
    _xprd(xprd),_yprd(yprd),_zprd(zprd),
    _xy(xy),_xz(xz),_yz(yz) {
    _pbc[0] = pbc[0]; _pbc[1] = pbc[1]; _pbc[2] = pbc[2];
    _pbc[3] = pbc[3]; _pbc[4] = pbc[4]; _pbc[5] = pbc[5];
...
int AtomVecSphereKokkos::pack_comm_self(
  ...
  if(commKK->forward_comm_on_host) {
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackCommSelf<Host,1,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackCommSelf<Host,1,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
      ...
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackCommSelf<Host,0,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackCommSelf<Host,0,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackCommSelf<Device,1,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackCommSelf<Device,1,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
      ...
      if(domain->triclinic) {
        struct AtomVecSphereKokkos_PackCommSelf<Device,0,1> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
        ...
        struct AtomVecSphereKokkos_PackCommSelf<Device,0,0> f(
          atomKK->k_x,
          atomKK->k_radius,atomKK->k_rmass,
          nfirst,list,iswap,
          domain->xprd,domain->yprd,domain->zprd,
...
void AtomVecSphereKokkos::unpack_comm_kokkos(
  ...
  if(commKK->forward_comm_on_host) {
    atomKK->modified(Host,X_MASK|RADIUS_MASK|RMASS_MASK);
    struct AtomVecSphereKokkos_UnpackComm<Host> f(
      atomKK->k_x,
      atomKK->k_radius,atomKK->k_rmass,
      buf,first);
    Kokkos::parallel_for(n,f);
    ...
    atomKK->modified(Device,X_MASK|RADIUS_MASK|RMASS_MASK);
    struct AtomVecSphereKokkos_UnpackComm<Device> f(
      atomKK->k_x,
      atomKK->k_radius,atomKK->k_rmass,
      buf,first);
    Kokkos::parallel_for(n,f);
...
template<ExecutionSpace Space,int RADVARY>
...
void AtomVecSphereKokkos::unpack_comm_vel_kokkos(
  ...
  if(commKK->forward_comm_on_host) {
    ...
    if (radvary == 0) {
      struct AtomVecSphereKokkos_UnpackCommVel<Host,0> f(
        atomKK->k_x,
        atomKK->k_radius,atomKK->k_rmass,
        atomKK->k_v,atomKK->k_omega,
        buf,first);
      ...
      struct AtomVecSphereKokkos_UnpackCommVel<Host,1> f(
        atomKK->k_x,
        atomKK->k_radius,atomKK->k_rmass,
        atomKK->k_v,atomKK->k_omega,
        buf,first);
    ...
    if (radvary == 0) {
      struct AtomVecSphereKokkos_UnpackCommVel<Device,0> f(
        atomKK->k_x,
        atomKK->k_radius,atomKK->k_rmass,
        atomKK->k_v,atomKK->k_omega,
        buf,first);
      ...
      struct AtomVecSphereKokkos_UnpackCommVel<Device,1> f(
        atomKK->k_x,
        atomKK->k_radius,atomKK->k_rmass,
        atomKK->k_v,atomKK->k_omega,
        buf,first);
  ...
  int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    if(space==Host) {
      AtomVecSphereKokkos_PackBorder<Host,1> f(
      ...
      AtomVecSphereKokkos_PackBorder<Device,1> f(
    ...
    dx = dy = dz = 0;
    if(space==Host) {
      AtomVecSphereKokkos_PackBorder<Host,0> f(
        buf.h_view, k_sendlist.h_view,
        iswap,h_x,h_tag,h_type,h_mask,
        h_radius,h_rmass,
        dx,dy,dz);
      ...
      AtomVecSphereKokkos_PackBorder<Device,0> f(
        buf.d_view, k_sendlist.d_view,
        iswap,d_x,d_tag,d_type,d_mask,
        d_radius,d_rmass,
        dx,dy,dz);
...
template<ExecutionSpace Space,int PBC_FLAG,int DEFORM_VREMAP>
struct AtomVecSphereKokkos_PackBorderVel {
  ...
  AtomVecSphereKokkos_PackBorderVel(
    ...
    const typename AT::t_float_1d &radius,
    const typename AT::t_float_1d &rmass,
    const typename AT::t_float_1d_3 &v,
    const typename AT::t_float_1d_3 &omega,
    const KK_FLOAT &dx, const KK_FLOAT &dy, const KK_FLOAT &dz,
  ...
  int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    if (!deform_vremap) {
      if(space==Host) {
        AtomVecSphereKokkos_PackBorderVel<Host,1,0> f(
          buf.h_view, k_sendlist.h_view,
          iswap,h_x,h_tag,h_type,h_mask,
          h_radius,h_rmass,
          h_v, h_omega,
        ...
        AtomVecSphereKokkos_PackBorderVel<Device,1,0> f(
          buf.d_view, k_sendlist.d_view,
          iswap,d_x,d_tag,d_type,d_mask,
          d_radius,d_rmass,
          d_v, d_omega,
      ...
      dvz = pbc[2]*h_rate[2];
      if(space==Host) {
        AtomVecSphereKokkos_PackBorderVel<Host,1,1> f(
          buf.h_view, k_sendlist.h_view,
          iswap,h_x,h_tag,h_type,h_mask,
          h_radius,h_rmass,
          h_v, h_omega,
        ...
        AtomVecSphereKokkos_PackBorderVel<Device,1,1> f(
          buf.d_view, k_sendlist.d_view,
          iswap,d_x,d_tag,d_type,d_mask,
          d_radius,d_rmass,
          d_v, d_omega,
    ...
    if(space==Host) {
      AtomVecSphereKokkos_PackBorderVel<Host,0,0> f(
        buf.h_view, k_sendlist.h_view,
        iswap,h_x,h_tag,h_type,h_mask,
        h_radius,h_rmass,
        h_v, h_omega,
      ...
      AtomVecSphereKokkos_PackBorderVel<Device,0,0> f(
        buf.d_view, k_sendlist.d_view,
        iswap,d_x,d_tag,d_type,d_mask,
        d_radius,d_rmass,
        d_v, d_omega,
...
void AtomVecSphereKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  if(space==Host) {
    ...
    struct AtomVecSphereKokkos_UnpackBorder<Device> f(buf.d_view,
...
void AtomVecSphereKokkos::unpack_border_vel_kokkos(
  ...
  if(space==Host) {
    ...
    struct AtomVecSphereKokkos_UnpackBorderVel<Device> f(buf.d_view,
      d_x,d_tag,d_type,d_mask,
      d_radius,d_rmass,
      d_v, d_omega,
      first);
...
struct AtomVecSphereKokkos_PackExchangeFunctor {
  ...
  typename AT::t_float_1d_3_randomread _omega;
  typename AT::t_float_1d_3 _xw;
  ...
  AtomVecSphereKokkos_PackExchangeFunctor(
    ...
    _radius(DualViewHelper<Space>::view(atom->k_radius)),
    _rmass(DualViewHelper<Space>::view(atom->k_rmass)),
    _omega(DualViewHelper<Space>::view(atom->k_omega)),
    ...
    _radiusw(DualViewHelper<Space>::view(atom->k_radius)),
    _rmassw(DualViewHelper<Space>::view(atom->k_rmass)),
    _omegaw(DualViewHelper<Space>::view(atom->k_omega)),
...
{
  if(nsend > (int) (k_buf.h_view.extent(0)*k_buf.h_view.extent(1))/16) {
  ...
  if(space == Host) {
    AtomVecSphereKokkos_PackExchangeFunctor<Host> f(atomKK,k_buf,k_sendlist,k_copylist,atom->nlocal,dim,lo,hi);
    Kokkos::parallel_for(nsend,f);
  } else {
    AtomVecSphereKokkos_PackExchangeFunctor<Device> f(atomKK,k_buf,k_sendlist,k_copylist,atom->nlocal,dim,lo,hi);
    Kokkos::parallel_for(nsend,f);
  }
  return nsend*16;
}
...
int AtomVecSphereKokkos::unpack_exchange_kokkos(DAT::tdual_double_2d_lr &k_buf,int nrecv,int nlocal,int dim,KK_FLOAT lo,KK_FLOAT hi,ExecutionSpace space) {
  if(space == Host) {
    ...
    k_count.modify_host();
    k_count.sync_device();
    AtomVecSphereKokkos_UnpackExchangeFunctor<Device> f(atomKK,k_buf,k_count,dim,lo,hi);
    Kokkos::parallel_for(nrecv/16,f);
...
void AtomVecSphereKokkos::sync_overlapping_device(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
    ...
    if ((mask & OMEGA_MASK) && atomKK->k_omega.need_sync_device())
      perform_async_copy<DAT::tdual_float_1d_3>(atomKK->k_omega,space);
    if ((mask & TORQUE_MASK) && atomKK->k_torque.need_sync_device())
      perform_async_copy<DAT::tdual_float_1d_3>(atomKK->k_torque,space);
  } else {
