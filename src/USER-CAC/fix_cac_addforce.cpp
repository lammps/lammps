...
FixCAC_Add_Force::FixCAC_Add_Force(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  xstr(NULL), ystr(NULL), zstr(NULL), idregion(NULL), sforce(NULL)
{
  if (narg < 6) error->all(FLERR,"Illegal fix cac/addforce command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix cac/addforce command");
      ...
      iarg += 2;
    } else error->all(FLERR,"Illegal fix cac/addforce command");
  }
...
FixCAC_Add_Force::~FixCAC_Add_Force()
{
  if (copymode) return;
...
void FixCAC_Add_Force::setup(int vflag)
{
  if (strstr(update->integrate_style,"verlet"))
    post_force(vflag);
  else
    error->all(FLERR, "Cannot use respa with cac/addforce");
}

/* ---------------------------------------------------------------------- */

void FixCAC_Add_Force::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixCAC_Add_Force::post_force(int vflag)
{
  double **x = atom->x;
  double **v = atom->v;
  double **f = atom->f;
...
void FixCAC_Add_Force::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixCAC_Add_Force::compute_vector(int n)
{
...
double FixCAC_Add_Force::memory_usage()
{
  double bytes = 0.0;
