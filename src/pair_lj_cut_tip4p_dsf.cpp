...
#include "math.h"
#include "stdlib.h"
#include "pair_lj_cut_tip4p_dsf.h"
#include "atom.h"
#include "force.h"
#include "neighbor.h"
...
PairLJCutTIP4PDSF::PairLJCutTIP4PDSF(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
...
PairLJCutTIP4PDSF::~PairLJCutTIP4PDSF()
{
  if (allocated) {
...
void PairLJCutTIP4PDSF::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fpair;
  double r,rsq,r2inv,r6inv,forcecoul,forcelj,factor_lj,factor_coul;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_coulsqplus) {
        ...
        if (rsq < cut_coulsq) {
          ...
          erfcd = exp(-alpha_dsf*alpha_dsf*r*r);
          t = 1.0 / (1.0 + EWALD_P*alpha_dsf*r);
          erfcc = t * (A1+t*(A2+t*(A3+t*(A4+t*A5)))) * erfcd;
          forcecoul = prefactor * (erfcc/r + 2.0*alpha_dsf/MY_PIS * erfcd + 
            r*f_shift) * r;
...
void PairLJCutTIP4PDSF::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutTIP4PDSF::settings(int narg, char **arg)
{
  if (narg < 7 || narg > 8) error->all(FLERR,"Illegal pair_style command");
  ...
  alpha_dsf = force->numeric(arg[6]);
  if (narg == 7) cut_coul = cut_lj_global;
...
void PairLJCutTIP4PDSF::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 5)
...
void PairLJCutTIP4PDSF::init_style()
{
  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style lj/cut/tip4p/dsf requires atom IDs");
  if (!force->newton_pair)
    error->all(FLERR,
               "Pair style lj/cut/tip4p/dsf requires newton pair on");
  if (!atom->q_flag)
    error->all(FLERR,
               "Pair style lj/cut/tip4p/dsf requires atom attribute q");
  if (force->bond == NULL)
  ...
  cut_coulsq = cut_coul * cut_coul;
  double erfcc = erfc(alpha_dsf*cut_coul); 
  double erfcd = exp(-alpha_dsf*alpha_dsf*cut_coulsq);
  f_shift = -(erfcc/cut_coulsq + 2.0/MY_PIS*alpha_dsf*erfcd/cut_coul); 
...
double PairLJCutTIP4PDSF::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutTIP4PDSF::write_restart(FILE *fp)
{
...
void PairLJCutTIP4PDSF::read_restart(FILE *fp)
{
...
void PairLJCutTIP4PDSF::write_restart_settings(FILE *fp)
{
...
void PairLJCutTIP4PDSF::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutTIP4PDSF::compute_newsite(double *xO,  double *xH1,
...
double PairLJCutTIP4PDSF::memory_usage()
{
