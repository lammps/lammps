...
{
  if (narg != 3) error->all(FLERR, 2, "Illegal compute smd/ulsph/strain/rate command");
...
void ComputeSMDULSPHStrainRate::init()
{
  ...
  if ((comm->me == 0) && (modify->get_compute_by_style("^smd/ulsph/strain/rate").size() > 1))
...
void ComputeSMDULSPHStrainRate::compute_peratom()
{
  ...
  if (L == nullptr) {
    error->all(FLERR,
               "compute smd/ulsph/strain/rate could not access any velocity gradients. "
               "Are the matching pair styles present?");
  }
