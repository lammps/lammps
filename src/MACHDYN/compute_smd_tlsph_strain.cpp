...
{
  if (narg != 3) error->all(FLERR, "Illegal compute smd/tlsph/strain command");
...
void ComputeSMDTLSPHstrain::init()
{
  ...
  if ((comm->me == 0) && (modify->get_compute_by_style("^smd/tlsph/strain$").size() > 1))
...
void ComputeSMDTLSPHstrain::compute_peratom()
{
  ...
  if (Fincr == nullptr) {
    error->all(FLERR, "compute smd/tlsph/strain failed to access Fincr array");
  }
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      Ftotal = F0 *
          Fincr
              [i];    // this is the total deformation gradient: reference deformation times incremental deformation
