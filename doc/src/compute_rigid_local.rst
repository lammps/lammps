...
Define a calculation that simply stores rigid body attributes for each
rigid body defined by the :doc:`fix rigid/small <fix_rigid>` command
or one of its NVE, NVT, NPT, NPH variants.  The data is stored as
local data so it can be accessed by other :doc:`output commands
<Howto_output>` that process local data, such as the :doc:`compute
reduce <compute_reduce>` or :doc:`dump local <dump>` commands.  See
the :doc:`compute rigid/atom <compute_rigid_atom>` command for how to
output only a copy of each rigid body's information with each atom.
...
information for :math:`N` bodies is generated.  :math:`N` may be less
than the number of bodies defined by the fix rigid command, if atoms
which store body information are not in the *group-ID*\ .
...
the rigid body.  See the :doc:`set <set>` command for an explanation
of the quaternion vector.
