...
#include "pair_lj_cut_coul_cut_soft_kokkos.h"
...
PairLJCutCoulCutSoftKokkos<DeviceType>::PairLJCutCoulCutSoftKokkos(LAMMPS *lmp):PairLJCutCoulCutSoft(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairLJCutCoulCutSoftKokkos<DeviceType>::~PairLJCutCoulCutSoftKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLJCutCoulCutSoftKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairLJCutCoulCutSoftKokkos<DeviceType>,void >
...
KK_FLOAT PairLJCutCoulCutSoftKokkos<DeviceType>::
...
compute_fcoul(const KK_FLOAT& rsq, const int& /*i*/, const int&j,
  ...
  return factor_coul * qqrd2e * lj1 * qtmp * q(j) / (denc*denc*denc);
}
...
compute_ecoul(const KK_FLOAT& rsq, const int& /*i*/, const int&j,
  ...
  return factor_coul * qqrd2e * lj1 * qtmp * q(j) / denc;
}
...
template<class DeviceType>
void PairLJCutCoulCutSoftKokkos<DeviceType>::allocate()
{
  PairLJCutCoulCutSoft::allocate();
  ...
  k_params = Kokkos::DualView<params_lj_coul_soft**,Kokkos::LayoutRight,DeviceType>("PairLJCutCoulCutSoft::params",n+1,n+1);
...
template<class DeviceType>
void PairLJCutCoulCutSoftKokkos<DeviceType>::init_style()
{
  PairLJCutCoulCutSoft::init_style();
...
template<class DeviceType>
double PairLJCutCoulCutSoftKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLJCutCoulCutSoft::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLJCutCoulCutSoftKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLJCutCoulCutSoftKokkos<LMPHostType>;
#endif
}
