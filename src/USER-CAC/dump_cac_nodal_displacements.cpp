...
#include <cstring>
#include "dump_cac_nodal_displacements.h"
#include "atom.h"
#include "group.h"
#include "error.h"
#include "memory.h"
...
DumpCACNodalDisplacements::DumpCACNodalDisplacements(LAMMPS *lmp, int narg, char **arg) : Dump(lmp, narg, arg)
{
...
DumpCACNodalDisplacements::~DumpCACNodalDisplacements()
{
...
void DumpCACNodalDisplacements::init_style()
{
  ...
  if (buffer_flag == 1) write_choice = &DumpCACNodalDisplacements::write_string;
  else write_choice = &DumpCACNodalDisplacements::write_lines;
...
int DumpCACNodalDisplacements::modify_param(int narg, char **arg)
{
...
/*------------------------------------------------------------------------*/
int DumpCACNodalDisplacements::count()
{
...
void DumpCACNodalDisplacements::write_header(bigint n)
{
...
void DumpCACNodalDisplacements::pack(tagint *ids)
{
  int m,n;
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    for (int k = 0; k < poly_count[i]; k++) {
      for (int j = 0; j < nodes_per_element_list[element_type[i]]; j++) {
        ...
        buf[m++] = nodal_positions[i][k][j][0]-initial_nodal_positions[i][k][j][0];
        buf[m++] = nodal_positions[i][k][j][1]-initial_nodal_positions[i][k][j][1];
        buf[m++] = nodal_positions[i][k][j][2]-initial_nodal_positions[i][k][j][2];
      }
      }
    }
  }
...
int DumpCACNodalDisplacements::convert_string(int n, double *mybuf)
{
  int offset = 0;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCACNodalDisplacements::write_data(int n, double *mybuf)
{
...
void DumpCACNodalDisplacements::write_string(int n, double *mybuf)
{
...
void DumpCACNodalDisplacements::write_lines(int n, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,format,
