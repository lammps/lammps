...
#include "pair_lj_cut_coul_long_cs_kokkos.h"
...
PairLJCutCoulLongCSKokkos<DeviceType>::PairLJCutCoulLongCSKokkos(LAMMPS *lmp):PairLJCutCoulLongCS(lmp)
{
  respa_enable = 0;
  tuner = nullptr;
...
template<class DeviceType>
PairLJCutCoulLongCSKokkos<DeviceType>::~PairLJCutCoulLongCSKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLJCutCoulLongCSKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev;
  if (ncoultablebits)
    ev = pair_compute<PairLJCutCoulLongCSKokkos<DeviceType>,CoulLongTable<1> >
      (this,(NeighListKokkos<DeviceType>*)list);
  else
    ev = pair_compute<PairLJCutCoulLongCSKokkos<DeviceType>,CoulLongTable<0> >
      (this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairLJCutCoulLongCSKokkos<DeviceType>::
...
template<class DeviceType>
void PairLJCutCoulLongCSKokkos<DeviceType>::allocate()
{
  PairLJCutCoulLongCS::allocate();
  ...
  k_params = Kokkos::DualView<params_lj_coul**,Kokkos::LayoutRight,DeviceType>("PairLJCutCoulLongCS::params",n+1,n+1);
  params = k_params.template view<DeviceType>();
}

template<class DeviceType>
void PairLJCutCoulLongCSKokkos<DeviceType>::init_tables(double cut_coul, double *cut_respa)
{
...
template<class DeviceType>
void PairLJCutCoulLongCSKokkos<DeviceType>::init_style()
{
  PairLJCutCoulLongCS::init_style();
  ...
  if (lmp->kokkos->autotuning > 0 && !tuner) {
    if (!force->newton_pair)
      ...
      error->warning(FLERR,"Autotuner for pair lj/cut/coul/long/cs/kk is disabled with 'newton on'");
  }
}
...
template<class DeviceType>
double PairLJCutCoulLongCSKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLJCutCoulLongCS::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLJCutCoulLongCSKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLJCutCoulLongCSKokkos<LMPHostType>;
#endif
}
