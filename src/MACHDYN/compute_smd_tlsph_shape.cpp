...
{
  if (narg != 3) error->all(FLERR, "Illegal compute smd/tlsph/shape command");
...
void ComputeSmdTlsphShape::init()
{
  ...
  if ((comm->me == 0) && (modify->get_compute_by_style("^smd/tlsph/shape").size() > 1))
...
void ComputeSmdTlsphShape::compute_peratom()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      for (int j = 0; j < size_peratom_cols; j++) { strainVector[i][j] = 0.0; }
    }
  }
}
