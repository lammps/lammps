...
RegPlaneKokkos<DeviceType>::RegPlaneKokkos(LAMMPS *lmp, int narg, char **arg)
  : RegPlane(lmp, narg, arg)
{
...
template<class DeviceType>
void RegPlaneKokkos<DeviceType>::match_all_kokkos(int groupbit_in, DAT::tdual_int_1d k_match_in)
{
  ...
  copymode = 1;
  Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagRegPlaneMatchAll>(0,nlocal),*this);
  copymode = 0;
...
void RegPlaneKokkos<DeviceType>::operator()(TagRegPlaneMatchAll, const int &i) const {
