...
Each BPM bond stores a reference state (see above) typically set
from the initial configuration and is preserved only in :doc:`binary
restart files <restart>`.  The *write/reference* and *read/reference*
...
restoring the reference state after reading a :doc:`data file <read_data>`
...
The *write/reference* keyword takes the ID of an internal fix and an
interval *N*, e.g. ``write/reference myfix 1000``.  Every *N* timesteps the
...
.. code-block:: LAMMPS
   ...
   bond_style bpm/spring write/reference myfix 1000
...
The *read/reference* keyword takes the name of a file to read, e.g.
``read/reference bond.ref``.  The file is read when the bond style is
...
below lists this order so that a reference file can be constructed
...
models, the history size is that of the largest model in use: three columns
of history if any *ves* bonds are present, otherwise two if any *eps* bonds
are present, otherwise one.
