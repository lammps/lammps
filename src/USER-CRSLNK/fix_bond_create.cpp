...
FixBondCreate::FixBondCreate(LAMMPS *lmp, int narg, char **arg) :
	...
	angledynflag = dihedraldynflag = improperdynflag = 0;

	iatomtype = force->inumeric(FLERR, arg[4]);
	jatomtype = force->inumeric(FLERR, arg[5]);
	double cutoff = force->numeric(FLERR, arg[6]);
	if (isalpha(arg[7][0])) {
		bond_types = input->variable->get_type_detector(arg[7]);
		if (bond_types == NULL)
			error->all(FLERR, "Invalid bond_type id!");
		if (bond_types->type != bond)
			error->all(FLERR,
					"bond_type id does not belong to a bond_type class!");
	} else {
	...
	while (iarg < narg) {
		if (strcmp(arg[iarg], "iparam") == 0) {
			...
			if (isalpha(arg[iarg + 1][0])) {
				angle_types = input->variable->get_type_detector(arg[iarg + 1]);
				if (angle_types == NULL)
					error->all(FLERR, "Invalid angle_types id!");
				if (angle_types->type != angle)
					error->all(FLERR,
							"angle_types id does not belong to a angle_types class!");
			} else {
			...
			if (iarg + 2 > narg)
				error->all(FLERR, "Illegal fix bond/create command");
			if (isalpha(arg[iarg + 1][0])) {
				dihedral_types = input->variable->get_type_detector(
						arg[iarg + 1]);
				if (dihedral_types == NULL)
					error->all(FLERR, "Invalid dihedral_type id!");
				if (dihedral_types->type != dihedral)
					error->all(FLERR,
							"dihedral_type id does not belong to a dihedral_type class!");
			} else {
			...
			if (iarg + 2 > narg)
				error->all(FLERR, "Illegal fix bond/create command");
			if (isalpha(arg[iarg + 1][0])) {
				improper_types = input->variable->get_type_detector(
						arg[iarg + 1]);
				if (improper_types == NULL)
					error->all(FLERR, "Invalid improper_type id!");
				if (improper_types->type != improper)
					error->all(FLERR,
							"improper_type id does not belong to a improper_type class!");
			} else {
...
void FixBondCreate::init() {
	...
	if ((atype || angle_types != NULL) && force->angle) {
		angleflag = 1;
		angledynflag = (angle_types != NULL);
		if (atype > atom->nangletypes)
			error->all(FLERR, "Fix bond/create angle type is invalid");
	} else
		angleflag = 0;

	if ((dtype || dihedral_types != NULL) && force->dihedral) {
		dihedralflag = 1;
		dihedraldynflag = (dihedral_types != NULL);
		if (dtype > atom->ndihedraltypes)
			error->all(FLERR, "Fix bond/create dihedral type is invalid");
	} else
		dihedralflag = 0;

	if ((itype || improper_types != NULL) && force->improper) {
		improperflag = 1;
		improperdynflag = (improper_types != NULL);
	...
	for (i = 0; i < n2; i++) {
		...
		for (j = i + 1; j < n2; j++) {
			...
			if (angledynflag) {
				...
				angle_types->get(ang_type, atype);
			}
			...
			for (k = 0; k < n3; k++) {
				...
				if (n < ncreate) {
					...
					if (dihedraldynflag) {
						...
						dihedral_types->get(dih_type, dtype);		
					}
		...
		for (j = i + 1; j < n1; j++) {
			...
			for (k = j + 1; k < n1; k++) {
				...
				if (improperdynflag) {
					...
					improper_types->get(imp_type, itype);					
				}
