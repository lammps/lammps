...
MinABCFire::MinABCFire(LAMMPS *lmp) : Min(lmp) {}

/* ---------------------------------------------------------------------- */

void MinABCFire::init()
{
  Min::init();
...
void MinABCFire::setup_style()
{
  double **v = atom->v;
  int nlocal = atom->nlocal;

  // print the parameters used within abcfire into the log

  const char *s1[] = {"eulerimplicit","verlet","leapfrog","eulerexplicit"};
  const char *s2[] = {"no","yes"};

  if (comm->me == 0 && logfile) {
      fprintf(logfile,"  Parameters for abcfire: \n"
...
int MinABCFire::iterate(int maxiter)
{
  bigint ntimestep;
      ...
      if (rmass) {
        for (int i = 0; i < nlocal; i++) {
          ...
          if (vdotfall > 0.0) {
            ...
            //making sure that the displacement is not larger than dmax
