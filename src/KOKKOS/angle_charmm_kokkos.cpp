...
AngleCharmmKokkos<Space>::AngleCharmmKokkos(LAMMPS *lmp) : AngleCharmm(lmp)
{
...
template<ExecutionSpace Space>
AngleCharmmKokkos<Space>::~AngleCharmmKokkos()
{
  if (!copymode) {
...
template<ExecutionSpace Space>
void AngleCharmmKokkos<Space>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
...
void AngleCharmmKokkos<Space>::operator()(TagAngleCharmmCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
...
template<int NEWTON_BOND, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void AngleCharmmKokkos<Space>::operator()(TagAngleCharmmCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
...
template<ExecutionSpace Space>
void AngleCharmmKokkos<Space>::allocate()
{
...
template<ExecutionSpace Space>
void AngleCharmmKokkos<Space>::coeff(int narg, char **arg)
{
  AngleCharmm::coeff(narg, arg);

  int n = atom->nangletypes;
  DAT::tdual_float_1d k_k("AngleCharmm::k",n+1);
  DAT::tdual_float_1d k_theta0("AngleCharmm::theta0",n+1);
  DAT::tdual_float_1d k_k_ub("AngleCharmm::k_ub",n+1);
  DAT::tdual_float_1d k_r_ub("AngleCharmm::r_ub",n+1);

  d_k = DualViewHelper<Space>::view(k_k);
  d_theta0 = DualViewHelper<Space>::view(k_theta0);
  d_k_ub = DualViewHelper<Space>::view(k_k_ub);
  d_r_ub = DualViewHelper<Space>::view(k_r_ub);
  ...
  DualViewHelper<Space>::sync(k_k_ub);
  DualViewHelper<Space>::sync(k_r_ub);
}
...
template<ExecutionSpace Space>
void AngleCharmmKokkos<Space>::read_restart(FILE *fp)
{
...
KOKKOS_INLINE_FUNCTION
void AngleCharmmKokkos<Space>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
...
namespace LAMMPS_NS {
template class AngleCharmmKokkos<Device>;
template class AngleCharmmKokkos<Host>;
}
