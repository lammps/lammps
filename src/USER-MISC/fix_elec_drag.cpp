...
static const char cite_fix_elec_drag_c[] =
...
FixElecDrag::FixElecDrag(LAMMPS *lmp, int narg, char **arg) :
 Fix(lmp,narg,arg)
{
   if (narg < 3 + 3*atom->ntypes) {
      error->all(FLERR,"Incorrect number of fix elec/drag arguments");
   }
...
void FixElecDrag::setup(int vflag)
{
...
void FixElecDrag::post_force(int vflag)
{
   double **v = atom->v;
   double **f = atom->f;
   int *type  = atom->type;
...
void FixElecDrag::post_force_respa(int vflag, int ilevel, int iloop)
{
...
double FixElecDrag::compute_scalar()
{
