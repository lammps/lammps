...
void PairReaxFFKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  ...
  if (neighflag == HALF) {
      if (evflag)
        ...
        Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, PairReaxFFComputeTorsion<HALF,0>>(0,nnz),*this);
      ev_all += ev;
    } else {
      if (evflag) {
        Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, PairReaxFFComputeTorsion<HALFTHREAD,1>>(0,nnz),*this,ev);
      } else{
        Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, PairReaxFFComputeTorsion<HALFTHREAD,0>>(0,nnz),*this);
      }
      ev_all += ev;
    }
    #endif
