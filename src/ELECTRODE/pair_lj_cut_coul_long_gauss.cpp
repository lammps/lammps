...
void PairLJCutCoulLongGauss::coeff(int narg, char **arg)
{
  ...
  bool ispoint_one = (strcmp(arg[4], "NULL") == 0);
  double eta_one = 0.;
  if (!ispoint_one) {
    eta_one = utils::numeric(FLERR, arg[4], false, lmp);
    if (eta_one < -SMALL)
      error->all(FLERR, "Gaussian parameter eta is set to a negative value");
    else if (eta_one < SMALL && comm->me == 0)
      error->warning(FLERR, "Gaussian parameter eta is set to zero");
  }
...
void PairLJCutCoulLongGauss::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (i == j) fwrite(&ispoint[i], sizeof(bool), 1, fp);
...
void PairLJCutCoulLongGauss::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          if (i == j) utils::sfread(FLERR, &ispoint[i], sizeof(bool), 1, fp, nullptr, error);
        ...
        if (i == j) MPI_Bcast(&ispoint[i], 1, MPI_DOUBLE, 0, world);
