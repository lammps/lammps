...
FixCACSwap::FixCACSwap(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
{
  if (narg < 10) error->all(FLERR,"Illegal fix cac/swap command");
  ...
  if (ncycles < 0) error->all(FLERR,"Illegal fix cac/swap command");
  if (seed <= 0) error->all(FLERR,"Illegal fix cac/swap command");

  memory->create(type_list,atom->ntypes,"cac/swap:type_list");
  memory->create(mu,atom->ntypes+1,"cac/swap:mu");
...
void FixCACSwap::options(int narg, char **arg)
{
  if (narg < 0) error->all(FLERR,"Illegal fix cac/swap command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix cac/swap command");
      iregion = domain->find_region(arg[iarg+1]);
      if (iregion == -1)
        error->all(FLERR,"Region ID for fix cac/swap does not exist");
      ...
      if (atom->CAC_flag) error->all(FLERR,"the ke keyword is not implemented for CAC atom styles");
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix cac/swap command");
      ...
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix cac/swap command");
      iarg++;
      while (iarg < narg) {
...
void FixCACSwap::init()
{
  ...
  if (nswaptypes < 2)
    error->all(FLERR,"Must specify at least 2 types in fix cac/swap command");

  if (semi_grand_flag) {
    if (nswaptypes != nmutypes)
      error->all(FLERR,"Need nswaptypes mu values in fix cac/swap command");
  } else {
    if (nswaptypes != 2)
      error->all(FLERR,"Only 2 types allowed when not using semi-grand in fix cac/swap command");
    if (nmutypes != 0)
      error->all(FLERR,"Mu not allowed when not using semi-grand in fix cac/swap command");
  }

  for (int iswaptype = 0; iswaptype < nswaptypes; iswaptype++)
    if (type_list[iswaptype] <= 0 || type_list[iswaptype] > atom->ntypes)
      error->all(FLERR,"Invalid atom type in fix cac/swap command");
  ...
  if (atom->q_flag && !semi_grand_flag) {
    double qmax,qmin;
    int firstall,first;
    memory->create(qtype,nswaptypes,"cac/swap:qtype");
    for (int iswaptype = 0; iswaptype < nswaptypes; iswaptype++) {
      first = 1;
  ...
  memory->create(sqrt_mass_ratio,atom->ntypes+1,atom->ntypes+1,"cac/swap:sqrt_mass_ratio");
  ...
  if (atom->firstgroup >= 0) {
    ...
    if (flagall)
      error->all(FLERR,"Cannot do cac/swap on atoms in atom_modify first group");
  }
}
...
int FixCACSwap::attempt_semi_grand()
{
  if (nswap == 0) return 0;
  ...
  if (i >= 0) {
    ...
    itype = atom->node_types[i][0];
    while (itype == jtype) {
      jswaptype = static_cast<int> (nswaptypes*random_unequal->uniform());
      jtype = type_list[jswaptype];
    }
    atom->node_types[i][0] = jtype;
  }
  ...
  if (random_equal->uniform() <
    ...
    if (conserve_ke_flag) {
      if (i >= 0) {
        ...
        atom->v[i][1] *= sqrt_mass_ratio[itype][jtype];
        atom->v[i][2] *= sqrt_mass_ratio[itype][jtype];
        atom->nodal_velocities[i][0][0][0] *= sqrt_mass_ratio[itype][jtype];
        atom->nodal_velocities[i][0][0][1] *= sqrt_mass_ratio[itype][jtype];
        atom->nodal_velocities[i][0][0][2] *= sqrt_mass_ratio[itype][jtype];
      }
      if (j >= 0) {
        atom->v[j][0] *= sqrt_mass_ratio[jtype][itype];
        atom->v[j][1] *= sqrt_mass_ratio[jtype][itype];
        atom->v[j][2] *= sqrt_mass_ratio[jtype][itype];
        atom->nodal_velocities[j][0][0][0] *= sqrt_mass_ratio[itype][jtype];
        atom->nodal_velocities[j][0][0][1] *= sqrt_mass_ratio[itype][jtype];
        atom->nodal_velocities[j][0][0][2] *= sqrt_mass_ratio[itype][jtype];
      }
    }
    return 1;
  } else {
    if (i >= 0) {
      atom->node_types[i][0] =  type_list[0];
      if (atom->q_flag) atom->q[i] = qtype[0];
    }
    if (j >= 0) {
      atom->node_types[j][0] =  type_list[1];
...
int FixCACSwap::pick_semi_grand_atom()
{
  int i = -1;
...
int FixCACSwap::pick_i_swap_atom()
{
  int i = -1;
...
int FixCACSwap::pick_j_swap_atom()
{
  int j = -1;
...
void FixCACSwap::update_semi_grand_atoms_list()
{
  int nlocal = atom->nlocal;
  double **x = atom->x;

  double ****x_node = atom->nodal_positions;
  ...
  if (regionflag) {
    for (int i = 0; i < nlocal; i++) {
      if (atom->nodes_per_element_list[element_type[i]] != 1) 
        error->all(FLERR,"fix cac/swap may currently only be used on regions containing CAC Atoms");
      if (domain->regions[iregion]->match(x_node[i][0][0][0],x_node[i][0][0][1],x_node[i][0][0][2]) == 1) {
        if (atom->mask[i] & groupbit) {
          int itype = atom->node_types[i][0];
          int iswaptype;
    ...
    for (int i = 0; i < nlocal; i++) {
      if (atom->nodes_per_element_list[element_type[i]] != 1) 
        error->all(FLERR,"fix cac/swap may currently only be used on regions containing CAC Atoms, use region keyword");
      if (atom->mask[i] & groupbit) {
...
void FixCACSwap::update_swap_atoms_list()
{
  int nlocal = atom->nlocal;
  int *type = atom->type;
  double **x = atom->x;
  ...
  if (regionflag) {
    for (int i = 0; i < nlocal; i++) {
      ...
      if (domain->regions[iregion]->match(x_node[i][0][0][0],x_node[i][0][0][1],x_node[i][0][0][2]) == 1) {
        if (atom->mask[i] & groupbit) {
          if (node_types[i][0] ==  type_list[0]) {
            local_swap_iatom_list[niswap_local] = i;
            niswap_local++;
          } else if (node_types[i][0] ==  type_list[1]) {
...
int FixCACSwap::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/)
{
  int i,j,m;
...
void FixCACSwap::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double FixCACSwap::compute_vector(int n)
{
...
double FixCACSwap::memory_usage()
{
...
void FixCACSwap::write_restart(FILE *fp)
{
  int n = 0;
  double list[6];
...
void FixCACSwap::restart(char *buf)
{
  int n = 0;
  ...
  if (ntimestep_restart != update->ntimestep)
    error->all(FLERR,"Must not reset timestep when restarting fix cac/swap");
}
