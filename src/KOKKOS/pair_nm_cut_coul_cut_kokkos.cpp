...
#include "pair_nm_cut_coul_cut_kokkos.h"
...
PairNMCutCoulCutKokkos<DeviceType>::PairNMCutCoulCutKokkos(LAMMPS *lmp)
  : PairNMCutCoulCut(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairNMCutCoulCutKokkos<DeviceType>::~PairNMCutCoulCutKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairNMCutCoulCutKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  ev = pair_compute<PairNMCutCoulCutKokkos<DeviceType>,void>
...
KK_FLOAT PairNMCutCoulCutKokkos<DeviceType>::
compute_fpair(const KK_FLOAT& rsq, const int& /*i*/, const int& /*j*/,
              const int& itype, const int& jtype) const
{
  const KK_FLOAT r2inv = static_cast<KK_FLOAT>(1.0) / rsq;
  const KK_FLOAT nn_val = (STACKPARAMS?m_params[itype][jtype].nn:params(itype,jtype).nn);
  const KK_FLOAT mm_val = (STACKPARAMS?m_params[itype][jtype].mm:params(itype,jtype).mm);
  const KK_FLOAT nm_val = (STACKPARAMS?m_params[itype][jtype].nm:params(itype,jtype).nm);
  const KK_FLOAT rninv = Kokkos::pow(r2inv, nn_val * static_cast<KK_FLOAT>(0.5));
  const KK_FLOAT rminv = Kokkos::pow(r2inv, mm_val * static_cast<KK_FLOAT>(0.5));

  const KK_FLOAT forcenm =
    (STACKPARAMS?m_params[itype][jtype].e0nm:params(itype,jtype).e0nm) * nm_val *
    ((STACKPARAMS?m_params[itype][jtype].r0n:params(itype,jtype).r0n)*rninv -
     (STACKPARAMS?m_params[itype][jtype].r0m:params(itype,jtype).r0m)*rminv);
  return forcenm*r2inv;
}

/* ----------------------------------------------------------------------
   plain Coulomb cut force
------------------------------------------------------------------------- */
              ...
              const KK_FLOAT& factor_coul, const KK_FLOAT& qtmp) const
{
  ...
  return factor_coul * qqrd2e * qtmp * q(j) * rinv * r2inv;
}
               ...
               const int& itype, const int& jtype) const
{
  ...
  return (STACKPARAMS?m_params[itype][jtype].e0nm:params(itype,jtype).e0nm) *
    (mm_val*(STACKPARAMS?m_params[itype][jtype].r0n:params(itype,jtype).r0n)*rninv -
     nn_val*(STACKPARAMS?m_params[itype][jtype].r0m:params(itype,jtype).r0m)*rminv) -
    (STACKPARAMS?m_params[itype][jtype].offset:params(itype,jtype).offset);
}

/* ----------------------------------------------------------------------
   plain Coulomb cut energy
------------------------------------------------------------------------- */
...
compute_ecoul(const KK_FLOAT& rsq, const int& /*i*/, const int& j,
               ...
               const KK_FLOAT& factor_coul, const KK_FLOAT& qtmp) const
{
  const KK_FLOAT rinv = Kokkos::sqrt(static_cast<KK_FLOAT>(1.0)/rsq);
...
template<class DeviceType>
void PairNMCutCoulCutKokkos<DeviceType>::allocate()
{
  PairNMCutCoulCut::allocate();
  ...
  k_params = Kokkos::DualView<params_nm_coul**,Kokkos::LayoutRight,DeviceType>(
    "PairNMCutCoulCut::params",n+1,n+1);
...
template<class DeviceType>
void PairNMCutCoulCutKokkos<DeviceType>::init_style()
{
  PairNMCutCoulCut::init_style();
...
template<class DeviceType>
double PairNMCutCoulCutKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairNMCutCoulCut::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairNMCutCoulCutKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairNMCutCoulCutKokkos<LMPHostType>;
#endif
}
