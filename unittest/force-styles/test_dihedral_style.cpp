...
/* ----------------------------------------------------------------------
   the per-atom virial of a dihedral style has to add up to its global
   ...
   branches of the dihedral styles and of their accelerated variants are
...
// the units of Dihedral::virial before it can be compared with it
...
TEST(DihedralStyle, vatom_only_omp)
{
    if (!Info::has_package("OPENMP")) GTEST_SKIP();
    if (test_config.skip_tests.count(test_info_->name())) GTEST_SKIP();
...
TEST(DihedralStyle, vatom_only_kokkos)
{
    ...
    LAMMPS::argv args = {"DihedralStyle", "-log", "none", "-echo", "screen", "-nocite",
                         "-k", "on", "t", "1", "-sf", "kk"};
    if (kk_gpu)
        args = {"DihedralStyle", "-log", "none", "-echo", "screen", "-nocite",
                "-k", "on", "g", "1", "-sf", "kk",
