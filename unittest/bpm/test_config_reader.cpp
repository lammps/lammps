...
TestConfigReader::TestConfigReader(TestConfig &config) : config(config)
{
    ...
    consumers["run_force"]      = &TestConfigReader::run_force;
...
void TestConfigReader::run_force(const yaml_event_t &event)
{
    parse_segment_block(config.seg_force, (const char *)event.data.scalar.value);
}
