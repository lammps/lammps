...
BondMorseKokkos<DeviceType>::BondMorseKokkos(LAMMPS *lmp) : BondMorse(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondMorseKokkos<DeviceType>::~BondMorseKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondMorseKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondMorseCompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondMorseCompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondMorseCompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondMorseCompute<0,0> >(0,nbondlist),*this);
    }
  }
...
void BondMorseKokkos<DeviceType>::operator()(TagBondMorseCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondMorseKokkos<DeviceType>::operator()(TagBondMorseCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondMorseCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondMorseKokkos<DeviceType>::allocate()
{
  BondMorse::allocate();
...
template<class DeviceType>
void BondMorseKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondMorse::coeff(narg, arg);

  int ilo,ihi;
  utils::bounds(FLERR,arg[0],1,atom->nbondtypes,ilo,ihi,error);

  for (int i = ilo; i <= ihi; i++) {
    k_d0.view_host()[i] = static_cast<KK_FLOAT>(d0[i]);
    k_alpha.view_host()[i] = static_cast<KK_FLOAT>(alpha[i]);
...
template<class DeviceType>
void BondMorseKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondMorse::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondMorseKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
...
namespace LAMMPS_NS {
template class BondMorseKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class BondMorseKokkos<LMPHostType>;
#endif
}
