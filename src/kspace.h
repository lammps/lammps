...
class KSpace : protected Pointers {
 ...
 public:
  ...
  // Return the electrostatic charge-site position for atom i and the atoms
  // that receive a force applied at that site.  Point-charge models use the
  // atom itself.  Virtual-site models such as TIP4P override this mapping.
  // The caller provides arrays with room for at least three force recipients.
  ...
  // Compute the reciprocal-space electrostatic potential generated by a
  // source group at atoms in a sensor group.  The returned potential omits
  // the Coulomb conversion factor (force->qqrd2e), so its natural units are
  // charge/length.  Solvers advertising support return 1; the default return
  // value lets clients reject unsupported KSpace styles without relying on
  // style-name checks.
  virtual int compute_group_potential(double *, int, int, bool) { return 0; }
