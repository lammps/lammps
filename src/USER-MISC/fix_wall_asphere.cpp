...
void FixWallAsphere::wall_particle(int m, int which, double coord)
{
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      double A[3][3] = {{0,0,0},{0,0,0},{0,0,0}}; //Rotation Matrix
      ...
      //sigma for the onflag check.
      double* shape = atom->bonus[ellipsoid[i]].shape ;
      MathExtra::quat_to_mat_trans(bonus[ellipsoid[i]].quat,A);
      MathExtra::transpose_matvec(A,nhat,tempvec); //tempvec = A . nhat
      for(int k = 0; k<3; k++) tempvec[k] *= shape[k]; //tempvec = S . tempvec = S . A . nhat
      for(int k = 0; k<3 ; k++) sigman2 += tempvec[k]*tempvec[k]; // s_n^2 = |tempvex|^2
      sigman = sqrt(sigman2);
      ...
      for(int k = 0; k<3; k++) SAn[k] = tempvec[k]; //  SAn = S . tempvec = S . A . nhat
    ...
    for(int k = 0; k<3; k++) tempvec2[k] *= shape[k]; //SAL_x|n>
    that[0] = MathExtra::dot3(SAn,tempvec2);


    MathExtra::matvec(Ly,nhat,tempvec);
    MathExtra::transpose_matvec(A,tempvec,tempvec2);
    for(int k = 0; k<3; k++) tempvec2[k] *= shape[k]; //SAL_y|n>
    that[1] = MathExtra::dot3(SAn,tempvec2);


    MathExtra::matvec(Lz,nhat,tempvec);
    MathExtra::transpose_matvec(A,tempvec,tempvec2);
    for(int k = 0; k < 3; k++) tempvec2[k] *= shape[k];//SAL_z|n>
