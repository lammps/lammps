...
void PairLJCutCoulLongTIP4POMP::compute(int eflag, int vflag)
{
  ...
  if (evflag) {
    if (eflag)
      if (vflag)
	return eval_tip4p<1,1,1>();
      else
	return eval_tip4p<1,1,0>();
    else 
      if (vflag)
	return eval_tip4p<1,0,1>();
      else
	return eval_tip4p<1,0,0>();
  } else {
    return eval_tip4p<0,0,0>();
  }
}

template <int EVFLAG, int EFLAG, int VFLAG>
void PairLJCutCoulLongTIP4POMP::eval_tip4p()
{
...
#endif
  {
	...
	if (rsq < cutsq[itype][jtype]) {
	  ...
	  if (rsq < cut_ljsq[itype][jtype]) {
	    ...
	    if (EVFLAG) ev_tally_thr(i,j,nlocal,1,evdwl,0.0,
				     forcelj,delx,dely,delz,tid);
	  }
	  ...
	  if (rsq < cut_coulsq) {
	    ...
	    if (EVFLAG) ev_tally_list_thr(n,vlist,ecoul,v,tid);
	  }
	}
      }
    }
...
void PairLJCutCoulLongTIP4POMP::settings(int narg, char **arg)
{
...
void PairLJCutCoulLongTIP4POMP::init_style()
{
  if (atom->tag_enable == 0)
    error->all("Pair style lj/cut/coul/long/tip4p/omp requires atom IDs");
  if (!force->newton_pair) 
    error->all("Pair style lj/cut/coul/long/tip4p/omp requires newton pair on");
  if (!atom->q_flag)
    error->all("Pair style lj/cut/coul/long/tip4p/omp requires atom attribute q");
  ...
  PairLJCutCoulLongOMP::init_style();
...
void PairLJCutCoulLongTIP4POMP::write_restart_settings(FILE *fp)
{
...
void PairLJCutCoulLongTIP4POMP::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutCoulLongTIP4POMP::find_M(int i, int &iH1, int &iH2, double *xM)
{
