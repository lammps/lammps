...
// 
//   bytes += podptr->memory_usage(); // SNA object
...
{    
    //printf("free memory for temporary arrays\n");
...
{        
    //printf("free memory for atom arrays\n");
...
{                    
    if (podpairlist) {
        //printf("free memory for pair arrays\n");
...
{
    //printf("allocate memory for temporary arrays\n");
...
{
    //printf("allocate memory for atom arrays\n");
...
{        
    if (podpairlist) {
        //printf("allocate memory for pair arrays\n");
...
//     podptr->print_matrix("force", dim, inum, f, dim);
...
//         cout<<nat<<endl;
//         podptr->print_matrix("localdesc", nat, nd1234, localdesc, nat);
//         podptr->print_matrix("gd", 1, nd1234, gd, 1);
//         podptr->print_matrix("eatom", 1, nat, ea, 1);
    }            
    //error->all(FLERR,"here");
...
//     printf("\nExecution time (in millisec) for lammpsenergy %g\n", 
...
//     gettimeofday(&tv2, NULL);            
//     printf("\nExecution time (in millisec) for lammpsforce %g\n", 
//         (double)(tv2.tv_usec-tv1.tv_usec)/1000 + 
//         (double)(tv2.tv_sec -tv1.tv_sec )*1000);    
...
{                    
    ...
    for (int file = 0; file < nfiles; file++) { // loop over each file in the training data set
        ...
        for (int ii=0; ii < nconfigs; ii++) { // loop over each configuration in a file
            ...
            //podptr->print_matrix("f", dim, natom, f, dim);            
            //error->all(FLERR,"here");
            ...
            //printf("%g %g %g\n", outarray[5 + m*ci], outarray[6 + m*ci], podptr->podArrayErrorNorm(f, DFTforce, nforce));
            // printf("%d %g %g\n", ci, energy, DFTenergy);
//             error->all(FLERR,"here");
