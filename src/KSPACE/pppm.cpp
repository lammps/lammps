...
/* ----------------------------------------------------------------------
   compute the mesh (reciprocal-space) potential from one atom group at
   atoms in another group

   This is intentionally a source/sensor operation instead of recovering a
   potential from per-atom energy.  Sensors may therefore carry zero charge,
   which is required by polarizable QM/MM Hamiltonians.  The regular PPPM
   compute() call rebuilds all overwritten mesh data, so this operation is
   safe immediately before the normal force calculation.
------------------------------------------------------------------------- */

int PPPM::compute_group_potential(double *potential, int sensor_groupbit, int source_groupbit,
                                  bool invert_source)
{
  // Dispersion PPPM has additional meshes that are outside the scope of this
  // electrostatic source/sensor solve.  TIP4P is supported through the
  // virtual charge-site mapping supplied by PPPMTIP4P.
  if (dispersionflag) return 0;
  ...
  // u_brick is normally allocated only for ik differentiation or per-atom
  // energy output.  A scalar source-to-sensor solve needs the same mesh even
  // when the production PPPM calculation uses analytic differentiation.
  const bool temporary_u_brick = (u_brick == nullptr);
  if (temporary_u_brick)
    memory->create3d_offset(u_brick,nzlo_out,nzhi_out,nylo_out,nyhi_out,
                            nxlo_out,nxhi_out,"pppm:u_brick");

  particle_map();
  make_rho_group(source_groupbit,invert_source);
  gc->reverse_comm(Grid3d::KSPACE,this,REVERSE_RHO,1,sizeof(FFT_SCALAR),
                   gc_buf1,gc_buf2,MPI_FFT_SCALAR);
  brick2fft();

  // Transform rho, multiply by the influence function, and interpolate the
  // resulting scalar potential back to the sensor atoms.
  ...
  gc->forward_comm(Grid3d::KSPACE,this,FORWARD_AD,1,sizeof(FFT_SCALAR),
                   gc_buf1,gc_buf2,MPI_FFT_SCALAR);
  project_group_potential(potential,sensor_groupbit);
...
void PPPM::make_rho_group(int source_groupbit, bool invert_source)
{
  int l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
  ...
  for (int i = 0; i < nlocal; i++) {
    const bool in_source = !!(mask[i] & source_groupbit) != invert_source;
    if (!in_source) continue;

    double site[3], weights[3];
    int indices[3];
    get_charge_site(i,site,indices,weights);

    nx = part2grid[i][0];
    ny = part2grid[i][1];
    nz = part2grid[i][2];
    dx = nx+shiftone - (site[0]-boxlo[0])*delxinv;
    dy = ny+shiftone - (site[1]-boxlo[1])*delyinv;
    dz = nz+shiftone - (site[2]-boxlo[2])*delzinv;
...
/* ----------------------------------------------------------------------
   interpolate a scalar mesh potential to selected sensor atoms
------------------------------------------------------------------------- */

void PPPM::project_group_potential(double *potential, int sensor_groupbit)
{
  int *mask = atom->mask;
  ...
  for (int i = 0; i < atom->nlocal; i++) {
    if (!(mask[i] & sensor_groupbit)) continue;
    ...
    FFT_SCALAR dix = nix+shiftone - (site[0]-boxlo[0])*delxinv;
    FFT_SCALAR diy = niy+shiftone - (site[1]-boxlo[1])*delyinv;
    FFT_SCALAR diz = niz+shiftone - (site[2]-boxlo[2])*delzinv;
    ...
    for (int ni = nlower; ni <= nupper; ni++) {
      ...
      for (int mi = nlower; mi <= nupper; mi++) {
        ...
        for (int li = nlower; li <= nupper; li++) {
          int mix = li+nix;
          value += iy0*rho1d[0][li]*u_brick[miz][miy][mix];
        }
      }
    }
