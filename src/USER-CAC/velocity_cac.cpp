...
void VelocityCAC::command(int narg, char **arg)
{
  if (!atom->CAC_flag) error->all(FLERR,"CAC commands require a CAC atom style");
  if (narg < 2) error->all(FLERR,"Illegal velocity/cac command");

  if (domain->box_exist == 0)
    error->all(FLERR,"VelocityCAC command before simulation box is defined");
  if (atom->natoms == 0)
    error->all(FLERR,"VelocityCAC command with no atoms existing");
  ...
  if (igroup == -1) error->all(FLERR,"Could not find velocity/cac group ID");
  ...
  else error->all(FLERR,"Illegal velocity/cac command");
...
/* ----------------------------------------------------------------------
   initialization of defaults before calling velocity/cac methods externaly
------------------------------------------------------------------------- */

void VelocityCAC::init_external(const char *extgroup)
{
...
void VelocityCAC::create(double t_desired, int seed)
{
  int i;
  double **vhold;

  if (seed <= 0) error->all(FLERR,"Illegal velocity/cac create command");
  ...
  if (sum_flag) {
    ...
    memory->create(vhold,nlocal,3,"velocity/cac:vhold");
    for (i = 0; i < nlocal; i++) {
      vhold[i][0] = v[i][0];
      vhold[i][1] = v[i][1];
      vhold[i][2] = v[i][2];
  ...
  // create a new ComputeTemp with the velocity/cac group
  ...
  if (loop_flag == ALL) {
    ...
    if (atom->natoms > MAXSMALLINT)
      error->all(FLERR,"Too big a problem to use velocity/cac create loop all");
    if (atom->tag_enable == 0)
      error->all(FLERR,
                 "Cannot use velocity/cac create loop all unless atoms have IDs");
    if (atom->tag_consecutive() == 0)
      error->all(FLERR,
                 "Atom IDs must be consecutive for velocity/cac create loop all");
...
void VelocityCAC::set(int /*narg*/, char **arg)
{
  ...
  if (xstr) {
    ...
    if (xvar < 0)
      error->all(FLERR,"Variable name for velocity/cac set does not exist");
    if (input->variable->equalstyle(xvar)) xstyle = EQUAL;
    else if (input->variable->atomstyle(xvar)) xstyle = ATOM;
    else error->all(FLERR,"Variable for velocity/cac set is invalid style");
  }
  if (ystr) {
  ...
  double **vfield = NULL;
  if (varflag == ATOM) memory->create(vfield,atom->nlocal,3,"velocity/cac:vfield");
...
void VelocityCAC::ramp(int /*narg*/, char **arg)
{
  // set scale factors
  ...
  if (v_dim == 2 && domain->dimension == 2)
    error->all(FLERR,"VelocityCAC ramp in z for a 2d problem");
...
void VelocityCAC::zero(int /*narg*/, char **arg)
{
  if (strcmp(arg[0],"linear") == 0) {
    if (rfix < 0) zero_momentum();
      ...
      modify->fix[rfix]->zero_momentum();
    } else error->all(FLERR,"VelocityCAC rigid used with non-rigid fix-ID");
...
void VelocityCAC::rescale(double t_old, double t_new)
{
...
void VelocityCAC::zero_momentum()
{
...
void VelocityCAC::zero_rotation()
{
  int i;
...
void VelocityCAC::options(int narg, char **arg)
{
  if (narg < 0) error->all(FLERR,"Illegal velocity/cac command");

  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"dist") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal velocity/cac command");
      ...
      if (icompute == modify->ncompute)
        error->all(FLERR,"Could not find velocity/cac temperature ID");
      temperature = modify->compute[icompute];
      if (temperature->tempflag == 0)
        error->all(FLERR,
                   "VelocityCAC temperature ID does not compute temperature");
      iarg += 2;
      ...
      if (rfix < 0) error->all(FLERR,"Fix ID for velocity/cac does not exist");
      iarg += 2;
  ...
  if (bias_flag && temperature->tempbias == 0)
    error->all(FLERR,"VelocityCAC temperature ID does calculate a velocity bias");
}
