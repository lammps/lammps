...
#include "pair_coul_cut_soft_kokkos.h"
...
PairCoulCutSoftKokkos<DeviceType>::PairCoulCutSoftKokkos(LAMMPS *lmp) : PairCoulCutSoft(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
PairCoulCutSoftKokkos<DeviceType>::~PairCoulCutSoftKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairCoulCutSoftKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairCoulCutSoftKokkos<DeviceType>,void >
...
KK_FLOAT PairCoulCutSoftKokkos<DeviceType>::
compute_fcoul(const KK_FLOAT& rsq, const int& /*i*/, const int&j, const int& itype,
  ...
  return factor_coul * qqrd2e * lam1 * qtmp * q(j) / (denc*denc*denc);
}
...
compute_ecoul(const KK_FLOAT& rsq, const int& /*i*/, const int&j, const int& itype,
  ...
  return factor_coul * qqrd2e * lam1 * qtmp * q(j) / denc;
}
...
template<class DeviceType>
void PairCoulCutSoftKokkos<DeviceType>::allocate()
{
  PairCoulCutSoft::allocate();
  ...
  k_params = Kokkos::DualView<params_coul**,Kokkos::LayoutRight,DeviceType>("PairCoulCutSoft::params",n+1,n+1);
...
template<class DeviceType>
void PairCoulCutSoftKokkos<DeviceType>::init_style()
{
  PairCoulCutSoft::init_style();
...
template<class DeviceType>
double PairCoulCutSoftKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairCoulCutSoft::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairCoulCutSoftKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairCoulCutSoftKokkos<LMPHostType>;
#endif
}
