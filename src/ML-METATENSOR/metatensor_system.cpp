...
MetatensorSystemAdaptor::MetatensorSystemAdaptor(LAMMPS *lmp, Pair *requestor,
                                                 MetatensorSystemOptions options) :
    Pointers(lmp), options_(std::move(options)), list_(nullptr), caches_(),
    atomic_types_(
        torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU)))
{
  ...
  this->strain = torch::eye(
      3, torch::TensorOptions().dtype(torch::kFloat64).device(torch::kCPU).requires_grad(true));
}

MetatensorSystemAdaptor::MetatensorSystemAdaptor(LAMMPS *lmp, Compute *requestor,
                                                 MetatensorSystemOptions options) :
    Pointers(lmp), options_(std::move(options)), list_(nullptr), caches_(),
    atomic_types_(
        torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU)))
...
void MetatensorSystemAdaptor::add_nl_request(double cutoff,
...
static std::array<int32_t, 3> cell_shifts(const std::array<std::array<double, 3>, 3> &cell_inv,
                                          const std::array<double, 3> &pair_shift)
{
  auto shift_a = static_cast<int32_t>(std::round(cell_inv[0][0] * pair_shift[0] +
                                                 cell_inv[0][1] * pair_shift[1] +
                                                 cell_inv[0][2] * pair_shift[2]));
  auto shift_b = static_cast<int32_t>(std::round(cell_inv[1][0] * pair_shift[0] +
                                                 cell_inv[1][1] * pair_shift[1] +
                                                 cell_inv[1][2] * pair_shift[2]));
  auto shift_c = static_cast<int32_t>(std::round(cell_inv[2][0] * pair_shift[0] +
...
void MetatensorSystemAdaptor::setup_neighbors(metatensor_torch::System &system)
{
  ...
  for (auto &cache : caches_) {
    ...
    for (int ii = 0; ii < (list_->inum + list_->gnum); ii++) {
      ...
      for (int jj = 0; jj < list_->numneigh[ii]; jj++) {
        ...
        auto sample = std::array<int32_t, 5>{
            original_atom_i, original_atom_j, shift[0], shift[1], shift[2],
        };
        ...
        if (cache.known_samples.insert(sample).second) {
          ...
          if (dtype == torch::kFloat64) {
            ...
            cache.distances_f32.push_back({static_cast<float>(distance[0]),
    ...
    auto samples_values =
        torch::from_blob(reinterpret_cast<int32_t *>(cache.samples.data()), {n_pairs, 5},
                         torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU));

    auto samples = torch::make_intrusive<metatensor_torch::LabelsHolder>(
        std::vector<std::string>{"first_atom", "second_atom", "cell_shift_a", "cell_shift_b",
                                 "cell_shift_c"},
        samples_values);
    ...
    if (dtype == torch::kFloat64) {
      distances_vectors =
          torch::from_blob(cache.distances_f64.data(), {n_pairs, 3, 1},
                           torch::TensorOptions().dtype(torch::kFloat64).device(torch::kCPU));
    } else if (dtype == torch::kFloat32) {
      distances_vectors =
          torch::from_blob(cache.distances_f32.data(), {n_pairs, 3, 1},
    ...
    auto neighbors = torch::make_intrusive<metatensor_torch::TensorBlockHolder>(
        distances_vectors.to(dtype).to(device), samples->to(device),
...
metatensor_torch::System MetatensorSystemAdaptor::system_from_lmp(bool do_virial,
                                                                  ...
                                                                  torch::Device device)
{
  ...
  this->positions =
      torch::from_blob(*x, {total_n_atoms, 3},
  ...
  auto system = torch::make_intrusive<metatensor_torch::SystemHolder>(atomic_types_.to(device),
                                                                      system_positions, cell);
