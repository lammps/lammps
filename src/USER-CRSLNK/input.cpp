...
void Input::parse() {
	...
	while (*ptr) {
		...
		if (quoteflag == 0) {
			...
			if (quoteflag == 3 && strstr(ptr, "\"\"\"") == ptr) {
				quoteflag = 0;
				ptr += 2;
			} else if (quoteflag == 2 && *ptr == '"')
				quoteflag = 0;
			else if (quoteflag == 1 && *ptr == '\'')
				quoteflag = 0;
		}
		ptr++;
	}
	...
	while (ptr) {
		if (narg == maxarg) {
			maxarg += DELTA;
			arg = (char **) memory->srealloc(arg, maxarg * sizeof(char *),
					"input:arg");
		}
	...
	while (*ptr) {
		...
		if (*ptr == '$' && !quoteflag) {
			...
			if (*(ptr + 1) == '{') {
				...
				if (var[i] == '\0')
					error->one(FLERR, "Invalid immediate variable");
				var[i] = '\0';
				beyond = ptr + strlen(var) + 3;
...
int Input::execute_command() {
	...
	if (!strcmp(command, "clear"))
		...
		next_command();
	else if (!strcmp(command, "partition"))
		partition();
	else if (!strcmp(command, "print"))
		print();
	else if (!strcmp(command, "python"))
		python();
	...
	else if (!strcmp(command, "angle_coeff"))
		angle_coeff();
	else if (!strcmp(command, "angle_style"))
		angle_style();
	else if (!strcmp(command, "atom_modify"))
		atom_modify();
	else if (!strcmp(command, "atom_style"))
		atom_style();
	else if (!strcmp(command, "bond_coeff"))
		bond_coeff();
	else if (!strcmp(command, "bond_style"))
		bond_style();
	else if (!strcmp(command, "bond_write"))
		bond_write();
		...
		compute();
	else if (!strcmp(command, "compute_modify"))
		compute_modify();
	else if (!strcmp(command, "dielectric"))
		dielectric();
	else if (!strcmp(command, "dihedral_coeff"))
		dihedral_coeff();
	else if (!strcmp(command, "dihedral_style"))
		dihedral_style();
		...
		fix();
	else if (!strcmp(command, "fix_modify"))
		fix_modify();
	else if (!strcmp(command, "group"))
		group_command();
	else if (!strcmp(command, "improper_coeff"))
		improper_coeff();
	else if (!strcmp(command, "improper_style"))
		improper_style();
	else if (!strcmp(command, "kspace_modify"))
		kspace_modify();
	else if (!strcmp(command, "kspace_style"))
		kspace_style();
	else if (!strcmp(command, "lattice"))
		lattice();
	else if (!strcmp(command, "mass"))
		mass();
	else if (!strcmp(command, "min_modify"))
		min_modify();
	else if (!strcmp(command, "min_style"))
		min_style();
	else if (!strcmp(command, "molecule"))
		molecule();
		...
		newton();
	else if (!strcmp(command, "package"))
		package();
	else if (!strcmp(command, "pair_coeff"))
		pair_coeff();
	else if (!strcmp(command, "pair_modify"))
		pair_modify();
	else if (!strcmp(command, "pair_style"))
		pair_style();
	else if (!strcmp(command, "pair_write"))
		pair_write();
		...
		restart();
	else if (!strcmp(command, "run_style"))
		run_style();
	else if (!strcmp(command, "special_bonds"))
		special_bonds();
	else if (!strcmp(command, "suffix"))
		suffix();
		...
		units();
	else if (!strcmp(command, "bond_type"))
		bond_type();
	else if (!strcmp(command, "angle_type"))
		angle_type();
	else if (!strcmp(command, "dihedral_type"))
		dihedral_type();
	else if (!strcmp(command, "improper_type"))
		improper_type();
	else
		flag = 0;
...
void Input::ifthenelse() {
	...
	while (iarg != narg) {
		...
		iarg = first;
		while (iarg < narg
				&& (strcmp(arg[iarg], "elif") != 0
						&& strcmp(arg[iarg], "else") != 0))
			iarg++;
		last = iarg - 1;
...
void Input::include() {
	...
	if (me == 0) {
		if (nfile == maxfile) {
			maxfile++;
			infiles = (FILE **) memory->srealloc(infiles,
					maxfile * sizeof(FILE *), "input:infiles");
		}
...
void Input::partition() {
	...
	if (yesflag) {
		if (universe->iworld + 1 >= ilo && universe->iworld + 1 <= ihi)
			one(ptr);
	} else {
		if (universe->iworld + 1 < ilo || universe->iworld + 1 > ihi)
			one(ptr);
...
void Input::quit() {
	if (narg == 0)
		error->done(0); // 1 would be fully backwards compatible
	if (narg == 1)
		error->done(force->inumeric(FLERR, arg[0]));
...
void Input::shell() {
	...
	if (narg < 1)
		error->all(FLERR, "Illegal shell command");

	if (strcmp(arg[0], "cd") == 0) {
		if (narg != 2)
			error->all(FLERR, "Illegal shell cd command");
		...
		if (narg < 2)
			error->all(FLERR, "Illegal shell mkdir command");
		if (me == 0)
		...
		if (narg != 3)
			error->all(FLERR, "Illegal shell mv command");
		...
		if (narg < 2)
			error->all(FLERR, "Illegal shell rm command");
		if (me == 0)
			for (int i = 1; i < narg; i++) {
				if (unlink(arg[i]) < 0) {
		...
		if (narg < 2)
			error->all(FLERR, "Illegal shell rmdir command");
		if (me == 0)
			for (int i = 1; i < narg; i++) {
				if (rmdir(arg[i]) < 0) {
		...
		if (narg < 2)
			error->all(FLERR, "Illegal shell putenv command");
		for (int i = 1; i < narg; i++) {
...
void Input::angle_style() {
	...
	if (force->angle)
		force->angle->settings(narg - 1, &arg[1]);
}
...
void Input::angle_type() {
	...
	variable->add_type_detector(angle, narg, arg);
}
...
void Input::atom_style() {
	if (narg < 1)
		error->all(FLERR, "Illegal atom_style command");
	if (domain->box_exist)
...
void Input::bond_type() { // i1_t i2_t bond_t
	...
	variable->add_type_detector(bond, narg, arg);
}
...
void Input::bond_write() {
	...
	if (force->bond == NULL)
		...
		force->bond->write_file(narg, arg);
}
...
void Input::comm_style() {
	if (narg < 1)
		error->all(FLERR, "Illegal comm_style command");
...
void Input::dielectric() {
	if (narg != 1)
		error->all(FLERR, "Illegal dielectric command");
...
void Input::dihedral_style() {
	...
	if (force->dihedral)
		force->dihedral->settings(narg - 1, &arg[1]);
}
...
void Input::dihedral_type() {
	...
	variable->add_type_detector(dihedral, narg, arg);
}
...
void Input::improper_style() {
	...
	if (force->improper)
		force->improper->settings(narg - 1, &arg[1]);
}
...
void Input::improper_type() {
	...
	variable->add_type_detector(improper, narg, arg);
}
...
void Input::mass() {
	if (narg != 2)
		error->all(FLERR, "Illegal mass command");
	if (domain->box_exist == 0)
...
void Input::newton() {
	...
	if (narg == 1) {
		if (strcmp(arg[0], "off") == 0)
			newton_pair = newton_bond = 0;
		else if (strcmp(arg[0], "on") == 0)
			newton_pair = newton_bond = 1;
		else
			error->all(FLERR, "Illegal newton command");
	} else if (narg == 2) {
		if (strcmp(arg[0], "off") == 0)
			newton_pair = 0;
		else if (strcmp(arg[0], "on") == 0)
...
void Input::package() {
	...
	if (narg < 1)
		error->all(FLERR, "Illegal package command");
...
void Input::pair_style() {
	...
	if (force->pair) {
		int match = 0;
		if (strcmp(arg[0], force->pair_style) == 0)
			match = 1;
		if (!match && lmp->suffix_enable) {
			char estyle[256];
			if (lmp->suffix) {
				sprintf(estyle, "%s/%s", arg[0], lmp->suffix);
				if (strcmp(estyle, force->pair_style) == 0)
					match = 1;
			}
			if (lmp->suffix2) {
...
void Input::special_bonds() {
	...
	if (domain->box_exist && atom->molecular == 1) {
		if (lj2 != force->special_lj[2] || lj3 != force->special_lj[3]
				|| coul2 != force->special_coul[2]
				|| coul3 != force->special_coul[3]
...
void Input::suffix() {
	if (narg < 1)
		error->all(FLERR, "Illegal suffix command");
...
void Input::uncompute() {
	if (narg != 1)
		error->all(FLERR, "Illegal uncompute command");
...
void Input::undump() {
	if (narg != 1)
		error->all(FLERR, "Illegal undump command");
...
void Input::unfix() {
	if (narg != 1)
		error->all(FLERR, "Illegal unfix command");
