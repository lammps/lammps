...
PairLJCoulOMP::PairLJCoulOMP(LAMMPS *lmp) : PairOMP(lmp)
{
  respa_enable = 1;
  ftable = NULL;
}
...
void PairLJCoulOMP::options(char **arg, int order)
{
...
void PairLJCoulOMP::settings(int narg, char **arg)
{
...
void *PairLJCoulOMP::extract(char *id)
{
  char *ids[] = {
...
void PairLJCoulOMP::coeff(int narg, char **arg)
{
...
void PairLJCoulOMP::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairLJCoulOMP::init_one(int i, int j)
{
...
void PairLJCoulOMP::write_restart(FILE *fp)
{
...
void PairLJCoulOMP::read_restart(FILE *fp)
{
...
void PairLJCoulOMP::write_restart_settings(FILE *fp)
{
...
void PairLJCoulOMP::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCoulOMP::compute_inner()
{
...
void PairLJCoulOMP::compute_middle()
{
...
template <int NEWTON_PAIR>
void PairLJCoulOMP::eval_middle()
{
#if defined(_OPENMP)
#pragma omp parallel default(shared)
#endif
...
void PairLJCoulOMP::compute_outer(int eflag, int vflag)
{
  if (eflag || vflag) {
    ev_setup(eflag,vflag);
    ev_setup_thr(eflag,vflag);
...
void PairLJCoulOMP::init_tables()
{
  int masklo,maskhi;
...
void PairLJCoulOMP::free_tables()
{
  memory->sfree(rtable);
...
double PairLJCoulOMP::single(int i, int j, int itype, int jtype,
