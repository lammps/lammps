...
#include "np_half_bin_newton_ssa.h"
#include "neighbor.h"
#include "nb_ssa.h"
...
NeighPairHalfBinNewtonSSA::NeighPairHalfBinNewtonSSA(LAMMPS *lmp) : 
  NeighPair(lmp) {}
...
void NeighPairHalfBinNewtonSSA::build(NeighList *list)
{
  ...
  NeighBinSSA *nb_ssa = dynamic_cast<NeighBinSSA*>(nb);
  if (!nb_ssa) error->one(FLERR, "NeigBin wasn't a NeighBinSSA object");
