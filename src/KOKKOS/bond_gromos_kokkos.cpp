...
BondGromosKokkos<DeviceType>::BondGromosKokkos(LAMMPS *lmp) : BondGromos(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondGromosKokkos<DeviceType>::~BondGromosKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondGromosKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType,
        TagBondGromosCompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType,
        TagBondGromosCompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,
        TagBondGromosCompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,
        TagBondGromosCompute<0,0> >(0,nbondlist),*this);
    }
  }
...
void BondGromosKokkos<DeviceType>::operator()(TagBondGromosCompute<NEWTON_BOND,EVFLAG>,
                                              const int &n, EV_FLOAT& ev) const
{
  ...
  const KK_FLOAT dr = rsq - r0sq;
  const KK_FLOAT kdr = d_k[type]*dr;

  // force & energy
  // E = k*(r^2 - r0^2)^2, dE/dr^2 = 2*k*(r^2 - r0^2), F/r = -dE/dr/r = -4*k*(r^2-r0^2)

  const KK_FLOAT fbond = static_cast<KK_FLOAT>(-4.0)*kdr;

  KK_FLOAT ebond = static_cast<KK_FLOAT>(0.0);
  if (EVFLAG && eflag) ebond = kdr*dr;
...
{
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondGromosCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondGromosKokkos<DeviceType>::allocate()
{
  BondGromos::allocate();
  int n = atom->nbondtypes;
  k_r0 = DAT::tdual_kkfloat_1d("BondGromos::r0",n+1);
  k_k  = DAT::tdual_kkfloat_1d("BondGromos::k",n+1);
...
template<class DeviceType>
void BondGromosKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondGromos::coeff(narg, arg);
  int ilo,ihi;
...
template<class DeviceType>
void BondGromosKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondGromos::read_restart(fp);
  int n = atom->nbondtypes;
  for (int i = 1; i <= n; i++) {
    k_r0.view_host()[i] = static_cast<KK_FLOAT>(r0[i]);
    k_k.view_host()[i]  = static_cast<KK_FLOAT>(k[i]);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondGromosKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
...
namespace LAMMPS_NS {
template class BondGromosKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class BondGromosKokkos<LMPHostType>;
#endif
}
