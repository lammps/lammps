...
#include "angle_cosine_delta_kokkos.h"
...
static constexpr KK_FLOAT SMALL = static_cast<KK_FLOAT>(0.001);

/* ---------------------------------------------------------------------- */

template<class DeviceType>
AngleCosineDeltaKokkos<DeviceType>::AngleCosineDeltaKokkos(LAMMPS *lmp) : AngleCosineDelta(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
AngleCosineDeltaKokkos<DeviceType>::~AngleCosineDeltaKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void AngleCosineDeltaKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  k_cos_theta0.template sync<DeviceType>();
  k_sin_theta0.template sync<DeviceType>();
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleCosineDeltaCompute<1,1> >(0,nanglelist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleCosineDeltaCompute<0,1> >(0,nanglelist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleCosineDeltaCompute<1,0> >(0,nanglelist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleCosineDeltaCompute<0,0> >(0,nanglelist),*this);
    }
  }
...
void AngleCosineDeltaKokkos<DeviceType>::operator()(TagAngleCosineDeltaCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  if (s < SMALL) s = SMALL;
  const KK_FLOAT cot = c/s;

  // cos(dtheta) = cos(theta - theta0) = c*c0 + s*s0

  const KK_FLOAT c0 = d_cos_theta0[type];
  const KK_FLOAT s0 = d_sin_theta0[type];
  const KK_FLOAT dcostheta = c*c0 + s*s0;
  const KK_FLOAT tk = d_k[type] * (static_cast<KK_FLOAT>(1.0) - dcostheta);
  ...
  const KK_FLOAT b11 = -a*c*cot / rsq1;
  const KK_FLOAT b12 = a*cot / (r1*r2);
  const KK_FLOAT b22 = -a*c*cot / rsq2;
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleCosineDeltaKokkos<DeviceType>::operator()(TagAngleCosineDeltaCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagAngleCosineDeltaCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void AngleCosineDeltaKokkos<DeviceType>::allocate()
{
  AngleCosineDelta::allocate();

  int n = atom->nangletypes;
  k_k = DAT::tdual_kkfloat_1d("AngleCosineDelta::k",n+1);
  k_cos_theta0 = DAT::tdual_kkfloat_1d("AngleCosineDelta::cos_theta0",n+1);
  k_sin_theta0 = DAT::tdual_kkfloat_1d("AngleCosineDelta::sin_theta0",n+1);

  d_k = k_k.template view<DeviceType>();
  d_cos_theta0 = k_cos_theta0.template view<DeviceType>();
  d_sin_theta0 = k_sin_theta0.template view<DeviceType>();
}
...
template<class DeviceType>
void AngleCosineDeltaKokkos<DeviceType>::coeff(int narg, char **arg)
{
  AngleCosineDelta::coeff(narg, arg);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    k_cos_theta0.view_host()[i] = static_cast<KK_FLOAT>(cos(theta0[i]));
    k_sin_theta0.view_host()[i] = static_cast<KK_FLOAT>(sin(theta0[i]));
  }

  k_k.modify_host();
  k_cos_theta0.modify_host();
  k_sin_theta0.modify_host();
}
...
template<class DeviceType>
void AngleCosineDeltaKokkos<DeviceType>::read_restart(FILE *fp)
{
  AngleCosineDelta::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleCosineDeltaKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
