...
PairDipoleCutOMP::PairDipoleCutOMP(LAMMPS *lmp) : PairOMP(lmp)
{
  single_enable = 0;
}
...
PairDipoleCutOMP::~PairDipoleCutOMP()
{
  if (allocated) {
...
void PairDipoleCutOMP::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairDipoleCutOMP::settings(int narg, char **arg)
{
  if (narg < 1 || narg > 2)
...
void PairDipoleCutOMP::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 6) 
...
void PairDipoleCutOMP::init_style()
{
...
double PairDipoleCutOMP::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairDipoleCutOMP::write_restart(FILE *fp)
{
...
void PairDipoleCutOMP::read_restart(FILE *fp)
{
...
void PairDipoleCutOMP::write_restart_settings(FILE *fp)
{
...
void PairDipoleCutOMP::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
