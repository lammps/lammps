...
inline double &elem(double *A, int n, int i, int j) { return A[i * n + j]; }
inline double elem(const double *A, int n, int i, int j) { return A[i * n + j]; }
...
inline int solve(int n, double *H, double *evals, double *evecs, int max_sweeps = 100)
{
  ...
  for (int sweep = 0; sweep < max_sweeps; sweep++) {
    ...
    for (int i = 0; i < n; i++)
      for (int j = i + 1; j < n; j++)
        off_diag += elem(H, n, i, j) * elem(H, n, i, j);
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    if (k != i) {
      double tmp = evals[i]; evals[i] = evals[k]; evals[k] = tmp;
      for (int r = 0; r < n; r++) {
