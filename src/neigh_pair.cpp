...
NeighPair::NeighPair(LAMMPS *lmp) : Pointers(lmp)
{
  last_build = -1;
...
void NeighPair::copy_neighbor_info()
{
  // general params
...
/* ----------------------------------------------------------------------
   copy bin geometry info from NeighBin class to this build class
------------------------------------------------------------------------- */

void NeighPair::copy_bin_setup_info()
{
  nbinx = nb->nbinx;
  nbiny = nb->nbiny;
  nbinz = nb->nbinz;
...
/* ----------------------------------------------------------------------
   copy per-atom and per-bin vectors from NeighBin class to this build class
------------------------------------------------------------------------- */

void NeighPair::copy_bin_info()
{
  bins = nb->bins;
  binhead = nb->binhead;
}
...
/* ----------------------------------------------------------------------
   copy needed info from NeighStencil class to this build class
------------------------------------------------------------------------- */
...
int NeighPair::exclusion(int i, int j, int itype, int jtype,
...
int NeighPair::coord2bin(double *x)
{
  int ix,iy,iz;
...
int NeighPair::coord2bin(double *x, int &ix, int &iy, int &iz)
{
  if (!ISFINITE(x[0]) || !ISFINITE(x[1]) || !ISFINITE(x[2]))
    error->one(FLERR,"Non-numeric positions - simulation unstable");
