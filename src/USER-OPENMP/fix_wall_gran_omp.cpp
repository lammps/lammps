  ...
  Fix(lmp, narg, arg)
{
  ...
  if (force->pair_match("gran/hertz/history/omp",1)) {
    kn /= force->nktv2p;
    kt /= force->nktv2p;
  }
...
void FixWallGranOMP::init()
{
  ...
  if (force->pair_match("gran/hooke/omp",1))
    pairstyle = HOOKE;
...
void FixWallGranOMP::hooke(double rsq, double dx, double dy, double dz,
...
void FixWallGranOMP::hooke_history(double rsq, double dx, double dy, double dz,
				double *vwall, double *v,
				double *f, double *omega, double *torque,
...
void FixWallGranOMP::hertz_history(double rsq, double dx, double dy, double dz,
				double *vwall, double *v,
				double *f, double *omega, double *torque,
				double radius, double mass, double *shear)
{
...
double FixWallGranOMP::memory_usage()
{
  int nmax = atom->nmax;
...
void FixWallGranOMP::grow_arrays(int nmax)
{
...
void FixWallGranOMP::copy_arrays(int i, int j)
{
  shear[j][0] = shear[i][0];
  shear[j][1] = shear[i][1];
  shear[j][2] = shear[i][2];
...
void FixWallGranOMP::set_arrays(int i)
{
  shear[i][0] = shear[i][1] = shear[i][2] = 0.0;
}
...
int FixWallGranOMP::pack_exchange(int i, double *buf)
{
  buf[0] = shear[i][0];
  buf[1] = shear[i][1];
  buf[2] = shear[i][2];
...
int FixWallGranOMP::unpack_exchange(int nlocal, double *buf)
{
...
int FixWallGranOMP::pack_restart(int i, double *buf)
{
  int m = 0;
  buf[m++] = 4;
  buf[m++] = shear[i][0];
...
void FixWallGranOMP::unpack_restart(int nlocal, int nth)
{
...
int FixWallGranOMP::maxsize_restart()
{
  return 4;
}
...
int FixWallGranOMP::size_restart(int nlocal)
{
  return 4;
}
