...
counter += check_style('Commands_bond.rst', doc_dir, ":doc:`(.+) <bond.+>`",bond,'Bond',skip=skip_bond,suffix=True)
counter += check_style('bond_style.rst', doc_dir, ":doc:`(.+) <bond.+>` -",bond,'Bond',skip=skip_bond,suffix=False)
...
counter += check_style_index("bond_style", bond, index["bond_style"], skip=['quartic'])
