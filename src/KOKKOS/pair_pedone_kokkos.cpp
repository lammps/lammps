...
PairPedoneKokkos<DeviceType>::PairPedoneKokkos(LAMMPS *lmp) : PairPedone(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairPedoneKokkos<DeviceType>::~PairPedoneKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairPedoneKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairPedoneKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairPedoneKokkos<DeviceType>::
compute_fpair(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const KK_FLOAT pedone1_val= (STACKPARAMS?m_params[itype][jtype].pedone1:params(itype,jtype).pedone1);
  const KK_FLOAT pedone2_val= (STACKPARAMS?m_params[itype][jtype].pedone2:params(itype,jtype).pedone2);
  const KK_FLOAT dr = r - r0_val;
  const KK_FLOAT dexp = Kokkos::exp(-alpha_val * dr);
  return pedone1_val*(dexp*dexp - dexp)/r + pedone2_val*r6inv*r6inv*r2inv;
}
...
compute_evdwl(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const KK_FLOAT c0_val     = (STACKPARAMS?m_params[itype][jtype].c0:params(itype,jtype).c0);
...
template<class DeviceType>
void PairPedoneKokkos<DeviceType>::allocate()
{
  PairPedone::allocate();
  ...
  k_params = Kokkos::DualView<params_pedone**,Kokkos::LayoutRight,DeviceType>("PairPedone::params",n+1,n+1);
...
template<class DeviceType>
void PairPedoneKokkos<DeviceType>::init_style()
{
  PairPedone::init_style();
...
template<class DeviceType>
double PairPedoneKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairPedone::init_one(i,j);
  ...
  k_params.view_host()(i,j).pedone1 = static_cast<KK_FLOAT>(pedone1[i][j]);
  k_params.view_host()(i,j).pedone2 = static_cast<KK_FLOAT>(pedone2[i][j]);
...
namespace LAMMPS_NS {
template class PairPedoneKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairPedoneKokkos<LMPHostType>;
#endif
}
