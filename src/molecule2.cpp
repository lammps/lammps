...
Molecule2::Molecule2(LAMMPS *lmp, int narg, char **arg, int &index) :
...
void Molecule2::compute_inertia()
{
  if (!inertiaflag) {
    inertiaflag = 1;
...
void Molecule2::coords(char *line)
{
...
// clang-format on
void Molecule2::types(char *line)
{
...
void Molecule2::molecules(char *line)
{
  for (int i = 0; i < natoms; i++) count[i] = 0;
  try {
    for (int i = 0; i < natoms; i++) {
...
void Molecule2::fragments(char *line)
{
  try {
...
void Molecule2::charges(char *line)
{
  for (int i = 0; i < natoms; i++) count[i] = 0;
  try {
    for (int i = 0; i < natoms; i++) {
...
void Molecule2::diameters(char *line)
{
  for (int i = 0; i < natoms; i++) count[i] = 0;
  try {
    maxradius = 0.0;
...
void Molecule2::dipoles(char *line)
{
  for (int i = 0; i < natoms; i++) count[i] = 0;
  try {
    for (int i = 0; i < natoms; i++) {
...
void Molecule2::masses(char *line)
{
  for (int i = 0; i < natoms; i++) count[i] = 0;
  try {
    for (int i = 0; i < natoms; i++) {
...
void Molecule2::line_segments(char *line)
{
...
void Molecule2::triangles(char *line)
{
...
void Molecule2::bonds(int flag, char *line)
{
...
void Molecule2::angles(int flag, char *line)
{
...
void Molecule2::dihedrals(int flag, char *line)
{
...
void Molecule2::impropers(int flag, char *line)
{
...
void Molecule2::nspecial_read(int flag, char *line)
{
...
void Molecule2::special_read(char *line)
{
  try {
    for (int i = 0; i < natoms; i++) {
      readline(line);
...
void Molecule2::special_generate()
{
  int newton_bond = force->newton_bond;
  tagint atom1, atom2;
...
void Molecule2::shakeflag_read(char *line)
{
  try {
    for (int i = 0; i < natoms; i++) {
      readline(line);
...
void Molecule2::shakeatom_read(char *line)
{
...
void Molecule2::shaketype_read(char *line)
{
  int nmatch = 0, nwant = 0;
  for (int i = 0; i < natoms; i++) {
    readline(line);
...
void Molecule2::body(int flag, int pflag, char *line)
{
  int nparam = nibody;
...
int Molecule2::findfragment(const char *name)
{
  for (int i = 0; i < nfragments; i++)
...
void Molecule2::check_attributes()
{
...
void Molecule2::readline(char *line)
{
  int n;
  if (me == 0) {
...
std::string Molecule2::parse_keyword(int flag, char *line)
{
...
void Molecule2::skip_lines(int n, char *line, const std::string &section)
{
  for (int i = 0; i < n; i++) {
    readline(line);
