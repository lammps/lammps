...
#include <cstring>
#include "dump_cac_nodal_virial.h"
#include "atom.h"
#include "group.h"
#include "error.h"
#include "memory.h"
...
DumpCACNodalVirial::DumpCACNodalVirial(LAMMPS *lmp, int narg, char **arg) : Dump(lmp, narg, arg)
{
  if (narg != 5) error->all(FLERR,"Illegal dump cac/nodal/virial command");
  if (binary || multiproc) error->all(FLERR,"Invalid dump cac/nodal/virial filename");
  ...
  char *str = (char *) "%d %d %d %g %g %g %g %g %g";
  int n = strlen(str) + 1;
...
DumpCACNodalVirial::~DumpCACNodalVirial()
{
...
void DumpCACNodalVirial::init_style()
{ 
  ...
  if (buffer_flag == 1) write_choice = &DumpCACNodalVirial::write_string;
  else write_choice = &DumpCACNodalVirial::write_lines;
...
int DumpCACNodalVirial::modify_param(int narg, char **arg)
{
...
void DumpCACNodalVirial::write_header(bigint n)
{
...
void DumpCACNodalVirial::pack(tagint *ids)
{
  int m,n;
...
int DumpCACNodalVirial::convert_string(int n, double *mybuf)
{
  int offset = 0;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCACNodalVirial::write_data(int n, double *mybuf)
{
...
void DumpCACNodalVirial::write_string(int n, double *mybuf)
{
...
void DumpCACNodalVirial::write_lines(int n, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,format,
