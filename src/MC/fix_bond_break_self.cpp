    ...
    broken(nullptr), copy(nullptr), random(nullptr)
{
    ...
    if (nevery <= 0) error->all(FLERR,"Illegal fix bond/break/self command");
    ...
    if (btype < 1 || btype > atom->nbondtypes)
        error->all(FLERR,"Invalid bond type in fix bond/break/self command");
    ...
    while (iarg < narg) {
        if (strcmp(arg[iarg],"prob") == 0) {
            ...
            if (fraction < 0.0 || fraction > 1.0)
                error->all(FLERR,"Illegal fix bond/break/self command");
            if (seed <= 0) error->all(FLERR,"Illegal fix bond/break/self command");
            iarg += 3;
        } else error->all(FLERR,"Illegal fix bond/break/self command");
    }

    // error check

    if (atom->molecular != Atom::MOLECULAR)
        error->all(FLERR,"Cannot use fix bond/break/self with non-molecular systems");
...
FixBondBreakSelf::~FixBondBreakSelf()
{
    delete random;
...
void FixBondBreakSelf::init()
{
    ...
    if (force->improper) {
        if (force->improper_match("^class2") || force->improper_match("^ring"))
            error->all(FLERR,"Cannot yet use fix bond/break/self with this improper style");
    }
...
void FixBondBreakSelf::post_integrate()
{
    ...
    for (i = 0; i < nall; i++) {
        ...
        if (j < 0 || tag[i] < tag[j]) {
            if (nbreak == maxbreak) {
                maxbreak += DELTA;
                memory->grow(broken,maxbreak,2,"bond/break/self:broken");
            }
...
void FixBondBreakSelf::check_ghosts()
{
    int i,j,n;
    tagint *slist;
    ...
    if (flagall)
        error->all(FLERR,"Fix bond/break/self needs ghost atoms from further away");
...
void FixBondBreakSelf::update_topology()
{
...
void FixBondBreakSelf::rebuild_special_one(int m)
{
...
void FixBondBreakSelf::break_angles(int m, tagint id1, tagint id2)
{
    int j,found;
...
void FixBondBreakSelf::break_dihedrals(int m, tagint id1, tagint id2)
{
    int j,found;
...
void FixBondBreakSelf::break_impropers(int m, tagint id1, tagint id2)
{
    int j,found;
...
int FixBondBreakSelf::dedup(int nstart, int nstop, tagint *copy)
{
    int i;
...
void FixBondBreakSelf::post_integrate_respa(int ilevel, int /*iloop*/)
{
...
int FixBondBreakSelf::pack_forward_comm(int n, int *list, double *buf,
...
void FixBondBreakSelf::unpack_forward_comm(int n, int first, double *buf)
{
    int i,j,m,ns,last;
...
int FixBondBreakSelf::pack_reverse_comm(int n, int first, double *buf)
{
    int i,m,last;
...
void FixBondBreakSelf::unpack_reverse_comm(int n, int *list, double *buf)
{
    int i,j,m;
...
double FixBondBreakSelf::compute_vector(int n)
{
...
double FixBondBreakSelf::memory_usage()
{
    int nmax = atom->nmax;
