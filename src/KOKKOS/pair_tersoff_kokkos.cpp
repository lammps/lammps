...
void PairTersoffKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  ...
  if (neighflag == HALF) {
    ...
    if (evflag)
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagPairTersoffComputeFullA<FULL,1>, Kokkos::LaunchBounds<256, 1> >(0,inum),*this,ev);
    else
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagPairTersoffComputeFullA<FULL,0>, Kokkos::LaunchBounds<256, 1> >(0,inum),*this);
    ev_all += ev;

    if (evflag)
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagPairTersoffComputeFullB<FULL,1>, Kokkos::LaunchBounds<256, 1> >(0,ignum),*this,ev);
    else
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagPairTersoffComputeFullB<FULL,0>, Kokkos::LaunchBounds<256, 1> >(0,ignum),*this);
    ev_all += ev;
  }
