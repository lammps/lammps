...
/* ----------------------------------------------------------------------
   the per-atom virial of a angle style has to add up to its global
   ...
   branches of the angle styles and of their accelerated variants are
...
// the units of Angle::virial before it can be compared with it
...
TEST(AngleStyle, vatom_only_omp)
{
    if (!Info::has_package("OPENMP")) GTEST_SKIP();
    if (test_config.skip_tests.count(test_info_->name())) GTEST_SKIP();
...
TEST(AngleStyle, vatom_only_kokkos)
{
    ...
    LAMMPS::argv args = {"AngleStyle", "-log", "none", "-echo", "screen", "-nocite",
                         "-k", "on", "t", "1", "-sf", "kk"};
    if (kk_gpu)
        args = {"AngleStyle", "-log", "none", "-echo", "screen", "-nocite",
                "-k", "on", "g", "1", "-sf", "kk",
