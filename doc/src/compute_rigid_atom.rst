.. index:: compute rigid/atom

compute rigid/atom command
===========================
...
.. code-block:: LAMMPS
   ...
   compute ID group-ID rigid/atom rigidID input1 input2 ...

* ID, group-ID are documented in :doc:`compute <compute>` command
* rigid/atom = style name of this compute command
...
.. code-block:: LAMMPS
   ...
   compute 1 all rigid/atom myRigid mol x y z
...
atom in a particular body will store the same data.  This can be
useful when creating a dump file for post-analyses or visualizations,
e.g. to color each atom based on a property of the rigid body it
belongs to.  See the :doc:`compute rigid/local <compute_rigid_local>`
command for how to output only one copy of each rigid body's
information.

Note that this command only works with the :doc:`fix rigid/small
<fix_rigid>` command or its variants, not the fix rigid command and
its variants.  The ID of the :doc:`fix rigid/small <fix_rigid>`
command used to define rigid bodies must be specified as *rigidID*\ .
The :doc:`fix rigid <fix_rigid>` command is typically used to define a
handful of (potentially very large) rigid bodies.  It outputs similar
per-body information as this command directly from the fix as global
data; see the :doc:`fix rigid <fix_rigid>` page for details

The values generated by this compute will be 0.0 for atoms not in the
specified compute group *group-ID*\ or not in a rigid body.

Here is an example of how to use this compute to dump rigid body info
with each atom to a file:
...
The *id* attribute is the atom-ID of the atom which owns the rigid
body, which is assigned by the :doc:`fix rigid/small <fix_rigid>`
command.
...
(:math:`\AA`, :math:`\sigma`, etc).  Use *xu*, *yu*, *zu* if you want
the COM "unwrapped" by the image flags for each body.  Unwrapped means
that if the body COM has passed through a periodic boundary one or
more times, the value is generated what the COM coordinate would be if
it had not been wrapped back into the periodic box.
...
box lengths to get the true value.  A value of :math:`-1` means
subtract 1 box length to get the true value.  LAMMPS updates these
...
This compute calculates a per-atom vector or array depending on the
number of keywords. If a single keyword is specified, a per-atom
vector is produced.  If two or more keywords are specified, a per-atom
array is produced where the number of columns = the number of
keywords.  The vector or array can be accessed by any command that
uses per-atom values from a compute as input.  See the :doc:`Howto
...
:doc:`compute rigid/local <compute_rigid_local>`
