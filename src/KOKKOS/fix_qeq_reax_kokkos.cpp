...
// b = b_t, x = t;
{
  ...
  if (neighflag != FULL) {
    ...
    #ifdef HIP_OPT_SPMV
    int teamsize = 1024/64;//LG need to use some Kokkos function to get WARP_SIZE (max vector size)
    Kokkos::parallel_for(Kokkos::TeamPolicy <DeviceType, TagSparseMatvec3> ((inum+teamsize-1)/teamsize, teamsize, 64), *this);
    #else
