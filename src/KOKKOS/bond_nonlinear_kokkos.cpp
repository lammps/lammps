...
#include "bond_nonlinear_kokkos.h"
...
BondNonlinearKokkos<DeviceType>::BondNonlinearKokkos(LAMMPS *lmp) : BondNonlinear(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondNonlinearKokkos<DeviceType>::~BondNonlinearKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondNonlinearKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  k_lamda.template sync<DeviceType>();
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondNonlinearCompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondNonlinearCompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondNonlinearCompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondNonlinearCompute<0,0> >(0,nbondlist),*this);
    }
  }
...
void BondNonlinearKokkos<DeviceType>::operator()(TagBondNonlinearCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT denom = lamdasq - drsq;
  const KK_FLOAT denomsq = denom*denom;

  // force & energy

  KK_FLOAT fbond = static_cast<KK_FLOAT>(0.0);
  if (r > static_cast<KK_FLOAT>(0.0))
    fbond = -d_epsilon[type]/r * static_cast<KK_FLOAT>(2.0)*dr*lamdasq/denomsq;

  KK_FLOAT ebond = static_cast<KK_FLOAT>(0.0);
  if (eflag)
    ebond = d_epsilon[type] * drsq / denom;
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondNonlinearKokkos<DeviceType>::operator()(TagBondNonlinearCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondNonlinearCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondNonlinearKokkos<DeviceType>::allocate()
{
  BondNonlinear::allocate();

  int n = atom->nbondtypes;
  k_epsilon = DAT::tdual_kkfloat_1d("BondNonlinear::epsilon",n+1);
  k_r0 = DAT::tdual_kkfloat_1d("BondNonlinear::r0",n+1);
  k_lamda = DAT::tdual_kkfloat_1d("BondNonlinear::lamda",n+1);

  d_epsilon = k_epsilon.template view<DeviceType>();
  d_r0 = k_r0.template view<DeviceType>();
  d_lamda = k_lamda.template view<DeviceType>();
}
...
template<class DeviceType>
void BondNonlinearKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondNonlinear::coeff(narg, arg);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    k_lamda.view_host()[i] = static_cast<KK_FLOAT>(lamda[i]);
  }
...
template<class DeviceType>
void BondNonlinearKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondNonlinear::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondNonlinearKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
...
namespace LAMMPS_NS {
template class BondNonlinearKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class BondNonlinearKokkos<LMPHostType>;
#endif
}
