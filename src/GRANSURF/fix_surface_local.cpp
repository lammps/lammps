...
void FixSurfaceLocal::post_constructor()
{
  ...
  if (ninput) {
    ...
    if (dimension == 2) {
      ...
      int nedges = connectivity3d_global(npoints, ntris, tris, connect3dall, neigh_e1, neigh_e2,
                                         neigh_e3, neigh_c1, neigh_c2, neigh_c3);
      assign3d();
    }
...
void FixSurfaceLocal::setup(int vflag)
{
  ...
  if (dimension == 2) {
    ...
    tagint itag;
    int m, iline, jline, np1, np2, j, imotion, jmotion;
    ecount = 0;
    ...
    tagint itag;
    int m, itri, jtri, ne1, ne2, ne3, nc1, nc2, nc3, j, imotion, jmotion;
    ecount = 0;
...
void FixSurfaceLocal::connectivity2d_local()
{
  ...
  for (i = 0; i < nlocal_connect; i++) p1_counts[i] = p2_counts[i] = 0;
...
void FixSurfaceLocal::connectivity3d_local()
{
  ...
  for (i = 0; i < nlocal_connect; i++) n1_counts[i] = n2_counts[i] = n3_counts[i] = 0;
  ...
  for (i = 0; i < nlocal_connect; i++) connect3d[i].ne1 = connect3d[i].ne2 = connect3d[i].ne3 = 0;
  ...
  for (i = 0; i < nlocal_connect; i++) connect3d[i].nc1 = connect3d[i].nc2 = connect3d[i].nc3 = 0;
...
int FixSurfaceLocal::overlap_bins_2d(double *pt, double eps, int *indices)
{
  ...
  int n = 0;
  for (j = jlo; j <= jhi; j++)
    for (i = ilo; i <= ihi; i++) indices[n++] = j * nbinx + i;
  return n;
}
...
int FixSurfaceLocal::overlap_bins_3d(double *pt, double eps, int *indices)
{
  ...
  for (k = klo; k <= khi; k++)
    for (j = jlo; j <= jhi; j++)
      for (i = ilo; i <= ihi; i++) indices[n++] = k * nbiny * nbinx + j * nbinx + i;
  return n;
}
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    for (m = 0; m < connect2d[iconnect].np1; m++) {
      ...
      if (fabs(dotnorm) > 1.0 - flatthresh)
        connect2d[iconnect].fflag_p1[m] = FLAT;
      else
        connect2d[iconnect].fflag_p1[m] = NONFLAT;
      ...
      if (same_point(endpts[iconnect][0], endpts[jconnect][0])) {
        ...
        if (icrossj[2] > 0.0)
          connect2d[iconnect].aflag_p1[m] = CONCAVE;
        else
          connect2d[iconnect].aflag_p1[m] = CONVEX;
    ...
    for (m = 0; m < connect2d[iconnect].np2; m++) {
      ...
      dotnorm = MathExtra::dot3(inorm, jnorm);
      if (fabs(dotnorm) > 1.0 - flatthresh)
        connect2d[iconnect].fflag_p2[m] = FLAT;
      else
        connect2d[iconnect].fflag_p2[m] = NONFLAT;
      ...
      if (same_point(endpts[iconnect][1], endpts[jconnect][0])) {
        ...
        if (icrossj[2] > 0.0)
          connect2d[iconnect].aflag_p2[m] = CONCAVE;
        else
          connect2d[iconnect].aflag_p2[m] = CONVEX;
...
void FixSurfaceLocal::connectivity3d_complete()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    for (m = 0; m < connect3d[iconnect].ne1; m++) {
      ...
      dotnorm = MathExtra::dot3(inorm, jnorm);
      if (fabs(dotnorm) > 1.0 - flatthresh)
        connect3d[iconnect].fflag_e1[m] = FLAT;
      else
        connect3d[iconnect].fflag_e1[m] = NONFLAT;
      ...
      if ((jpfirst == 1 && jpsecond == 2) || (jpfirst == 2 && jpsecond == 3) ||
        ...
        if (jpfirst == 2)
          connect3d[iconnect].ewhich_e1[m] = 0;
        else if (jpfirst == 3)
          connect3d[iconnect].ewhich_e1[m] = 1;
        else if (jpfirst == 1)
          connect3d[iconnect].ewhich_e1[m] = 2;
    ...
    for (m = 0; m < connect3d[iconnect].ne2; m++) {
      ...
      dotnorm = MathExtra::dot3(inorm, jnorm);
      if (fabs(dotnorm) > 1.0 - flatthresh)
        connect3d[iconnect].fflag_e2[m] = FLAT;
      else
        connect3d[iconnect].fflag_e2[m] = NONFLAT;
      ...
      if ((jpfirst == 1 && jpsecond == 2) || (jpfirst == 2 && jpsecond == 3) ||
        ...
        if (jpfirst == 2)
          connect3d[iconnect].ewhich_e2[m] = 0;
        else if (jpfirst == 3)
          connect3d[iconnect].ewhich_e2[m] = 1;
        else if (jpfirst == 1)
          connect3d[iconnect].ewhich_e2[m] = 2;
    ...
    for (m = 0; m < connect3d[iconnect].ne3; m++) {
      ...
      dotnorm = MathExtra::dot3(inorm, jnorm);
      if (fabs(dotnorm) > 1.0 - flatthresh)
        connect3d[iconnect].fflag_e3[m] = FLAT;
      else
        connect3d[iconnect].fflag_e3[m] = NONFLAT;
      ...
      if ((jpfirst == 1 && jpsecond == 2) || (jpfirst == 2 && jpsecond == 3) ||
        ...
        if (jpfirst == 2)
          connect3d[iconnect].ewhich_e3[m] = 0;
        else if (jpfirst == 3)
          connect3d[iconnect].ewhich_e3[m] = 1;
        else if (jpfirst == 1)
          connect3d[iconnect].ewhich_e3[m] = 2;
