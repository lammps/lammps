...
BondFENEnm::BondFENEnm(LAMMPS *lmp) : Bond(lmp)
{
...
// force from n-m term
// TWO_1_3 cutoff assumes sigma = 2^{1/6}
    if (rsq < TWO_1_3) {
        r = sqrt(rsq);
...
//        ebond += 4.0*epsilon[type]*sr6*(sr6-1.0) + epsilon[type];
      if (rsq < TWO_1_3) ebond += (epsilon[type]/(nn[type]-mm[type]))*(mm[type]*pow(sigma[type]/r,nn[type])-nn[type]*pow(sigma[type]/r,mm[type]));
    }
