...
PairGMUPINN::PairGMUPINN(LAMMPS *lmp) : Pair(lmp)
{
  ...
  deriv_wrt_big_a = NULL;
  deriv_wrt_alpha = NULL;
  deriv_wrt_big_b = NULL;
  deriv_wrt_beta = NULL;
  deriv_wrt_small_h = NULL;
  deriv_wrt_sigma = NULL;
  deriv_wrt_small_a = NULL;
  deriv_wrt_lambda = NULL;

}

// ----------------------------------------------------------------------

PairGMUPINN::~PairGMUPINN()
{
  if (copymode) return;
...
void PairGMUPINN::settings(int narg, char **/*arg*/)
{
  if (narg != 0) error->all(FLERR,"Illegal pair_style command");
}

void PairGMUPINN::coeff(int narg, char **arg)
{
  int i,j,n;
  ...
  //if (params) printf("Rank:%d -> allocated memory ! .... [%s:%s:%d]\n", comm->me,
  ...
  for (int i=0; i<nelements; i++) {
    for (int j=i; j<nelements; j++) {
      ...
      tmp++;
      //if (comm->me) printf(" [%d,%d] = %d", map_gi[i][j]);
    }
    //if (comm->me) printf("\n");
  }
  ...
  for (i = 3; i < narg; i++) {
    ...
    else error->all(FLERR,"No matching element in ANN potential file");
  }
...
void PairGMUPINN::setup_params()
{
  ...
  if (nelements <= 0) {
    ...
    error->all (FLERR, "Illegal # of unique elements");
  }
...
double PairGMUPINN::init_one(int i, int j)
{
...
void PairGMUPINN::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) { // loop over all neighbors of i
      ...
      dbij = pow(bij[jj], 3) * signof (1.0 + Zij[jj]);
      ...
      for (kk = 0; kk < knum; kk++) { // loop over all neighbors of i
        ...
        if (rsqik < params->cutsq) {
          ...
          fpair *= 0.5 * dbij * small_a[itype][jtype][ktype] * Sij[kk] *
              pow(CSijk[jj][kk] - small_h[itype][jtype][ktype], 2) * ann_fc_d (rik, params) / rik;
          ...
          for (ll = 0; ll < knum; ll++) {
            ...
            if (r < params->cut) {
              ...
              prefactor *= -0.5 * dbij * small_a[itype][jtype][ktype] *
                  pow(CSijk[jj][kk] - small_h[itype][jtype][ktype], 2) * ann_fc (rik, params);
      ...
      for (kk = 0; kk < knum; kk++) { // loop over all neighbors of i again
        ...
        for(int m = 0; m<params->nsigmas; m++) { // loop over r0s
          ...
          for(int l = 0; l<params->nLPOrders; l++) { // loop over Legendre polynomial orders
            ...
            double denom = sqrt(pow(sinh(gilist[gindx]), 2) + 1.0); // use the original Gis not the transformed here !!!
...
void PairGMUPINN::pack_derivs_wrt_bop_sets_to_vec(double *pvec, const int n)
{
  /* ------------------------------------------
...
void PairGMUPINN::force_Sijk_ri(double pref, double *dri1, double *dri2,
...
void PairGMUPINN::dcostheta_ijk_dri(double cstheta, double *drj,
...
void PairGMUPINN::force_costheta_ijk_ri(double pref, double cstheta, double *drj, double *drk,
...
double PairGMUPINN::dot_product(double *x, double *y, int n)
{
  double tmp = 0.0;
...
double PairGMUPINN::bop_fa(int it, int jt, double r)
{
  // only -f_A(r)
  return -exp(big_b[it][jt] - beta[it][jt] * r);
}

double PairGMUPINN::bop_fa_d(int it, int jt, Param *param, double r)
{
...
double PairGMUPINN::bop_zeta(int it, int jt, int kt, Param *param,
                             double s_ij, double cs_ijk, double r)
{
  double cut;

  cut = ann_fc (r, param);
  return s_ij * small_a[it][jt][kt] * cut * pow(cs_ijk - small_h[it][jt][kt], 2);
}

/* ---------------------------------------------------------------------- */

double PairGMUPINN::screen_ijk(int it, int jt, int kt, Param *param, double r)
{
...
double PairGMUPINN::bop_fr(int itype, int jtype, double r)
{
...
double PairGMUPINN::bop_fr_d(int itype, int jtype, Param *param, double r)
{
...
void PairGMUPINN::read_file(char *file)
{
  // open file on proc 0
  ...
  for (int i=0; i<params->nlayers; i++) {
    params->layers[i].Weights = NULL;
    params->layers[i].Biases = NULL;
    params->layers[i].fdot = NULL;
    if (!i) {
      //params->layers[i].vsum = memory->create(params->layers[i].vsum, params->layers[i].nnodes, "");
    } else {
      ...
      //params->layers[i].mvprod = memory->create(params->layers[i].mvprod, params->layers[i].nnodes, "");
      //params->layers[i].vsum = memory->create(params->layers[i].vsum, params->layers[i].nnodes, "");
  ...
  while (i < nwords - 1) {
    baseline_bo_params[i++] = atof(strtok(NULL," \t\n\r\f"));
    //if (comm->me) printf(" %e", baseline_bo_params[i-1]);
  }
  //if (comm->me) printf("\n");
  ...
  // only for debugging
  //if (comm->me == 0) write_potential ();
  //error->all(FLERR, "Just debugging !\n");
}

/* ---------------------------------------------------------------------- */
/*
void PairGMUPINN::write_potential ()
{
...
void PairGMUPINN::eval_nnet(double *input, int insize, double *output, int outsize)
{
  // sanity checks !!!
...
void PairGMUPINN::eval_nnet_d(Layer *aNNlayers, double *input,
...
void PairGMUPINN::compute_atomic_LSP(const int iam, double *gi)
{
  ...
  for (int jj = 0; jj < jnum; jj++) {    // loop over sites j
    ...
    for (int kk = 0; kk < jnum; kk++) {  // loop over sites k
      ...
      for (int n = 0; n < params->nsigmas; n++) { // loop over sigmas
        double tmp = exp(-pow((rij - params->sigmas[n]) / params->gwidth, 2))
            * ann_fc (rij, rc, params->cut_range);
        tmp *= exp(-pow((rik - params->sigmas[n]) / params->gwidth, 2))
            * ann_fc (rik, rc, params->cut_range) / pow(params->sigmas[n], 2);
...
double PairGMUPINN::ann_fc (double r, Param *par)
{
  double x, y, x4;
...
double PairGMUPINN::ann_fc_d (double r, Param *par)
{
  double x,y,x4;
  double x1, y2;
...
double PairGMUPINN::ann_fc (double r, double rc, double hc)
{
  double x, y, x4;
...
double PairGMUPINN::ann_fc_d (double r, double rc, double hc)
{
  double x,y,x4;
  double x1, y2;
...
double PairGMUPINN::ann_fs(double r, int s, Param *par)
{
  double tmpexp = exp(-pow((r - par->sigmas[s]) / par->gwidth, 2)) / par->sigmas[s];
...
double PairGMUPINN::pinn_fs(const double r, const int s)
{
  double tmpexp = exp(-pow((r - params->sigmas[s]) / params->gwidth, 2)) / params->sigmas[s];
...
double PairGMUPINN::ann_fs_d(double r, int s, Param *par)
{
  double tmp_exp = exp(-pow((r - par->sigmas[s]) / par->gwidth, 2)) / par->sigmas[s];
...
double PairGMUPINN::pinn_fs_d(const double r, const int s)
{
  double tmp_exp = exp(-pow((r - params->sigmas[s]) / params->gwidth, 2)) / params->sigmas[s];
...
void PairGMUPINN::unpack_vec_to_bop_sets(const double *pvec, const int nsize)
{
  /* ------------------------------------------
...
void PairGMUPINN::iderivs_wrt_bops(const int myID, const double ep)
{
  double xtmp, ytmp, ztmp;
  int j, k, jnum, itype, jtype, ktype;
  int *jlist, *numneigh, **firstneigh;
  ...
  for (int jj = 0; jj < jnum; jj++) {    // loop over neighbors of current atom
    ...
    double dbij = pow(bij[jj], 3) * signof (1.0 + Zij[jj]);
    ...
    for (int kk = 0; kk < jnum; ++kk) {
      ...
      if (rsqik < params->cutsq) {
        deriv_wrt_small_a[itype][jtype][ktype] += (prefactor_dzij_1 + prefactor_dzij_2) *
            Sij[kk] * pow(CSijk[jj][kk] - small_h[itype][jtype][ktype], 2) * ann_fc (rik, params);
