...
#include "pair_morse_smooth_linear_kokkos.h"
...
PairMorseSmoothLinearKokkos<DeviceType>::PairMorseSmoothLinearKokkos(LAMMPS *lmp) : PairMorseSmoothLinear(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairMorseSmoothLinearKokkos<DeviceType>::~PairMorseSmoothLinearKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairMorseSmoothLinearKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairMorseSmoothLinearKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairMorseSmoothLinearKokkos<DeviceType>::
compute_fpair(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const KK_FLOAT der_val = (STACKPARAMS?m_params[itype][jtype].der_at_cutoff:params(itype,jtype).der_at_cutoff);
  const KK_FLOAT dr = r - r0_val;
  const KK_FLOAT dexp = Kokkos::exp(-alpha_val * dr);

  return (morse1_val * (dexp*dexp - dexp) + der_val) / r;
}
...
compute_evdwl(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  return d0_val * (dexp*dexp - static_cast<KK_FLOAT>(2.0)*dexp) - offset_val
         - (r - cut_val) * der_val;
}
...
template<class DeviceType>
void PairMorseSmoothLinearKokkos<DeviceType>::allocate()
{
  PairMorseSmoothLinear::allocate();

  int n = atom->ntypes;
  memory->destroy(cutsq);
  memoryKK->create_kokkos(k_cutsq,cutsq,n+1,n+1,"pair:cutsq");
  d_cutsq = k_cutsq.template view<DeviceType>();
  k_params = Kokkos::DualView<params_morse_sl**,Kokkos::LayoutRight,DeviceType>("PairMorseSmoothLinear::params",n+1,n+1);
...
template<class DeviceType>
void PairMorseSmoothLinearKokkos<DeviceType>::init_style()
{
  PairMorseSmoothLinear::init_style();
...
template<class DeviceType>
double PairMorseSmoothLinearKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairMorseSmoothLinear::init_one(i,j);
  ...
  k_params.view_host()(i,j).der_at_cutoff = static_cast<KK_FLOAT>(der_at_cutoff[i][j]);
