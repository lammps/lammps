...
void NEB::run()
{
  ...
  // If running in equal spacing, perform additional NEB cycle of n2steps

  if (fneb->EqualForceNEB) {

    if (me_universe == 0) {
      if (uscreen)
        fprintf(uscreen,"Setting up equal force run....\n");
      if (ulogfile)
        fprintf(ulogfile,"Setting up equal force run....\n");
    }
