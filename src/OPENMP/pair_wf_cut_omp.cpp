...
PairWFCutOMP::PairWFCutOMP(LAMMPS *lmp) : PairWFCut(lmp), ThrOMP(lmp, THR_PAIR)
{
...
void PairWFCutOMP::compute(int eflag, int vflag)
{
  ev_init(eflag, vflag);
...
void PairWFCutOMP::eval(int iifrom, int iito, ThrData *const thr)
{
  ...
  for (ii = iifrom; ii < iito; ++ii) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (EFLAG) {
          evdwl = e0nm[itype][jtype] * (rm * powint(rn, 2 * nu[itype][jtype])) * factor_lj;
        }
...
double PairWFCutOMP::memory_usage()
{
  double bytes = memory_usage_thr();
  bytes += PairWFCut::memory_usage();
  return bytes;
}
