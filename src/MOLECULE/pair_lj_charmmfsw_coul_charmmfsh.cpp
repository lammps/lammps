...
void PairLJCharmmfswCoulCharmmfsh::write_data(FILE *fp, char ** type)
{
  for (int i = 1; i <= atom->ntypes; i++)
    fprintf(fp,"%s %g %g %g %g\n",
            type[i-1],epsilon[i][i],sigma[i][i],eps14[i][i],sigma14[i][i]);
}
...
void PairLJCharmmfswCoulCharmmfsh::write_data_all(FILE *fp, char **type)
{
  for (int i = 1; i <= atom->ntypes; i++)
