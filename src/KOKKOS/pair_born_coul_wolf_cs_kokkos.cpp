...
#include "pair_born_coul_wolf_cs_kokkos.h"
...
PairBornCoulWolfCSKokkos<DeviceType>::PairBornCoulWolfCSKokkos(LAMMPS *lmp)
  : PairBornCoulWolfCS(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairBornCoulWolfCSKokkos<DeviceType>::~PairBornCoulWolfCSKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairBornCoulWolfCSKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
...
KK_FLOAT PairBornCoulWolfCSKokkos<DeviceType>::
...
KOKKOS_INLINE_FUNCTION
KK_FLOAT PairBornCoulWolfCSKokkos<DeviceType>::
compute_fcoul(const KK_FLOAT& rsq_in, const int& /*i*/, const int& j,
...
KOKKOS_INLINE_FUNCTION
KK_FLOAT PairBornCoulWolfCSKokkos<DeviceType>::
compute_ecoul(const KK_FLOAT& rsq_in, const int& /*i*/, const int& j,
               const int& /*itype*/, const int& /*jtype*/,
               const KK_FLOAT& factor_coul, const KK_FLOAT& qtmp) const
{
  // r = 0 must stay finite here, exactly as in the CPU style, which adds
...
template<class DeviceType>
void PairBornCoulWolfCSKokkos<DeviceType>::allocate()
{
  PairBornCoulWolfCS::allocate();
  ...
  k_params = Kokkos::DualView<params_born_wolf**,Kokkos::LayoutRight,DeviceType>(
    "PairBornCoulWolfCS::params",n+1,n+1);
...
template<class DeviceType>
void PairBornCoulWolfCSKokkos<DeviceType>::init_style()
{
  PairBornCoulWolfCS::init_style();
...
template<class DeviceType>
double PairBornCoulWolfCSKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairBornCoulWolfCS::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairBornCoulWolfCSKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairBornCoulWolfCSKokkos<LMPHostType>;
#endif
}
