...
{
  if (narg != 3) error->all(FLERR, "Illegal compute smd/tlsph_strain command");
...
void ComputeSMDULSPHstrain::init()
{
  ...
  if ((comm->me == 0) && (modify->get_compute_by_style("^smd/ulsph/strain$").size() > 1))
...
{
  double **atom_data9 =
      atom->smd_data_9;    // ULSPH strain is stored in the first 6 entries of this data field
  ...
  if (atom->nmax > nmax) {
    ...
    nmax = atom->nmax;
    memory->create(strainVector, nmax, size_peratom_cols, "smd/ulsph/strain:strainVector");
