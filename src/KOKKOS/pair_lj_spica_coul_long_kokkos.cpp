...
compute_fpair(const KK_FLOAT& rsq, const int& i, const int&j, const int& itype, const int& jtype) const {
  ...
  const KK_FLOAT a = ljt==LJ12_4?r4inv:(ljt==LJ12_5?r4inv*Kokkos::sqrt(r2inv):r6inv);
  const KK_FLOAT b = ljt==LJ12_4?r4inv:(ljt==LJ9_6?static_cast<KK_FLOAT>(1.0)/Kokkos::sqrt(r2inv):(ljt==LJ12_5?r2inv*Kokkos::sqrt(r2inv):r2inv));
...
double PairLJSPICACoulLongKokkos<DeviceType>::init_one(int i, int j)
{
  ...
  k_params.view_host()(i,j).cut_ljsq = static_cast<KK_FLOAT>(cut_ljsq[i][j]);
  ...
  if (i<MAX_TYPES_STACKPARAMS+1 && j<MAX_TYPES_STACKPARAMS+1) {
    ...
    m_cut_ljsq[j][i] = m_cut_ljsq[i][j] = static_cast<KK_FLOAT>(cut_ljsq[i][j]);
    m_cut_coulsq[j][i] = m_cut_coulsq[i][j] = static_cast<KK_FLOAT>(cut_coulsq);
  }

  k_cutsq.view_host()(i,j) = k_cutsq.view_host()(j,i) = cutone*cutone;
  k_cut_ljsq.view_host()(i,j) = k_cut_ljsq.view_host()(j,i) = cut_ljsq[i][j];
