...
of MPI processes you have defined via the :doc:`-partition command-line
switch <Run_options>`.
...
variable is used in a :doc:`jump <jump>` command.

The "partition" command is another mechanism for having as input script
operate differently on different partitions.  It is basically a prefix
on any LAMMPS command.  The command will only be invoked on the
...
partitions specified by the :doc:`-partition command-line switch
<Run_options>`.
...
partitions from 1 to *Np*.  A leading asterisk means all partitions from
1 to n (inclusive).  A trailing asterisk means all partitions from n to
*Np* (inclusive).  A middle asterisk means all partitions from m to n
(inclusive).
...
command lays out a 3d grid of MPI processes in each of 2 partitions.
