...
PairLJCutTIP4PCutKokkos<DeviceType>::PairLJCutTIP4PCutKokkos(LAMMPS *lmp) : PairLJCutTIP4PCut(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
PairLJCutTIP4PCutKokkos<DeviceType>::~PairLJCutTIP4PCutKokkos()
{
  if (copymode) return;
  if (allocated) {
...
template<class DeviceType>
void PairLJCutTIP4PCutKokkos<DeviceType>::init_style()
{
  PairLJCutTIP4PCut::init_style();
...
void PairLJCutTIP4PCutKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  ...
  // copy per-type-pair LJ coefficients to device (once; small)
  ...
  if ((int)d_newsite.extent(0) < atom->nmax) {
    d_newsite = typename AT::t_kkfloat_1d_3("lj/cut/tip4p/cut/kk:newsite", atom->nmax);
    d_hneigh  = typename AT::t_int_1d_3("lj/cut/tip4p/cut/kk:hneigh", atom->nmax);
  }
  ...
  Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,TagPairLJCutTIP4PCutNewsite>(0,nall), *this);
...
int PairLJCutTIP4PCutKokkos<DeviceType>::closest_image(const int i, int j) const
{
  if (j < 0) return j;
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void PairLJCutTIP4PCutKokkos<DeviceType>::operator()(TagPairLJCutTIP4PCutNewsite, const int &i) const
{
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void PairLJCutTIP4PCutKokkos<DeviceType>::ev_tally_tip4p(EV_FLOAT &ev, const int &key,
