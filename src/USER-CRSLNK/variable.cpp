...
void Variable::set(int narg, char **arg) {
	...
	if (strcmp(arg[1], "delete") == 0) {
		...
		if (flag)
			error->all(FLERR, "File variable could not read value");
		...
		if (flag)
			error->all(FLERR, "Atomfile variable could not read values");
...
PriorityList* Variable::get_type_detector(char* id) {
	PriorityList* detector = NULL;	
	for (std::vector<PriorityList*>::iterator it = this->type_dectors.begin();
...
void Variable::add_type_detector(CoordinateType type, int narg, char** args) {

	PriorityList* priority = new PriorityList(args[0], type);
...
int Variable::next(int narg, char **arg) {
	...
	if (narg == 0)
		error->all(FLERR, "Illegal next command");
...
// exception: UNIVERSE and ULOOP variables can be mixed in same next command
	...
	for (int iarg = 0; iarg < narg; iarg++) {
		...
		if (style[ivar] == ULOOP && style[find(arg[0])] == UNIVERSE)
			continue;
		else if (style[ivar] == UNIVERSE && style[find(arg[0])] == ULOOP)
			continue;
...
// if istyle = UNIVERSE or ULOOP, insure all such variables are incremented
	...
	if (istyle == UNIVERSE || istyle == ULOOP)
		for (int i = 0; i < nvar; i++) {
			if (style[i] != UNIVERSE && style[i] != ULOOP)
				continue;
			int iarg = 0;
			for (iarg = 0; iarg < narg; iarg++)
				if (strcmp(arg[iarg], names[i]) == 0)
					break;
			if (iarg == narg)
...
// if any variable is exhausted, set flag = 1 and remove var to allow re-use
	...
	if (istyle == INDEX || istyle == LOOP) {
		...
		if (me == 0) {
			...
			usleep(delay);
			while (1) {
				if (!rename("tmp.lammps.variable", "tmp.lammps.variable.lock"))
					break;
...
int Variable::atomstyle(int ivar) {
	if (style[ivar] == ATOM || style[ivar] == ATOMFILE)
		return 1;
	return 0;
}
...
char *Variable::pythonstyle(char *name, char *funcname) {
	...
	if (style[ivar] != PYTHON)
		return NULL;
	if (strcmp(data[ivar][0], funcname) != 0)
		return NULL;
	return data[ivar][1];
}
...
char *Variable::retrieve(char *name) {
	...
	char *str = NULL;
	if (style[ivar] == INDEX || style[ivar] == WORLD || style[ivar] == UNIVERSE
			|| style[ivar] == STRING || style[ivar] == SCALARFILE) {
		str = data[ivar][which[ivar]];
	} else if (style[ivar] == LOOP || style[ivar] == ULOOP) {
		char result[16];
		if (pad[ivar] == 0)
			sprintf(result, "%d", which[ivar] + 1);
		else {
			char padstr[16];
...
double Variable::compute_equal(int ivar) {
	...
	double value = 0.0;
	if (style[ivar] == EQUAL)
		value = evaluate(data[ivar][0], NULL);
	else if (style[ivar] == PYTHON) {
		int ifunc = python->find(data[ivar][0]);
		if (ifunc < 0)
			error->all(FLERR, "Python variable has no function");
		python->invoke_function(ifunc, data[ivar][1]);
...
void Variable::compute_atom(int ivar, int igroup, double *result, int stride,
		int sumflag) {
	Tree *tree;
	double *vstore;
	...
	if (style[ivar] == ATOM) {
		...
		vstore = reader[ivar]->fixstore->vstore;
...
int Variable::compute_vector(int ivar, double **result) {
	...
	if (nlen == 0)
		error->all(FLERR, "Vector-style variable has zero length");
	if (nlen < 0)
		error->all(FLERR, "Inconsistent lengths in vector-style variable");
...
void Variable::grow() {
	int old = maxvar;
	maxvar += VARDELTA;
	names = (char **) memory->srealloc(names, maxvar * sizeof(char *),
			"var:names");
	...
	reader = (VarReader **) memory->srealloc(reader,
			maxvar * sizeof(VarReader *), "var:reader");
	for (int i = old; i < maxvar; i++)
		reader[i] = NULL;

	data = (char ***) memory->srealloc(data, maxvar * sizeof(char **),
			"var:data");

	vecs = (VecVar *) memory->srealloc(vecs, maxvar * sizeof(VecVar),
			"var:vecvar");
	for (int i = old; i < maxvar; i++) {
		vecs[i].nmax = 0;
	...
	while (1) {
		...
		// ----------------
			...
			if (tree) {
				...
				argstack[nargstack++] = evaluate(contents, NULL);
			...
			if (strncmp(word, "c_", 2) == 0 || strncmp(word, "C_", 2) == 0) {
				...
				if (nbracket == 0 && compute->scalar_flag && lowercase) {
					...
					if (update->whichflag == 0) {
						if (compute->invoked_scalar != update->ntimestep)
							error->all(FLERR,
									"Compute used in variable between runs "
											"is not current");
					...
					if (compute->size_array_rows_variable
							&& index1 > compute->size_array_rows)
						value1 = 0.0;
					else
					...
					if (compute->size_vector == 0)
						error->all(FLERR,
								"Variable formula compute vector is zero length");
					if (update->whichflag == 0) {
						if (compute->invoked_vector != update->ntimestep)
							error->all(FLERR,
									"Compute used in variable between runs "
					...
					if (compute->size_array_rows == 0)
						error->all(FLERR,
								"Variable formula compute array is zero length");
					if (update->whichflag == 0) {
						if (compute->invoked_array != update->ntimestep)
							error->all(FLERR,
									"Compute used in variable between runs "
					...
					if (compute->array_atom)
						peratom2global(1, NULL,
								&compute->array_atom[0][index2 - 1],
								compute->size_peratom_cols, index1, tree,
					...
					error->all(FLERR, "Mismatched compute in variable formula");
				...
				if (nbracket == 0 && fix->scalar_flag && lowercase) {
					...
					if (update->whichflag > 0
							&& update->ntimestep % fix->global_freq)
						error->all(FLERR,
					...
					if (fix->size_vector == 0)
						error->all(FLERR,
								"Variable formula fix vector is zero length");
					...
					if (fix->size_array_rows == 0)
						error->all(FLERR,
								"Variable formula fix array is zero length");
					...
					peratom2global(1, NULL, fix->vector_atom, 1, index1, tree,
							treestack, ntreestack, argstack, nargstack);
					...
					if (fix->array_atom)
						peratom2global(1, NULL, &fix->array_atom[0][index2 - 1],
								fix->size_peratom_cols, index1, tree, treestack,
								ntreestack, argstack, nargstack);
					else
						peratom2global(1, NULL, NULL, fix->size_peratom_cols,
								index1, tree, treestack, ntreestack, argstack,
								nargstack);
					...
					error->all(FLERR, "Mismatched fix in variable formula");
				...
				if (nbracket == 0 && style[ivar] != ATOM
					...
					peratom2global(1, NULL, result, 1, index, tree, treestack,
							ntreestack, argstack, nargstack);
					...
					peratom2global(1, NULL, reader[ivar]->fixstore->vstore, 1,
							index, tree, treestack, ntreestack, argstack,
							nargstack);
					...
					if (index <= 0 || index > nvec)
						error->all(FLERR,
								"Invalid index into vector-style variable");
				...
				if (str[i] == '(') {
					...
					if (math_function(word, contents, tree, treestack,
							ntreestack, argstack, nargstack))
						;
					else if (group_function(word, contents, tree, treestack,
							ntreestack, argstack, nargstack))
						;
					else if (special_function(word, contents, tree, treestack,
							ntreestack, argstack, nargstack))
						;
					else
						error->all(FLERR, "Invalid math/group/special function "
								"in variable formula");
					delete[] contents;
					...
					peratom2global(0, word, NULL, 0, id, tree, treestack,
							ntreestack, argstack, nargstack);
			...
			while (nopstack
					&& precedence[opstack[nopstack - 1]] >= precedence[op]) {
...
double Variable::collapse_tree(Tree *tree) {
	...
	if (tree->type == VALUE)
		return tree->value;
	if (tree->type == ATOMARRAY)
		return 0.0;
	if (tree->type == TYPEARRAY)
		return 0.0;
	if (tree->type == INTARRAY)
		return 0.0;
	if (tree->type == BIGINTARRAY)
		return 0.0;
	if (tree->type == VECTORARRAY)
		return 0.0;
...
// random() or normal() do not become a single collapsed value
	...
	if (tree->type == STAGGER) {
		...
		if (delta < ivalue2)
			tree->value = lower + ivalue2;
		else
			tree->value = lower + ivalue1;
		return tree->value;
	}
	...
	if (tree->type == LOGFREQ) {
		...
		if (tree->first->type != VALUE || tree->second->type != VALUE
				|| tree->extra[0]->type != VALUE)
			return 0.0;
		tree->type = VALUE;
		if (ivalue1 <= 0 || ivalue2 <= 0 || ivalue3 <= 0 || ivalue2 >= ivalue3)
			error->one(FLERR, "Invalid math function in variable formula");
		if (update->ntimestep < ivalue1)
			tree->value = ivalue1;
		else {
			int lower = ivalue1;
			while (update->ntimestep >= ivalue3 * lower)
				lower *= ivalue3;
			int multiple = update->ntimestep / lower;
			if (multiple < ivalue2)
				tree->value = (multiple + 1) * lower;
			else
				tree->value = lower * ivalue3;
		}
		return tree->value;
	}
	...
	if (tree->type == STRIDE) {
		...
		if (update->ntimestep < ivalue1)
			...
			if (tree->value > ivalue2)
				tree->value = MAXBIGINT;
		} else
			tree->value = MAXBIGINT;
		return tree->value;
	}
	...
	if (tree->type == STRIDE2) {
		...
		if (tree->first->type != VALUE || tree->second->type != VALUE
				|| tree->extra[0]->type != VALUE
				|| tree->extra[1]->type != VALUE
				|| tree->extra[2]->type != VALUE
				|| tree->extra[3]->type != VALUE)
			return 0.0;
		tree->type = VALUE;
		if (ivalue1 < 0 || ivalue2 < 0 || ivalue3 <= 0 || ivalue1 > ivalue2)
			error->one(FLERR, "Invalid math function in variable formula");
...
double Variable::eval_tree(Tree *tree, int i) {
	...
	if (tree->type == VALUE)
		return tree->value;
	if (tree->type == ATOMARRAY)
		return tree->array[i * tree->nstride];
	if (tree->type == TYPEARRAY)
		return tree->array[atom->type[i]];
	if (tree->type == INTARRAY)
		return (double) tree->iarray[i * tree->nstride];
	if (tree->type == BIGINTARRAY)
		return (double) tree->barray[i * tree->nstride];
	if (tree->type == VECTORARRAY)
		return tree->array[i * tree->nstride];
	...
	if (tree->type == EQ) {
		if (eval_tree(tree->first, i) == eval_tree(tree->second, i))
			return 1.0;
		else
			return 0.0;
	}
	...
	if (tree->type == RMASK) {
		if (domain->regions[tree->ivalue1]->match(atom->x[i][0], atom->x[i][1],
				atom->x[i][2]))
			return 1.0;
		else
			return 0.0;
...
int Variable::size_tree_vector(Tree *tree) {
	...
	if (tree->type == VECTORARRAY)
		nsize = tree->nvector;
	if (tree->first)
		nsize = compare_tree_vector(nsize, size_tree_vector(tree->first));
	if (tree->second)
		nsize = compare_tree_vector(nsize, size_tree_vector(tree->second));
	if (tree->nextra) {
		for (int i = 0; i < tree->nextra; i++)
			nsize = compare_tree_vector(nsize,
					size_tree_vector(tree->extra[i]));
	}
	return nsize;
}
...
void Variable::free_tree(Tree *tree) {
	...
	if (tree->second)
		free_tree(tree->second);
	if (tree->nextra) {
		for (int i = 0; i < tree->nextra; i++)
			free_tree(tree->extra[i]);
		delete[] tree->extra;
	}
...
tagint Variable::int_between_brackets(char *&ptr, int varallow) {
	...
	if (*ptr != ']')
		error->all(FLERR, "Mismatched brackets in variable");
	if (ptr == start)
		error->all(FLERR, "Empty brackets in variable");
...
// word not a match to any math function
	...
	if (strcmp(word, "sqrt") && strcmp(word, "exp") && strcmp(word, "ln")
			&& strcmp(word, "log") && strcmp(word, "abs") && strcmp(word, "sin")
			&& strcmp(word, "cos") && strcmp(word, "tan")
			&& strcmp(word, "asin") && strcmp(word, "acos")
			&& strcmp(word, "atan") && strcmp(word, "atan2")
			&& strcmp(word, "random") && strcmp(word, "normal")
			&& strcmp(word, "ceil") && strcmp(word, "floor")
			&& strcmp(word, "round") && strcmp(word, "ramp")
			&& strcmp(word, "stagger") && strcmp(word, "logfreq")
			&& strcmp(word, "logfreq2") && strcmp(word, "stride")
...
// customize by adding a function
	...
	if (strcmp(word, "sqrt") == 0) {
		...
		if (tree)
			...
			argstack[nargstack++] = fabs(value1);
		...
		if (tree)
			...
			argstack[nargstack++] = randomequal->uniform() * (value2 - value1)
					+ value1;
		}
		...
		if (tree)
			...
			if (update->ntimestep < ivalue1)
				...
				if (update->ntimestep < ivalue4
						|| update->ntimestep > ivalue5) {
					int offset = update->ntimestep - ivalue1;
					istep = ivalue1 + (offset / ivalue3) * ivalue3 + ivalue3;
					if (update->ntimestep < ivalue4 && istep > ivalue4)
						istep = ivalue4;
				} else {
					int offset = update->ntimestep - ivalue4;
					istep = ivalue4 + (offset / ivalue6) * ivalue6 + ivalue6;
					if (istep > ivalue5) {
						int offset = ivalue5 - ivalue1;
						istep = ivalue1 + (offset / ivalue3) * ivalue3
								+ ivalue3;
						if (istep > ivalue2)
							istep = MAXBIGINT;
					}
		...
		if (tree)
			...
			double omega = 2.0 * MY_PI / values[0];
			double value = value1
					+ value2 * (1.0 - cos(omega * delta * update->dt));
			argstack[nargstack++] = value;
		}
	}
...
// word not a match to any group function
	...
	if (strcmp(word, "count") && strcmp(word, "mass") && strcmp(word, "charge")
			&& strcmp(word, "xcm") && strcmp(word, "vcm") && strcmp(word, "fcm")
...
// match word to group function
	...
	if (strcmp(word, "count") == 0) {
		if (narg == 1)
			...
			value = group->count(igroup, region_function(args[1]));
		else
			error->all(FLERR, "Invalid group function in variable formula");

	} else if (strcmp(word, "mass") == 0) {
		if (narg == 1)
			value = group->mass(igroup);
		else if (narg == 2)
			value = group->mass(igroup, region_function(args[1]));
		else
			error->all(FLERR, "Invalid group function in variable formula");
		...
		if (narg == 1)
			...
			value = group->charge(igroup, region_function(args[1]));
		else
			error->all(FLERR, "Invalid group function in variable formula");
		...
		if (narg == 2)
			...
			group->fcm(igroup, fcm, region_function(args[2]));
		else
			error->all(FLERR, "Invalid group function in variable formula");
		if (strcmp(args[1], "x") == 0)
			value = fcm[0];
		...
		if (narg == 2)
			...
			group->bounds(igroup, minmax, region_function(args[2]));
		else
			error->all(FLERR, "Invalid group function in variable formula");
		if (strcmp(args[1], "xmin") == 0)
			value = minmax[0];
		else if (strcmp(args[1], "xmax") == 0)
			value = minmax[1];
		else if (strcmp(args[1], "ymin") == 0)
			value = minmax[2];
		else if (strcmp(args[1], "ymax") == 0)
			value = minmax[3];
		else if (strcmp(args[1], "zmin") == 0)
			value = minmax[4];
		else if (strcmp(args[1], "zmax") == 0)
			value = minmax[5];
		else
			error->all(FLERR, "Invalid group function in variable formula");
		...
		if (narg == 1)
			value = group->ke(igroup);
		else if (narg == 2)
			value = group->ke(igroup, region_function(args[1]));
		else
			error->all(FLERR, "Invalid group function in variable formula");
		...
		if (narg == 2) {
			...
			error->all(FLERR, "Invalid group function in variable formula");
		if (strcmp(args[1], "xx") == 0)
			value = inertia[0][0];
		else if (strcmp(args[1], "yy") == 0)
			value = inertia[1][1];
		else if (strcmp(args[1], "zz") == 0)
			value = inertia[2][2];
		else if (strcmp(args[1], "xy") == 0)
			value = inertia[0][1];
		else if (strcmp(args[1], "yz") == 0)
			value = inertia[1][2];
		else if (strcmp(args[1], "xz") == 0)
			value = inertia[0][2];
		else
			error->all(FLERR, "Invalid group function in variable formula");
...
// word not a match to any special function
	...
	if (strcmp(word, "sum") && strcmp(word, "min") && strcmp(word, "max")
			...
			&& strcmp(word, "slope") && strcmp(word, "gmask")
			&& strcmp(word, "rmask") && strcmp(word, "grmask")
			&& strcmp(word, "next") && strcmp(word, "is_active")
			&& strcmp(word, "is_defined") && strcmp(word, "is_available"))
		return 0;
...
// special functions that operate on global vectors
	...
	if (strcmp(word, "sum") == 0 || strcmp(word, "min") == 0
		...
		if (strcmp(word, "sum") == 0)
			...
			method = XMAX;
		else if (strcmp(word, "ave") == 0)
			method = AVE;
		else if (strcmp(word, "trap") == 0)
			method = TRAP;
		else if (strcmp(word, "slope") == 0)
			method = SLOPE;
		...
		if (compute) {
			...
			if (method == TRAP)
				value -= 0.5 * vec[0] + 0.5 * vec[nvec - 1];
		}
		...
		if (fix) {
			...
			for (int i = 0; i < nvec; i++) {
				if (index)
					one = fix->compute_array(i, index - 1);
				else
					one = fix->compute_vector(i);
				if (method == SUM)
					value += one;
				else if (method == XMIN)
					value = MIN(value, one);
			...
			if (method == TRAP) {
				if (index)
					value -= 0.5 * fix->compute_array(0, index - 1)
							+ 0.5 * fix->compute_array(nvec - 1, index - 1);
				else
					value -= 0.5 * fix->compute_vector(0)
		...
		if (method == SLOPE) {
			...
			if (denominator != 0.0)
				value = numerator / denominator / nvec;
			else
				value = BIG;
		}
		...
		if (style[ivar] == SCALARFILE) {
			...
			double *result;
			memory->create(result, atom->nlocal, "variable:result");
			memcpy(result, reader[ivar]->fixstore->vstore,
					atom->nlocal * sizeof(double));
...
void Variable::peratom2global(int flag, char *word, double *vector, int nstride,
		tagint id, Tree **tree, Tree **treestack, int &ntreestack,
	...
	if (index >= 0 && index < atom->nlocal) {
		...
		if (flag == 0) {
			if (strcmp(word, "id") == 0)
				...
				error->one(FLERR, "Invalid atom vector in variable formula");

		} else
			mine = vector[index * nstride];
...
void Variable::atom_vector(char *word, Tree **tree, Tree **treestack,
		int &ntreestack) {
	if (tree == NULL)
	...
	if (strcmp(word, "id") == 0) {
		...
		newtree->array = &atom->x[0][0];
	else if (strcmp(word, "y") == 0)
		newtree->array = &atom->x[0][1];
	else if (strcmp(word, "z") == 0)
		newtree->array = &atom->x[0][2];
	else if (strcmp(word, "vx") == 0)
		newtree->array = &atom->v[0][0];
	else if (strcmp(word, "vy") == 0)
		newtree->array = &atom->v[0][1];
	else if (strcmp(word, "vz") == 0)
		newtree->array = &atom->v[0][2];
	else if (strcmp(word, "fx") == 0)
		newtree->array = &atom->f[0][0];
	else if (strcmp(word, "fy") == 0)
		newtree->array = &atom->f[0][1];
	else if (strcmp(word, "fz") == 0)
		newtree->array = &atom->f[0][2];
...
void Variable::print_tree(Tree *tree, int level) {
	...
	if (tree->first)
		print_tree(tree->first, level + 1);
	if (tree->second)
		print_tree(tree->second, level + 1);
	if (tree->nextra)
		for (int i = 0; i < tree->nextra; i++)
			print_tree(tree->extra[i], level + 1);
	return;
}
