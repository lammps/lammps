...
void DihedralHelixKokkos<DeviceType>::operator()(TagDihedralHelixCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT p = d_aphi[type]*(1.0 - c) + d_bphi[type]*(1.0 + cos(3.0*phi)) +
    d_cphi[type]*(1.0 + cos(phi + MY_PI4));
  const KK_FLOAT pd = -d_aphi[type] + 3.0*d_bphi[type]*sin(3.0*phi)*siinv +
    d_cphi[type]*sin(phi + MY_PI4)*siinv;
  ...
  const KK_FLOAT a22 = -sb2*(2.0*c0*s12a - ca*(s1+s2));
...
void DihedralHelixKokkos<DeviceType>::coeff(int narg, char **arg)
{
  ...
  for (int i = ilo; i <= ihi; i++) {
    k_aphi.view_host()[i] = aphi[i];
    k_bphi.view_host()[i] = bphi[i];
    k_cphi.view_host()[i] = cphi[i];
  }
