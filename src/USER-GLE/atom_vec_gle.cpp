...
/* GLE momenta should be carried around with the atoms as they move across
 * processors. Don't know what is the correct place to do that....
 */
#define COMM_GLES  // communicates GLE additional momenta across processors
#define DELTA 10000

/* ---------------------------------------------------------------------- */

AtomVecGLE::AtomVecGLE(LAMMPS * lmp): AtomVec(lmp)  //, int narg, char **arg) :  AtomVec(lmp, narg, arg)
{

  //! AtomVec classes used to have the infrastructure for reading extra arguments. Now I am hardcoding this, but it should really be a parameter.
  //if (narg>0)
  {
    ns = 6; //atoi(arg[0]);
  }
  ...
  size_border = 8;
#ifdef COMM_GLES
  size_velocity = 3 + 3 * (atom->ns+1);  // also transfer additional momenta with velocities!
#else
  size_velocity = 3;
#endif
  size_data_atom = 7;
  size_data_vel = 4;   // may need to manipulate this in order to save additional momenta to data file
  xcol_data = 5;
...
void AtomVecGLE::grow(int n)
{
  ...
  s = memory->grow(atom->s,nmax*comm->nthreads,3*(ns+1),"atom:s");
...
void AtomVecGLE::grow_reset()
{
  tag = atom->tag; type = atom->type;
  mask = atom->mask; image = atom->image;
  x = atom->x; v = atom->v; f = atom->f; s = atom->s;
...
void AtomVecGLE::copy(int i, int j, int delflag)
{
  int k;
  ...
  for (k = 0; k < 3*(ns+1); k++) s[j][k] = s[i][k];
...
int AtomVecGLE::pack_comm(int n, int *list, double *buf,
...
int AtomVecGLE::pack_comm_vel(int n, int *list, double *buf,
                               int pbc_flag, int *pbc)
{
  int i,j,k,m;
  ...
  if (pbc_flag == 0) {
    for (i = 0; i < n; i++) {
      ...
      buf[m++] = v[j][2];
#ifdef COMM_GLES
      for (k=0; k<3*(ns+1); k++) buf[m++] = s[j][k];
#endif
    }
  } else {
    if (domain->triclinic == 0) {
...
void AtomVecGLE::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecGLE::unpack_comm_vel(int n, int first, double *buf)
{
  int i,k,m,last;
  ...
  for (i = first; i < last; i++) {
    ...
    v[i][2] = buf[m++];
#ifdef COMM_GLES
    for (k=0; k<3*(ns+1); k++) s[i][k] = buf[m++];
#endif
  }
}
...
int AtomVecGLE::pack_reverse(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecGLE::unpack_reverse(int n, int *list, double *buf)
{
  int i,j,m;
...
int AtomVecGLE::pack_border(int n, int *list, double *buf,
                             int pbc_flag, int *pbc)
{
  int i,j,m;
  double dx,dy,dz;
...
int AtomVecGLE::pack_border_vel(int n, int *list, double *buf,
                                 int pbc_flag, int *pbc)
{
  int i,j,k,m;
  double dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecGLE::pack_border_hybrid(int n, int *list, double *buf)
{
  int i,j,m;
...
void AtomVecGLE::unpack_border(int n, int first, double *buf)
{
  int i,m,last;
...
void AtomVecGLE::unpack_border_vel(int n, int first, double *buf)
{
  int i,k,m,last;
...
int AtomVecGLE::unpack_border_hybrid(int n, int first, double *buf)
{
  int i,m,last;
...
int AtomVecGLE::pack_exchange(int i, double *buf)
{
  int k;
...
int AtomVecGLE::unpack_exchange(double *buf)
{
  int k;
  ...
  v[nlocal][2] = buf[m++];
#ifdef COMM_GLES
  for (k=0; k<3*(ns+1); k++) s[nlocal][k] = buf[m++];
#endif
...
int AtomVecGLE::pack_restart(int i, double *buf)
{
  int k;
...
int AtomVecGLE::unpack_restart(double *buf)
{
  int k;
...
void AtomVecGLE::create_atom(int itype, double *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
  ...
  v[nlocal][1] = 0.0;
  v[nlocal][2] = 0.0;
  for (int k=0; k<3*(ns+1); k++) s[nlocal][k] = 0.0;
...
void AtomVecGLE::data_atom(double *coord, tagint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
int AtomVecGLE::data_atom_hybrid(int nlocal, char **values)
{
...
void AtomVecGLE::pack_data(double **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecGLE::pack_data_hybrid(int i, double *buf)
{
...
void AtomVecGLE::write_data(FILE *fp, int n, double **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecGLE::write_data_hybrid(FILE *fp, double *buf)
{
...
bigint AtomVecGLE::memory_usage()
{
  bigint bytes = 0;
  ...
  if (atom->memcheck("s")) bytes += memory->usage(f,nmax,3*(ns+1));
