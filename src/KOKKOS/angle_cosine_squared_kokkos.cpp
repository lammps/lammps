...
#include "angle_cosine_squared_kokkos.h"
...
AngleCosineSquaredKokkos<DeviceType>::AngleCosineSquaredKokkos(LAMMPS *lmp) : AngleCosineSquared(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
AngleCosineSquaredKokkos<DeviceType>::~AngleCosineSquaredKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void AngleCosineSquaredKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleCosineSquaredCompute<1,1> >(0,nanglelist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleCosineSquaredCompute<0,1> >(0,nanglelist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleCosineSquaredCompute<1,0> >(0,nanglelist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleCosineSquaredCompute<0,0> >(0,nanglelist),*this);
    }
  }
...
void AngleCosineSquaredKokkos<DeviceType>::operator()(TagAngleCosineSquaredCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT dcostheta = c - d_cos_theta0[type];
  const KK_FLOAT tk = d_k[type] * dcostheta;

  KK_FLOAT eangle = static_cast<KK_FLOAT>(0.0);
  if (eflag) eangle = tk * dcostheta;

  const KK_FLOAT a = static_cast<KK_FLOAT>(2.0) * tk;
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleCosineSquaredKokkos<DeviceType>::operator()(TagAngleCosineSquaredCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagAngleCosineSquaredCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void AngleCosineSquaredKokkos<DeviceType>::allocate()
{
  AngleCosineSquared::allocate();

  int n = atom->nangletypes;
  k_k = DAT::tdual_kkfloat_1d("AngleCosineSquared::k",n+1);
  k_cos_theta0 = DAT::tdual_kkfloat_1d("AngleCosineSquared::cos_theta0",n+1);
...
template<class DeviceType>
void AngleCosineSquaredKokkos<DeviceType>::coeff(int narg, char **arg)
{
  AngleCosineSquared::coeff(narg, arg);
...
template<class DeviceType>
void AngleCosineSquaredKokkos<DeviceType>::read_restart(FILE *fp)
{
  AngleCosineSquared::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleCosineSquaredKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
...
namespace LAMMPS_NS {
template class AngleCosineSquaredKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class AngleCosineSquaredKokkos<LMPHostType>;
#endif
}
