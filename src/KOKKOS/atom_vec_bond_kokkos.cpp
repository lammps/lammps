                                          ...
                                          int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    dx = dy = dz = 0;
    if(space==Host) {
      AtomVecBondKokkos_PackBorder<Host,0> f(
        buf.h_view, k_sendlist.h_view,
        iswap,h_x,h_tag,h_type,h_mask,h_molecule,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecBondKokkos_PackBorder<Device,0> f(
        buf.d_view, k_sendlist.d_view,
        iswap,d_x,d_tag,d_type,d_mask,d_molecule,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
...
void AtomVecBondKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  atomKK->modified(space,X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|MOLECULE_MASK);
  if(space==Host) {
    struct AtomVecBondKokkos_UnpackBorder<Host>
      f(buf.h_view,h_x,h_tag,h_type,h_mask,h_molecule,first);
    Kokkos::parallel_for(n,f);
  } else {
    struct AtomVecBondKokkos_UnpackBorder<Device>
                                            ...
                                            KK_FLOAT hi )
{
  ...
  if(space == Host) {
    AtomVecBondKokkos_PackExchangeFunctor<Host>
      f(atomKK,k_buf,k_sendlist,k_copylist,atom->nlocal,dim,lo,hi);
    Kokkos::parallel_for(nsend,f);
    return nsend*elements;
  } else {
    AtomVecBondKokkos_PackExchangeFunctor<Device>
      f(atomKK,k_buf,k_sendlist,k_copylist,atom->nlocal,dim,lo,hi);
    Kokkos::parallel_for(nsend,f);
    return nsend*elements;
  }
...
int AtomVecBondKokkos::unpack_exchange_kokkos(DAT::tdual_float_2d &k_buf,int nrecv,
  ...
  if(space == Host) {
    ...
    AtomVecBondKokkos_UnpackExchangeFunctor<Host>
    ...
    k_count.modify_host();
    k_count.sync_device();
    AtomVecBondKokkos_UnpackExchangeFunctor<Device>
      f(atomKK,k_buf,k_count,dim,lo,hi);
    Kokkos::parallel_for(nrecv/elements,f);
