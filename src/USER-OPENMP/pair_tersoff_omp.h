...
class PairTersoffOMP : public PairOMP {
 ...
 protected:
  ...
  void setup();
  virtual void repulsive(const Param &, const double &, double &, int, double &);
  double zeta(const Param &, double, double, double *, double *);
  void force_zeta(const Param &, double, double, double &, double &, int, double &);
  void attractive(const Param &, const double &, const double &, 
		  const double &, double *, double *,
		  double *, double *, double *);

  double ters_fc_d(const double &, const Param &) const;
  virtual double ters_fa(const double &, const Param &) const;
  virtual double ters_fa_d(const double &, const Param &) const;
  double ters_bij(const double &, const Param &) const;
  double ters_bij_d(const double &, const Param &) const;

  void ters_zetaterm_d(double, double *, double, double *, double,
			       double *, double *, double *, const Param &);
  ...
  inline double vec3_dot(const double *x, const double *y) const {
    return x[0]*y[0] + x[1]*y[1] + x[2]*y[2];
  }
  inline void vec3_add(const double *x, const double *y, double *z) const {
    z[0] = x[0]+y[0];  z[1] = x[1]+y[1];  z[2] = x[2]+y[2];
  }
  inline void vec3_scale(const double k, const double *x, double *y) const {
