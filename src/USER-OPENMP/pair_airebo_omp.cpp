...
PairAIREBOOMP::PairAIREBOOMP(LAMMPS *lmp) : PairOMP(lmp)
{
  single_enable = 0;
  one_coeff = 1;
...
void PairAIREBOOMP::compute(int eflag, int vflag)
{
  ...
  if (evflag) {
    if (eflag) {
      if (vflag_atom) {
	FREBO<1,1,1,1>();
	if (ljflag) FLJ<1,1,1,1>();
      } else {
	FREBO<1,1,0,1>();
	if (ljflag) FLJ<1,1,0,1>();
      }
    } else {
      if (vflag_atom) {
	FREBO<1,0,1,1>();
	if (ljflag) FLJ<1,0,1,1>();
      } else {
	FREBO<1,0,0,1>();
	if (ljflag) FLJ<1,0,0,1>();
      }
    }
  } else {
    FREBO<0,0,0,1>();
    if (ljflag) FLJ<0,0,0,1>();
  }
...
void PairAIREBOOMP::settings(int narg, char **arg)
{
...
void PairAIREBOOMP::coeff(int narg, char **arg)
{
...
double PairAIREBOOMP::init_one(int i, int j)
{
...
------------------------------------------------------------------------- */
template <int EVFLAG, int EFLAG, int VFLAG_ATOM, int NEWTON_PAIR>
...
#endif
  {
    int i,j,k,m,ii,inum,itype,jtype,tid;
	...
	del[2] = delz;
	if (VFLAG_ATOM)
	  bij = bondorder<1>(i,j,del,rij,VA,f,tid);
	else
	  bij = bondorder<0>(i,j,del,rij,VA,f,tid);
	dVAdi = bij*dVA;
...
#endif
  {
    int i,j,k,m,ii,jj,kk,mm,inum,jnum,itype,jtype,ktype,mtype,tid;
	...
	if (Str > 0.0) {
	  ...
	  if (VFLAG_ATOM)
	    Stb = bondorderLJ<1>(i,j,delscale,rcmin[itype][jtype],
				 VA,delij,rij,f,tid);
	  else
	    Stb = bondorderLJ<0>(i,j,delscale,rcmin[itype][jtype],
				 VA,delij,rij,f,tid);
	} else Stb = 0.0;
...
void PairAIREBOOMP::TORSION(int eflag, int vflag)
{
	...
	for (kk = 0; kk < REBO_numneigh[i]; kk++) {
	  ...
	  for (ll = 0; ll < REBO_numneigh[j]; ll++) {
	    ...
	    if (evflag) {
	      ...
	      ev_tally4_thr(i,j,k,l,evdwl,fi,fj,fk,delil,del34,delkl,tid);
	    }
	  }
	}
      }
...
double PairAIREBOOMP::bondorder(int i, int j, double rij[3],
...
------------------------------------------------------------------------- */
template <int VFLAG_ATOM>
double PairAIREBOOMP::bondorderLJ(int i, int j, double rij[3], double rijmag,
	  ...
	  if (sin321 != 0.0) { 
		...
		if (sin234 != 0.0) {
		  ...
		  if (VFLAG_ATOM) {
		    r13[0] = -rjk[0]; r13[1] = -rjk[1]; r13[2] = -rjk[2];
		    r43[0] = -r34[0]; r43[1] = -r34[1]; r43[2] = -r34[2];
		    v_tally4_thr(atom1,atom2,atom3,atom4,
				 f1,f2,f4,r13,r23,r43,tid);
		  }
		}
	      }
	    }
...
double PairAIREBOOMP::gSpline(double costh, double Nij, int typei,
...
double PairAIREBOOMP::PijSpline(double NijC, double NijH, int typei, int typej,
			     double dN2[2])
{
  int x,y,i,done;
...
double PairAIREBOOMP::piRCSpline(double Nij, double Nji, double Nijconj,
...
double PairAIREBOOMP::TijSpline(double Nij, double Nji,
...
void PairAIREBOOMP::add_pages(int npage)
{
  maxpage += PGDELTA;
  pages = (int **) 
...
void PairAIREBOOMP::read_file(char *filename)
{
  int i,j,k,l,limit;
  char s[MAXLINE];
...
double PairAIREBOOMP::Spbicubic(double x, double y,
...
double PairAIREBOOMP::Sptricubic(double x, double y, double z,
...
void PairAIREBOOMP::spline_init()
{
  int i,j,k;
