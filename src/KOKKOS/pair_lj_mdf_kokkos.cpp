...
PairLJMDFKokkos<DeviceType>::PairLJMDFKokkos(LAMMPS *lmp) : PairLJMDF(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairLJMDFKokkos<DeviceType>::~PairLJMDFKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLJMDFKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairLJMDFKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairLJMDFKokkos<DeviceType>::
...
template<class DeviceType>
void PairLJMDFKokkos<DeviceType>::allocate()
{
  PairLJMDF::allocate();
  ...
  k_params = Kokkos::DualView<params_lj_mdf**,Kokkos::LayoutRight,DeviceType>("PairLJMDF::params",n+1,n+1);
...
template<class DeviceType>
void PairLJMDFKokkos<DeviceType>::init_style()
{
  PairLJMDF::init_style();
...
template<class DeviceType>
double PairLJMDFKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLJMDF::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLJMDFKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLJMDFKokkos<LMPHostType>;
#endif
}
