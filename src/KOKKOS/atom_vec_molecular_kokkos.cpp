                                             ...
                                             const int* const pbc)
{
  ...
  if(commKK->forward_comm_on_host) {
    ...
    if(pbc_flag) {
      ...
      if(domain->triclinic) {
        struct AtomVecMolecularKokkos_PackComm<Host,0,1>
          f(atomKK->k_x,buf,list,iswap,domain->xprd,domain->yprd,domain->zprd,
            domain->xy,domain->xz,domain->yz,pbc);
        Kokkos::parallel_for(n,f);
      } else {
        struct AtomVecMolecularKokkos_PackComm<Host,0,0>
          f(atomKK->k_x,buf,list,iswap,domain->xprd,domain->yprd,domain->zprd,
            domain->xy,domain->xz,domain->yz,pbc);
        Kokkos::parallel_for(n,f);
      }
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
        struct AtomVecMolecularKokkos_PackComm<Device,1,1>
          f(atomKK->k_x,buf,list,iswap,domain->xprd,domain->yprd,domain->zprd,
            domain->xy,domain->xz,domain->yz,pbc);
        Kokkos::parallel_for(n,f);
      } else {
        struct AtomVecMolecularKokkos_PackComm<Device,1,0>
          f(atomKK->k_x,buf,list,iswap,domain->xprd,domain->yprd,domain->zprd,
            domain->xy,domain->xz,domain->yz,pbc);
        Kokkos::parallel_for(n,f);
      }
      ...
      if(domain->triclinic) {
        struct AtomVecMolecularKokkos_PackComm<Device,0,1>
          f(atomKK->k_x,buf,list,iswap,domain->xprd,domain->yprd,domain->zprd,
            domain->xy,domain->xz,domain->yz,pbc);
        Kokkos::parallel_for(n,f);
      } else {
        struct AtomVecMolecularKokkos_PackComm<Device,0,0>
          f(atomKK->k_x,buf,list,iswap,domain->xprd,domain->yprd,domain->zprd,
            domain->xy,domain->xz,domain->yz,pbc);
        Kokkos::parallel_for(n,f);
      }
...
struct AtomVecMolecularKokkos_PackCommSelf {
  ...
  AtomVecMolecularKokkos_PackCommSelf(
      ...
      const KK_FLOAT &xprd, const KK_FLOAT &yprd, const KK_FLOAT &zprd,
      const KK_FLOAT &xy, const KK_FLOAT &xz, const KK_FLOAT &yz, const int* const pbc):
    _x(DualViewHelper<Space>::view(x)),_xw(DualViewHelper<Space>::view(x)),_nfirst(nfirst),
...
int AtomVecMolecularKokkos::pack_comm_self(const int &n, const DAT::tdual_int_2d &list,
  ...
  if(commKK->forward_comm_on_host) {
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
      struct AtomVecMolecularKokkos_PackCommSelf<Host,1,1>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      } else {
      struct AtomVecMolecularKokkos_PackCommSelf<Host,1,0>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      }
      ...
      if(domain->triclinic) {
      struct AtomVecMolecularKokkos_PackCommSelf<Host,0,1>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      } else {
      struct AtomVecMolecularKokkos_PackCommSelf<Host,0,0>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      }
    ...
    if(pbc_flag) {
      if(domain->triclinic) {
      struct AtomVecMolecularKokkos_PackCommSelf<Device,1,1>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      } else {
      struct AtomVecMolecularKokkos_PackCommSelf<Device,1,0>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      }
      ...
      if(domain->triclinic) {
      struct AtomVecMolecularKokkos_PackCommSelf<Device,0,1>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      } else {
      struct AtomVecMolecularKokkos_PackCommSelf<Device,0,0>
        f(atomKK->k_x,nfirst,list,iswap,domain->xprd,domain->yprd,domain->zprd,
          domain->xy,domain->xz,domain->yz,pbc);
      Kokkos::parallel_for(n,f);
      }
...
void AtomVecMolecularKokkos::unpack_comm_kokkos(const int &n, const int &first,
  ...
  if(commKK->forward_comm_on_host) {
    atomKK->sync(Host,X_MASK);
    atomKK->modified(Host,X_MASK);
    struct AtomVecMolecularKokkos_UnpackComm<Host> f(atomKK->k_x,buf,first);
    Kokkos::parallel_for(n,f);
  } else {
    atomKK->sync(Device,X_MASK);
    atomKK->modified(Device,X_MASK);
    struct AtomVecMolecularKokkos_UnpackComm<Device> f(atomKK->k_x,buf,first);
    Kokkos::parallel_for(n,f);
  }
}
                                               ...
                                               int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    if(space==Host) {
      AtomVecMolecularKokkos_PackBorder<Host,1> f(
        buf.h_view, k_sendlist.h_view,
        iswap,h_x,h_tag,h_type,h_mask,h_molecule,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecMolecularKokkos_PackBorder<Device,1> f(
    ...
    dx = dy = dz = 0;
    if(space==Host) {
      AtomVecMolecularKokkos_PackBorder<Host,0> f(
        buf.h_view, k_sendlist.h_view,
        iswap,h_x,h_tag,h_type,h_mask,h_molecule,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecMolecularKokkos_PackBorder<Device,0> f(
        buf.d_view, k_sendlist.d_view,
        iswap,d_x,d_tag,d_type,d_mask,d_molecule,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
...
void AtomVecMolecularKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  atomKK->modified(space,X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|MOLECULE_MASK);
  if(space==Host) {
    struct AtomVecMolecularKokkos_UnpackBorder<Host>
      f(buf.h_view,h_x,h_tag,h_type,h_mask,h_molecule,first);
    Kokkos::parallel_for(n,f);
  } else {
    struct AtomVecMolecularKokkos_UnpackBorder<Device>
...
struct AtomVecMolecularKokkos_PackExchangeFunctor {
  ...
  AtomVecMolecularKokkos_PackExchangeFunctor(
    ...
    _num_dihedral(DualViewHelper<Space>::view(atom->k_num_dihedral)),
    _dihedral_type(DualViewHelper<Space>::view(atom->k_dihedral_type)),
    _dihedral_atom1(DualViewHelper<Space>::view(atom->k_dihedral_atom1)),
    _dihedral_atom2(DualViewHelper<Space>::view(atom->k_dihedral_atom2)),
    _dihedral_atom3(DualViewHelper<Space>::view(atom->k_dihedral_atom3)),
    _dihedral_atom4(DualViewHelper<Space>::view(atom->k_dihedral_atom4)),
    _num_improper(DualViewHelper<Space>::view(atom->k_num_improper)),
    _improper_type(DualViewHelper<Space>::view(atom->k_improper_type)),
    _improper_atom1(DualViewHelper<Space>::view(atom->k_improper_atom1)),
    _improper_atom2(DualViewHelper<Space>::view(atom->k_improper_atom2)),
    _improper_atom3(DualViewHelper<Space>::view(atom->k_improper_atom3)),
    _improper_atom4(DualViewHelper<Space>::view(atom->k_improper_atom4)),
    ...
    _num_dihedralw(DualViewHelper<Space>::view(atom->k_num_dihedral)),
    _dihedral_typew(DualViewHelper<Space>::view(atom->k_dihedral_type)),
    _dihedral_atom1w(DualViewHelper<Space>::view(atom->k_dihedral_atom1)),
    _dihedral_atom2w(DualViewHelper<Space>::view(atom->k_dihedral_atom2)),
    _dihedral_atom3w(DualViewHelper<Space>::view(atom->k_dihedral_atom3)),
    _dihedral_atom4w(DualViewHelper<Space>::view(atom->k_dihedral_atom4)),
    _num_improperw(DualViewHelper<Space>::view(atom->k_num_improper)),
    _improper_typew(DualViewHelper<Space>::view(atom->k_improper_type)),
    _improper_atom1w(DualViewHelper<Space>::view(atom->k_improper_atom1)),
    _improper_atom2w(DualViewHelper<Space>::view(atom->k_improper_atom2)),
    _improper_atom3w(DualViewHelper<Space>::view(atom->k_improper_atom3)),
    _improper_atom4w(DualViewHelper<Space>::view(atom->k_improper_atom4)),
                                                 ...
                                                 KK_FLOAT hi )
{
  ...
  if(space == Host) {
    AtomVecMolecularKokkos_PackExchangeFunctor<Host>
      f(atomKK,k_buf,k_sendlist,k_copylist,atom->nlocal,dim,lo,hi);
    Kokkos::parallel_for(nsend,f);
    return nsend*elements;
  } else {
    AtomVecMolecularKokkos_PackExchangeFunctor<Device>
      f(atomKK,k_buf,k_sendlist,k_copylist,atom->nlocal,dim,lo,hi);
    Kokkos::parallel_for(nsend,f);
    return nsend*elements;
  }
...
int AtomVecMolecularKokkos::unpack_exchange_kokkos(DAT::tdual_double_2d_lr &k_buf,int nrecv,
  ...
  if(space == Host) {
    ...
    AtomVecMolecularKokkos_UnpackExchangeFunctor<Host>
    ...
    k_count.modify_host();
    k_count.sync_device();
    AtomVecMolecularKokkos_UnpackExchangeFunctor<Device>
      f(atomKK,k_buf,k_count,dim,lo,hi);
    Kokkos::parallel_for(nrecv/elements,f);
