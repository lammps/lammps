...
FixQMMM::~FixQMMM()
{
  ...
  if( comm_buf ) memory->destroy(comm_buf);
  if( qm_coord ) memory->destroy(qm_coord);
  if( qm_force ) memory->destroy(qm_force);
  if( qm_charge) memory->destroy(qm_charge);
}
      ...
      for (i=0; i<nlocal; ++i) {
        ...
        if (mask[i] & groupbit) {
          ...
          if (j != 3*HASH_FAIL) {
            ...
            mm_mask_all[ i ] = atom->type[i];
          }
        }
        mm_coord_all[ 3*i + 0] = x[i][0];
        mm_coord_all[ 3*i + 1] = x[i][1];
        mm_coord_all[ 3*i + 2] = x[i][2];
        mm_charge_all[ i ] = charge[i];
      }
...
/*
 *  siamo nel loop comm->me == 0 questo ciclo for qui non ha senso
 *
 ...
 for ( int k=0; k < other_nlocal; ++k ) {
 mm_coord_all[ 3*count_all + 3*k + 0 ]   = other_mm_coord[ 3*k + 0 ];
 mm_coord_all[ 3*count_all + 3*k + 1 ]   = other_mm_coord[ 3*k + 1 ];
 mm_coord_all[ 3*count_all + 3*k + 2 ]   = other_mm_coord[ 3*k + 2 ];
 mm_charge_all[ count_all + k ]  = other_mm_charge[ k ];
 mm_mask_all[ count_all + k ] = other_mm_mask[ k ];
 }

 count_all += other_nlocal;

 if( other_mm_coord ) free( other_mm_coord );
 if( other_mm_charge ) free( other_mm_charge );
 if( other_mm_mask ) free( other_mm_mask );
...
*/
      ...
      MPI_Rsend(&nlocal, 1, MPI_INT, 0, 1, world);
      for (i=0; i<nlocal; ++i) {
        mm_coord_all[ 3*i + 0 ]   = x[i][0];
        mm_coord_all[ 3*i + 1 ]   = x[i][1];
        mm_coord_all[ 3*i + 2 ]   = x[i][2];
      ...
      MPI_Rsend(mm_coord_all, 3*nlocal, MPI_DOUBLE, 0, 2, world);
      MPI_Rsend(mm_charge_all, nlocal, MPI_DOUBLE, 0, 3, world);
  ...
  //
  if( aradii ) free( aradii );
  if( mm_coord_all ) free( mm_coord_all );
  if( mm_charge_all ) free( mm_charge_all );
  if( mm_mask_all ) free( mm_mask_all );
...
void FixQMMM::exchange_forces()
{
  ...
  if (qmmm_role == QMMM_ROLE_MASTER) {
    ...
    int * nlocal_all = (int *) calloc( comm->nprocs , sizeof(int) );
    nlocal_all[ comm->me ] = nlocal;
    MPI_Reduce(MPI_IN_PLACE,nlocal_all, comm->nprocs, MPI_INT, MPI_SUM, 0, world );

    double * mm_force_all = (double *) calloc( num_mm * 3 , sizeof( double ) );
    ...
    if (comm->me == 0) {
      ...
      MPI_Irecv(mm_force_all,3*num_mm,MPI_DOUBLE,1,QMMM_TAG_FORCE2,qm_comm,req+1);
      MPI_Wait(req+1,MPI_STATUS_IGNORE);
      // receive MM forces from LAMMPS
      MPI_Irecv( mm_force_on_qm_atoms, 3*num_qm,MPI_DOUBLE,1,QMMM_TAG_FORCE,mm_comm,req+2);
    ...
    MPI_Bcast(mm_force_all,num_mm*3,MPI_DOUBLE,0,world);

    int offset = 0;
    for( int i = 0; i < comm->me; i++ ) offset += nlocal_all[ i ]; 

    if (screen)  fprintf(screen,"nlocal=%d,num_mm=%d,num_qm=%d \n",nlocal,num_mm,num_qm);
    if (logfile)  fprintf(logfile,"nlocal=%d,num_mm=%d,num_qm=%d \n",nlocal,num_mm,num_qm);
    ...
    if (qmmm_mode == QMMM_MODE_MECH){
        for (int i=0; i < nlocal; ++i)
      	    if (mask[i] & groupbit)
                for (int j=0; j < num_qm; ++j) {
                    if (tag[i] == buf[j].tag) {
                        f[i][0] += buf[j].x;
                        f[i][1] += buf[j].y;  // NOTE bug in the original version from Axel
                        f[i][2] += buf[j].z;  // NOTE bug in the original version from Axel
                    }
                }
        ...
        for (int i=0; i < nlocal; ++i) {
            if (mask[i] & groupbit) {
                ...
                f[i][0] += qmmm_fscale * mm_force_all[ 3*(i+offset) + 0 ];
                f[i][1] += qmmm_fscale * mm_force_all[ 3*(i+offset) + 1 ];
                f[i][2] += qmmm_fscale * mm_force_all[ 3*(i+offset) + 2 ];
            }
        }
    }
...
/*
    ...
    for (int i=0; i < nlocal; ++i) {
      if (mask[i] & groupbit) {
         const int j = 3*taginthash_lookup((taginthash_t *)qm_idmap, tag[i]);
         if (j != 3*HASH_FAIL) {
	    fprintf(stdout,"*** FORCE on QM atoms (MASTER)= %lf %lf %lf\n",f[i][0],f[i][1],f[i][2]);
	    fprintf(stdout,"*** FORCE on QM atoms (SLAVE )= %lf %lf %lf\n", mm_force_on_qm_atoms[j+0], mm_force_on_qm_atoms[j+1], mm_force_on_qm_atoms[j+2]);
	    fprintf(stdout,"*** FORCE on QM atoms (QE    )= %lf %lf %lf\n", qm_force[j+0], qm_force[j+1], qm_force[j+2]);
            f[i][0] = f[i][0] - mm_force_on_qm_atoms[j+0] + qm_force[j+0];
            f[i][1] = f[i][1] - mm_force_on_qm_atoms[j+1] + qm_force[j+1];
            f[i][2] = f[i][2] - mm_force_on_qm_atoms[j+2] + qm_force[j+2];
	   fprintf(stdout,"*** FORCE on QM atoms (TOTAL )= %lf %lf %lf\n\n",f[i][0],f[i][1],f[i][2]);
         }
         if (qmmm_mode == QMMM_MODE_ELEC) {
         }
      } else {
         if (qmmm_mode == QMMM_MODE_ELEC) {
           // add "Laio" contribution to all atoms
	   fprintf(stdout,"*** FORCE on MM atoms (MASTER)= %lf %lf %lf\n",f[i][0],f[i][1],f[i][2]);
	   fprintf(stdout,"*** FORCE on MM atoms (QE    )= %lf %lf %lf\n",mm_force_all[ 3*i ], mm_force_all[ 3*i + 1 ], mm_force_all[ 3*i + 2 ]);
           f[i][0] +=  mm_force_all[ 3*i + 0 ];
           f[i][1] +=  mm_force_all[ 3*i + 1 ];
           f[i][2] +=  mm_force_all[ 3*i + 2 ];
	   fprintf(stdout,"*** FORCE on MM atoms (TOTAL )= %lf %lf %lf\n\n",f[i][0],f[i][1],f[i][2]);
         }
      }
    }
...
*/
    ...
    if( nlocal_all ) free( nlocal_all );
    if( mm_force_all ) free( mm_force_all );
...
void FixQMMM::init()
{
  ...
  if (do_init) {
    ...
    if (qmmm_role == QMMM_ROLE_MASTER) {
      ...
      if (screen)  fprintf(screen,fmt,num_mm);
      if (logfile) fprintf(logfile,fmt,num_mm);
...
static const int FixQMMM_Z[QMMM_ISOTOPES] = { 1, 1, 1, 2, 2, 3, 3, 4, 5, 5, 6, 6, 6, 7, 7, 8, 8, 8, 9,
...
static const char FixQMMM_EL[QMMM_ISOTOPES][4] = {"H", "H", "H", "He", "He", "Li", "Li", "Be", "B", "B",
...
static const int FixQMMM_A[QMMM_ISOTOPES] = {1, 2, 3, 3, 4, 6, 7, 9, 10, 11, 12, 13, 14, 14, 15, 16, 17, 18,
...
static const double FixQMMM_MASS[QMMM_ISOTOPES] = {1.00782503207, 2.0141017778, 3.0160492777, 3.0160293191, 4.00260325415, 6.015122795, 7.01600455, 9.0121822,
