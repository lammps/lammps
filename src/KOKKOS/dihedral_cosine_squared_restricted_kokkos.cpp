...
/* ----------------------------------------------------------------------
   ...
   [ based on dihedral_harmonic_kokkos.cpp and
     dihedral_cosine_squared_restricted.cpp ]
------------------------------------------------------------------------- */

#include "dihedral_cosine_squared_restricted_kokkos.h"
...
DihedralCosineSquaredRestrictedKokkos<DeviceType>::DihedralCosineSquaredRestrictedKokkos(LAMMPS *lmp)
  : DihedralCosineSquaredRestricted(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
DihedralCosineSquaredRestrictedKokkos<DeviceType>::~DihedralCosineSquaredRestrictedKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagDihedralCosineSquaredRestrictedCompute<1,1> >(0,ndihedrallist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagDihedralCosineSquaredRestrictedCompute<0,1> >(0,ndihedrallist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagDihedralCosineSquaredRestrictedCompute<1,0> >(0,ndihedrallist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagDihedralCosineSquaredRestrictedCompute<0,0> >(0,ndihedrallist),*this);
    }
  }
...
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::operator()(TagDihedralCosineSquaredRestrictedCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT p0 = Kokkos::cos(d_phi0[type]);
  const KK_FLOAT sq_sin = static_cast<KK_FLOAT>(1.0) - c*c;

  const KK_FLOAT pd = static_cast<KK_FLOAT>(2.0)*d_k[type]*(c - p0)*(static_cast<KK_FLOAT>(1.0) - c*p0) / (sq_sin*sq_sin);

  KK_FLOAT edihedral = 0.0;
  if (EVFLAG && eflag) edihedral = d_k[type]*(c - p0)*(c - p0) / sq_sin;

  KK_FLOAT a = pd;
  KK_FLOAT cs = c * a;
  KK_FLOAT s12a = s12 * a;
  const KK_FLOAT a11 = cs*sb1*s1;
  const KK_FLOAT a22 = -sb2*(static_cast<KK_FLOAT>(2.0)*c0*s12a - cs*(s1+s2));
  const KK_FLOAT a33 = cs*sb3*s2;
  const KK_FLOAT a12 = -r12c1*(c1mag*cs*s1 + c2mag*s12a);
  const KK_FLOAT a13 = -rb1*rb3*s12a;
  const KK_FLOAT a23 = r12c2*(c2mag*cs*s2 + c1mag*s12a);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::operator()(TagDihedralCosineSquaredRestrictedCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagDihedralCosineSquaredRestrictedCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::allocate()
{
  DihedralCosineSquaredRestricted::allocate();

  int n = atom->ndihedraltypes;
  k_k = DAT::tdual_kkfloat_1d("DihedralCosineSquaredRestricted::k",n+1);
  k_phi0 = DAT::tdual_kkfloat_1d("DihedralCosineSquaredRestricted::phi0",n+1);

  d_k = k_k.template view<DeviceType>();
  d_phi0 = k_phi0.template view<DeviceType>();
}
...
template<class DeviceType>
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::coeff(int narg, char **arg)
{
  DihedralCosineSquaredRestricted::coeff(narg, arg);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    k_phi0.view_host()[i] = static_cast<KK_FLOAT>(phi0[i]);
  }
...
template<class DeviceType>
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::read_restart(FILE *fp)
{
  DihedralCosineSquaredRestricted::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void DihedralCosineSquaredRestrictedKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i1, const int i2, const int i3, const int i4,
