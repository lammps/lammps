...
class PPPM : public KSpace {
 ...
 protected:
  ...
  // FFTs (heFFTe) and grid communication

  // heFFTe 1D FFT backed selection
  #ifdef Heffte_ENABLE_GPU
  // NVIDIA CUFFT backend (default)
  auto backend_tag = heffte::backend::cufft;
  // AMD CUFFT backend
  // auto backend_tag = heffte::backend::rocfft;
  #else 
  // FFTW3 backend (default)
  auto backend_tag = heffte::backend::fftw;
  // MKL backend
  // auto backend_tag = heffte::backend::mkl;
  #endif

  std::unique_ptr< heffte::fft3d<backend_tag> > fft1, fft2;
