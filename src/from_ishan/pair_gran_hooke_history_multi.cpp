...
double PairGranHookeHistoryMulti::init_one(int i, int j)
{
  ...
  if (cut[i][j] < 0.0) cutoff = maxrad_dynamic[i]+maxrad_dynamic[j];
  if (cut[i][j] < 0.0) cutoff = MAX(cutoff,maxrad_frozen[i]+maxrad_dynamic[j]);
  if (cut[i][j] < 0.0) cutoff = MAX(cutoff,maxrad_dynamic[i]+maxrad_frozen[j]);

  // cut[i][j] = MAX(maxrad_dynamic[i],maxrad_dynamic[j]);     
