...
#include "pair_lj_class2_soft_kokkos.h"
...
PairLJClass2SoftKokkos<DeviceType>::PairLJClass2SoftKokkos(LAMMPS *lmp) : PairLJClass2Soft(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairLJClass2SoftKokkos<DeviceType>::~PairLJClass2SoftKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairLJClass2SoftKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  EV_FLOAT ev = pair_compute<PairLJClass2SoftKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
...
KK_FLOAT PairLJClass2SoftKokkos<DeviceType>::
compute_fpair(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  return lj1 * eps * (static_cast<KK_FLOAT>(18.0)*r4sig6/(denlj*denlj*Kokkos::sqrt(denlj)) -
                      static_cast<KK_FLOAT>(18.0)*r4sig6/(denlj*denlj));
}
...
compute_evdwl(const KK_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  return lj1 * eps *
    (static_cast<KK_FLOAT>(2.0)/(denlj*Kokkos::sqrt(denlj)) - static_cast<KK_FLOAT>(3.0)/denlj) -
...
template<class DeviceType>
void PairLJClass2SoftKokkos<DeviceType>::allocate()
{
  PairLJClass2Soft::allocate();

  int n = atom->ntypes;
  memory->destroy(cutsq);
  memoryKK->create_kokkos(k_cutsq,cutsq,n+1,n+1,"pair:cutsq");
  d_cutsq = k_cutsq.template view<DeviceType>();
  k_params = Kokkos::DualView<params_lj**,Kokkos::LayoutRight,DeviceType>("PairLJClass2Soft::params",n+1,n+1);
...
template<class DeviceType>
void PairLJClass2SoftKokkos<DeviceType>::init_style()
{
  PairLJClass2Soft::init_style();
...
template<class DeviceType>
double PairLJClass2SoftKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairLJClass2Soft::init_one(i,j);
...
namespace LAMMPS_NS {
template class PairLJClass2SoftKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairLJClass2SoftKokkos<LMPHostType>;
#endif
}
