...
#include "eam_lj_cl.h"
#else
#include "eam_lj_ptx.h"
#endif

#include "lal_eam_lj.h"
#include <cassert>
using namespace LAMMPS_AL;
#define EAMLJT EAMLJ<numtyp, acctyp>
...
EAMLJT::EAMLJ() : BaseAtomic<numtyp,acctyp>(), 
...
template <class numtyp, class acctyp>
int EAMLJT::init(const int ntypes, double host_cutforcesq,
              int **host_type2rhor, int **host_type2z2r, int *host_type2frho,
              double ***host_rhor_spline, double ***host_z2r_spline,
              double ***host_frho_spline, double **host_cutsq, double **host_lj1, 
...
template <class numtyp, class acctyp>
double EAMLJT::host_memory_usage() const {
  return this->host_memory_usage_atomic()+sizeof(EAMLJ<numtyp,acctyp>);
}
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void EAMLJT::compute(const int f_ago, const int inum_full, const int nlocal,
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
int** EAMLJT::compute(const int ago, const int inum_full,
                    const int nall, double **host_x, int *host_type,
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void EAMLJT::compute2(int *ilist, const bool eflag, const bool vflag,
                    const bool eatom, const bool vatom) {
  if (this->ans->inum()==0) 
    return;
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void EAMLJT::loop(const bool _eflag, const bool _vflag) {
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void EAMLJT::loop2(const bool _eflag, const bool _vflag) {
  // Compute the block size and grid size to keep all cores busy
  const int BX=this->block_size();
  int eflag, vflag;
  if (_eflag)
...
template class EAMLJ<PRECISION,ACC_PRECISION>;
