...
PairMIECutOMP::PairMIECutOMP(LAMMPS *lmp) : PairMIECut(lmp), ThrOMP(lmp, THR_PAIR)
{
...
void PairMIECutOMP::compute(int eflag, int vflag)
{
  ev_init(eflag, vflag);
...
void PairMIECutOMP::eval(int iifrom, int iito, ThrData *const thr)
{
  ...
  for (ii = iifrom; ii < iito; ++ii) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      factor_mie = special_lj[sbmask(j)];
      j &= NEIGHMASK;
...
double PairMIECutOMP::memory_usage()
{
  double bytes = memory_usage_thr();
  bytes += PairMIECut::memory_usage();
  return bytes;
}
