  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"disc") == 0) {
      ...
      if (domain->dimension != 2)
        error->all(FLERR, Error::NOLASTLINE, "Fix {} disc option requires 2d simulation", style);
    }
    iarg++;
  }
}
...
#pragma omp parallel for LMP_DEFAULT_NONE
      ...
      // NOTE: this branch also updates omega, so omega must not be const here

      double * const * const omega_dlm = atom->omega;
...
#pragma omp parallel for LMP_DEFAULT_NONE
      ...
      for (int i = 0; i < nlocal; i++) {
        ...
        if (mask[i] & groupbit && mu[i][3] > 0.0) {
          ...
          w[0] = omega_dlm[i][0]; w[1] = omega_dlm[i][1]; w[2] = omega_dlm[i][2];
          ...
          omega_dlm[i][0] = w_temp[0]; omega_dlm[i][1] = w_temp[1]; omega_dlm[i][2] = w_temp[2];
