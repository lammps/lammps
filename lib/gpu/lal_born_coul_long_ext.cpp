...
int borncl_gpu_init(const int ntypes, double **cutsq, double **host_rhoinv,
  ...
  int init_ok=0;
  if (world_me==0)
    init_ok = BORNCLMF.init(ntypes, cutsq, host_rhoinv, host_born1, host_born2, host_born3, host_a,
  ...
  for (int i=0; i<procs_per_gpu; i++) {
    ...
    if (gpu_rank==i && world_me!=0)
      init_ok = BORNCLMF.init(ntypes, cutsq, host_rhoinv, host_born1, host_born2, host_born3,
                              host_a, host_c, host_d, sigma, offset, special_lj, inum, nall,
                              max_nbors, maxspecial, cell_size, screen, host_cut_ljsq,
