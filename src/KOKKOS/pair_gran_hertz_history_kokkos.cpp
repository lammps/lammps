...
void PairGranHertzHistoryKokkos<DeviceType>::init_style()
{
  ...
  if (history && fix_history == nullptr) {
    this->set_history_id();
    auto cmd = std::string(this->id_history) + " all ";
...
void PairGranHertzHistoryKokkos<DeviceType>::operator()(TagPairGranHertzHistoryCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG,SHEARUPDATE>, const int ii, EV_FLOAT &ev) const {
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    // Hertzian contact scales the normal and tangential force by the
    // square root of the overlap and the reduced radius

    const KK_FLOAT polyhertz = Kokkos::sqrt((radsum-r)*irad*jrad / radsum);
    ccel *= polyhertz;
    ...
    KK_FLOAT fs1 = -polyhertz * (kt_kk*shear1 + meff*gammat_kk*vtr1);
    KK_FLOAT fs2 = -polyhertz * (kt_kk*shear2 + meff*gammat_kk*vtr2);
    KK_FLOAT fs3 = -polyhertz * (kt_kk*shear3 + meff*gammat_kk*vtr3);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void PairGranHertzHistoryKokkos<DeviceType>::operator()(TagPairGranHertzHistoryCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG,SHEARUPDATE>, const int ii) const {
  EV_FLOAT ev;
  this->template operator()<NEIGHFLAG,NEWTON_PAIR,VFLAG,SHEARUPDATE>(TagPairGranHertzHistoryCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG,SHEARUPDATE>(), ii, ev);
}
