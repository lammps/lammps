...
//=====================================================================================================
  ...
  while (iarg < narg){
	...
	Gamma[itype] = scalefactor;
      else for(int it=1; it <= atom->ntypes; it++)
	     Gamma[it] = scalefactor;
      iarg += 3;
    }
...
a z wall velocity without implementing fixed BCs in z");
  ...
  density_lb = nullptr;
  u_lb = nullptr;
  Ff = nullptr;  Wf = nullptr;
  Fftempx = nullptr; Wftempx = nullptr;
  Fftempy = nullptr; Wftempy = nullptr;
  Fftempz = nullptr; Wftempz = nullptr;
  ...
  if(setdx == 1){
    ...
    double mindomain = std::min(std::min(domain->xprd/comm->procgrid[0],domain->yprd/comm->procgrid[1]),
  ...
  //--------------------------------------------------------------------------
  if(seta0 == 1)
    a_0_real = 0.333333333333333*dx_lb*dx_lb/dt_lb/dt_lb;
  ...
  if (fabs(rhofactor) > 2)
    error->all(FLERR,"Illegal pressure jump");
  ...
  if(fabs(aa - floor(aa+0.5)) > eps){
    ...
    sprintf(errormessage,"With dx= %f, and the simulation domain divided by %i processors in the x direction, the simulation domain in the x direction must be a multiple of %f",
  ...
  if(fabs(aa - floor(aa+0.5)) > eps){
    ...
    char errormessage[200];
    sprintf(errormessage,"With dx= %f, and the simulation domain divided by %i processors in the y direction, the simulation domain in the y direction must be a multiple of %f",
  ...
  if(fabs(aa - floor(aa+0.5)) > eps){
    ...
    char errormessage[200];
    sprintf(errormessage,"With dx= %f, and the simulation domain divided by %i processors in the z direction, the simulation domain in the z direction must be a multiple of %f",
  ...
  if(npits != -1) {
    ...
    if(comm->me==0){
      if(h_s <= 1)
	error->all(FLERR,"hp too big for system size in z-direction");
      if(w_p > Nby)
	error->all(FLERR,"wp bigger than system size in y-direction");
      if(w_p && sw)
	error->message(FLERR,"wp ignored if there are side walls, pits are full width in y-direction");
      if(2*l_e+npits*l_p+(npits-1)*l_pp < Nbx)
	error->all(FLERR,"length of pits and end segments too little to fill system size in x-direction");
      else if(2*l_e+npits*l_p+(npits-1)*l_pp > Nbx)
	error->message(FLERR,"length of pits and end segments larger than system size in x-direction: truncation will occur");
      if(numvel == 19)
    ...
    if(sw && comm->me==0)
      error->all(FLERR,"Sidewalls require pit geometry (you can enable by setting npits to 0 or higher)");
  }
  ...
  MPI_Type_create_hvector(subNbx+3,1,1*(subNbz+3)*5*sizeofdouble,oneslice,&passyWtemp);
  MPI_Type_commit(&passyWtemp);

  MPI_Type_free(&oneslice);
  MPI_Type_vector(subNby+3,1,1*5,MPI_DOUBLE,&oneslice);
  MPI_Type_commit(&oneslice);
  MPI_Type_create_hvector(subNbx+3,1,1*5*(subNby+3)*sizeofdouble,oneslice,&passzWtemp);
...
{
  return FixConst::INITIAL_INTEGRATE | FixConst::PRE_FORCE | FixConst::POST_FORCE | FixConst::FINAL_INTEGRATE | FixConst::END_OF_STEP;
}
...
void FixLbFluid::init(void)
{
  ...
  if(fabs(dt_lb_now - dt_lb) > 1.0e-12){
    error->warning(FLERR,"Timestep change between runs with the same lb/fluid. Unphysical results may occur");
  }
  ...
  if (comm->me==0 && domain->boundary[1][0] == 0 && sw){
    error->warning(FLERR,"Particle-Particle interactions are periodic in y-direction but there are fixed boundaries in y-direction for lb/fluid.");
  }
  if (comm->me==0 && domain->boundary[1][0] == 1 && sw==0){
    error->warning(FLERR,"lb/fluid boundaries are periodic in y-direction but there are fixed boundies in y-direction for particle-particle interactions.");
  }
  if (domain->boundary[2][0] == 2 || domain->boundary[2][0] == 3){
    error->all(FLERR,"the z-direction cannot have shrink-wrap boundary conditions");
  }
  if (domain->boundary[2][0] == 0 && npits != -1) {
    error->all(FLERR,"the z-direction cannot have periodic boundary conditions when there are pits.");
  }
  ...
  if(!(groupbit_viscouslb) && comm->me==0){
    error->message(FLERR,"Not adding the fluid force to any of the MD particles.  To add this force use lb/viscous fix");
  }
}
...
void FixLbFluid::setup(int /* vflag */)
{
  ...
  //  calc_fluidforceI();
  //  if(comm->me == 0)
  //    std::cout << "calc_force in lb setup..." << std::endl;
  //}
}
...
void FixLbFluid::InitializeFirstRun(void) {
  ...
  if (me==0) std::cout << "First Run initialized\n";
}
...
{ 
  for(int i=0; i<subNbx+3; i++) {
    ...
    int ix;
    if (i==subNbx+2) ix = round(((domain->sublo[0] - domain->boxlo[0])/dx_lb)) - 2;
    else ix = round(((domain->sublo[0] - domain->boxlo[0])/dx_lb)) + (i-1);
    
    for(int j=0; j<subNby+3; j++)
      for(int k=0; k<subNbz+3; k++){

	if (lin_init)
	  density_lb[i][j][k] = rhoH + (rhoL-rhoH)*ix/(Nbx-1); //linear interpolation between boundary values
	else
	  density_lb[i][j][k] = densityinit;
	
	int iz;
	if (k==subNbz+2) iz = round(((domain->sublo[2] - domain->boxlo[2])/dx_lb)) - 2;
	else iz = round(((domain->sublo[2] - domain->boxlo[2])/dx_lb)) + (k-1);
	
	u_lb[i][j][k][0]=0.0;
	if (lin_init)
	  u_lb[i][j][k][1]=vwbt + (vwtp-vwbt)*iz/(Nbz-1); //linear interpolation between boundary velocities
	else
	  u_lb[i][j][k][1]=0.0;
        u_lb[i][j][k][2]=0.0;
      }
	...
	if (numvel == 15)
	  for(int m=0; m<15; m++)
	    f_lb[i][j][k][m] = density_lb[i][j][k]*w_lb15[m]*
	      (1+(u_lb[i][j][k][0]*e15[m][0]+u_lb[i][j][k][1]*e15[m][1]+u_lb[i][j][k][2]*e15[m][2])/a_0);
	else
	  for(int m=0; m<19; m++)
	    f_lb[i][j][k][m] = density_lb[i][j][k]*w_lb19[m]*
	      (1+(u_lb[i][j][k][0]*e19[m][0]+u_lb[i][j][k][1]*e19[m][1]+u_lb[i][j][k][2]*e19[m][2])/a_0);
...
void FixLbFluid::calc_fluidforceweight(void)
{
  ...
  if(domain->periodicity[2]!=0 || comm->myloc[2] != 0){
    ...
    MPI_Isend(&Wf[0][0][2],1,passzrho,comm->procneigh[2][0],40,world,&requests[requestcount+1]);
    MPI_Isend(&Wf[0][0][3],1,passzrho,comm->procneigh[2][0],50,world,&requests[requestcount+2]);

    MPI_Irecv(&Wf[0][0][subNbz+2],1,passzrho,comm->procneigh[2][0],60,world,&requests[requestcount+3]);
    MPI_Irecv(&Wf[0][0][0],1,passzrho,comm->procneigh[2][0],70,world,&requests[requestcount+4]);
    requestcount=requestcount+5;
  }
  if(domain->periodicity[2]!=0 || comm->myloc[2] != (comm->procgrid[2]-1)){
    MPI_Isend(&Wf[0][0][subNbz-3],1,passzrho,comm->procneigh[2][1],60,world,&requests[requestcount]);
    MPI_Isend(&Wf[0][0][subNbz-2],1,passzrho,comm->procneigh[2][1],70,world,&requests[requestcount+1]);

    MPI_Irecv(&Wf[0][0][subNbz-1],1,passzrho,comm->procneigh[2][1],30,world,&requests[requestcount+2]);
    MPI_Irecv(&Wf[0][0][subNbz],1,passzrho,comm->procneigh[2][1],40,world,&requests[requestcount+3]);
    MPI_Irecv(&Wf[0][0][subNbz+1],1,passzrho,comm->procneigh[2][1],50,world,&requests[requestcount+4]);
    requestcount=requestcount+5;
  }
...
void FixLbFluid::keys_interpolationweight(int i)
{
  ...
  if(ix < 0 || ix > (subNbx-1) || iy < 0 || iy > (subNby-1) || iz < 0 || iz > (subNbz-1)) {
    ...
    error->one(FLERR,"Atom outside local processor simulation domain!"
...
void FixLbFluid::keys_interpolation(int i, int uonly)
{
  ...
  unew[0]=0.0; unew[1]=0.0; unew[2]=0.0;
  mnode = 0.0;
  double myweight = 0.0;
  isten=0;
  ...
  if (Gamma[type[i]] < 0) {  // stationary or prescribed motion for particles, massone->infinity limit
    ...
    for(k=0; k< 3; k++) {
      ...
      //Full force on fluid: 
      Ffull[k] = (hF[k] + f[i][k]*dt_lb*dt_lb/dm_lb/dx_lb*mnode/(mnode+massone))/myweight;
    }
  }
  ...
  for(ii=-1; ii<3; ii++)
    for(jj=-1; jj<3; jj++)
      for(kk=-1; kk<3; kk++){
        ...
        for(k=0; k<3; k++){
	  Ff[ixp][iyp][izp][k] += Ffull[k]*TfP[isten];
        }
...
void FixLbFluid::IBM3_interpolationweight(int i)
{
  ...
  if (dx1 < 0.0) {std::cout << "fuck\n"; exit(0);}
  if (dy1 < 0.0) {std::cout << "fuck\n"; exit(0);}
  if (dz1 < 0.0) {std::cout << "fuck\n"; exit(0);}
...
{// usual call had uonly=0
  ...
  for (k=0; k<3; k++) {         // tri-linearly interpolated mass-weighted velocity at node
    unew[k] = (density_lb[ix][iy][iz]*u_lb[ix][iy][iz][k]*FfP[0]
		...
		+ density_lb[ixp][iyp][izp]*u_lb[ixp][iyp][izp][k]*FfP[7])/mnode;
  }
...
static MPI_Datatype mpiTypeGlobalWrite(const int local_ghost, const int * local_size,
                                       ...
                                       const int global_ghost, const int * global_size,
...
/// \param[in] global_size   Size of global buffer excluding ghost points
///
static MPI_Datatype mpiTypeDumpGlobal(const int * local_size,
                                      const int * global_offset,
...
void FixLbFluid::SetupBuffers(void)
{
  ...
  fluid_local_n0[2] = subNbz-1 - (domain->periodicity[2]==0 && (comm->myloc[2]==comm->procgrid[2]-1));
  ...
  for (int i=xstart; i<xend; i++) {
	...
	if(type != 2) {	
	  ...
	  etacov[7] = rho*u_lb[i][j][k][0]*u_lb[i][j][k][1] +
	    tauR*(u_lb[i][j][k][0]*(Fy_w+rho*bodyforcey) + (Fx_w+rho*bodyforcex)*u_lb[i][j][k][1]);
	  etacov[8] = rho*u_lb[i][j][k][1]*u_lb[i][j][k][2] +
	    tauR*(u_lb[i][j][k][1]*(Fz_w+rho*bodyforcez) + (Fy_w+rho*bodyforcey)*u_lb[i][j][k][2]);
	  etacov[9] = rho*u_lb[i][j][k][0]*u_lb[i][j][k][2] +
	    tauR*(u_lb[i][j][k][0]*(Fz_w+rho*bodyforcez) + (Fx_w+rho*bodyforcex)*u_lb[i][j][k][2]);
	  etacov[10] = 0.0;
	  etacov[11] = 0.0;
	  etacov[12] = 0.0;
	  ...
	  if(noisestress==1){
	    ...
	    for(int jj=0; jj<3; jj++)
	      S[0][jj] = stdv*random->gaussian();
	    for(int jj=0; jj<3; jj++)
	      S[1][jj] = stdv*random->gaussian();
		...
		(mg_lb15[4][l]*etacov[4]*Ng_lb15[4] + mg_lb15[5][l]*etacov[5]*Ng_lb15[5] +
		 ...
		 mg_lb15[10][l]*etacov[10]*Ng_lb15[10] + mg_lb15[11][l]*etacov[11]*Ng_lb15[11]
		 + mg_lb15[12][l]*etacov[12]*Ng_lb15[12] + mg_lb15[13][l]*etacov[13]*Ng_lb15[13]
...
void FixLbFluid::equilibriumdist19(int xstart, int xend, int ystart, int yend, int zstart, int zend) {
  ...
  for (i=xstart; i<xend; i++) {
	...
	Pxx = p0 + kappa_lb*(drhox*drhox - 0.5*grs)+tauR*(1.0/3.0-dPdrho)*
	  (3.0*u_lb[i][j][k][0]*drhox+u_lb[i][j][k][1]*drhoy+u_lb[i][j][k][2]*drhoz);
	Pyy = p0 + kappa_lb*(drhoy*drhoy - 0.5*grs)+tauR*(1.0/3.0-dPdrho)*
	  (u_lb[i][j][k][0]*drhox+3.0*u_lb[i][j][k][1]*drhoy+u_lb[i][j][k][2]*drhoz);
	Pzz = p0 + kappa_lb*(drhoz*drhoz - 0.5*grs)+tauR*(1.0/3.0-dPdrho)*
	  (u_lb[i][j][k][0]*drhox+u_lb[i][j][k][1]*drhoy+3.0*u_lb[i][j][k][2]*drhoz);
	Pxy = kappa_lb*drhox*drhoy+tauR*(1.0/3.0-dPdrho)*
	  (u_lb[i][j][k][0]*drhoy+u_lb[i][j][k][1]*drhox);
	Pxz = kappa_lb*drhox*drhoz+tauR*(1.0/3.0-dPdrho)*
	  (u_lb[i][j][k][0]*drhoz+u_lb[i][j][k][2]*drhox);
	Pyz = kappa_lb*drhoy*drhoz+tauR*(1.0/3.0-dPdrho)*
          ...
          for (l=0; l<19; l++) {
            ghostnoise = w_lb19[l]*
              (mg_lb19[4][l]*etacov[4]*Ng_lb19[4] + mg_lb19[5][l]*etacov[5]*Ng_lb19[5] +
               ...
               mg_lb19[10][l]*etacov[10]*Ng_lb19[10] + mg_lb19[11][l]*etacov[11]*Ng_lb19[11]
               + mg_lb19[12][l]*etacov[12]*Ng_lb19[12] + mg_lb19[13][l]*etacov[13]*Ng_lb19[13]
               + mg_lb19[14][l]*etacov[14]*Ng_lb19[14] + mg_lb19[15][l]*etacov[15]*Ng_lb19[15]
               + mg_lb19[16][l]*etacov[16]*Ng_lb19[16] + mg_lb19[17][l]*etacov[17]*Ng_lb19[17]
...
void FixLbFluid::correctu_full(void)
{
  ...
  int requestcount=0;
  if(domain->periodicity[2]!=0 || comm->myloc[2] != 0){
    MPI_Isend(&u_lb[0][0][1][0],1,passzu,comm->procneigh[2][0],5,world,&requests2[requestcount]);
    MPI_Isend(&u_lb[0][0][2][0],1,passzu,comm->procneigh[2][0],10,world,&requests2[requestcount+1]);
    MPI_Isend(&u_lb[0][0][3][0],1,passzu,comm->procneigh[2][0],20,world,&requests2[requestcount+2]);
    MPI_Irecv(&u_lb[0][0][subNbz+2][0],1,passzu,comm->procneigh[2][0],30,world,&requests2[requestcount+3]);
    MPI_Irecv(&u_lb[0][0][0][0],1,passzu,comm->procneigh[2][0],35,world,&requests2[requestcount+4]);
    requestcount=requestcount+5;

  }
  if(domain->periodicity[2]!=0 || comm->myloc[2] != (comm->procgrid[2]-1)){
    MPI_Isend(&u_lb[0][0][subNbz-3][0],1,passzu,comm->procneigh[2][1],30,world,&requests2[requestcount]);
    MPI_Isend(&u_lb[0][0][subNbz-2][0],1,passzu,comm->procneigh[2][1],35,world,&requests2[requestcount+1]);
    MPI_Irecv(&u_lb[0][0][subNbz-1][0],1,passzu,comm->procneigh[2][1],5,world,&requests2[requestcount+2]);
    MPI_Irecv(&u_lb[0][0][subNbz][0],1,passzu,comm->procneigh[2][1],10,world,&requests2[requestcount+3]);
    MPI_Irecv(&u_lb[0][0][subNbz+1][0],1,passzu,comm->procneigh[2][1],20,world,&requests2[requestcount+4]);
    requestcount=requestcount+5;
...
void FixLbFluid::correctu_part(int xstart, int xend, int ystart, int yend, int zstart, int zend)
{
  for(int i=xstart; i<xend; i++){
	...
	if (sublattice[i][j][k].type == 0) { // update bulk sites, not boundaries (with set u), or exterior
  ...
  for(int i=xstart; i<xend; i++)
	...
	if (type == 0) { // bulk fluid nodes
	  ...
	  for(int nbb=1; nbb<=bbl[ori][0]; nbb++) {
	    ...
	    fnew[i][j][k][ll] = f_lb[i][j][k][od[ll]] - (f_lb[i][j][k][od[ll]] - feq[i][j][k][od[ll]])/tau;
	  }
	  // normal propagation
	  fnew[i][j][k][0] = f_lb[i][j][k][0] - (f_lb[i][j][k][0]-feq[i][j][k][0])/tau;
	  for(int nbb=bbl[ori][0]+1; nbb<=15; nbb++) {
	    int ll=bbl[ori][nbb];
	    int imod=(i-e15[ll][0]); int jmod=(j-e15[ll][1]); int kmod=(k-e15[ll][2]);
	    
	    fnew[i][j][k][ll] = f_lb[imod][jmod][kmod][ll] - (f_lb[imod][jmod][kmod][ll]-feq[imod][jmod][kmod][ll])/tau;
	  }
	}
	  ...
	  if (numvel==15) {
		...
		fnew[i][j][k][6]+fnew[i][j][k][11]+fnew[i][j][k][12]+fnew[i][j][k][13]+fnew[i][j][k][14]+
		f_lb[i][j][k][6] + (feq[i][j][k][6]-f_lb[i][j][k][6])/tau +
		f_lb[i][j][k][11] + (feq[i][j][k][11]-f_lb[i][j][k][11])/tau +
		f_lb[i][j][k][12] + (feq[i][j][k][12]-f_lb[i][j][k][12])/tau+
		f_lb[i][j][k][13] + (feq[i][j][k][13]-f_lb[i][j][k][13])/tau+
		f_lb[i][j][k][14] + (feq[i][j][k][14]-f_lb[i][j][k][14])/tau;
	      rbvw=rb*vwbt;
	      rvw2 = rbvw*vwbt;
	      seqyy = rb*a_0 + rvw2;
	      ...
	      fnew[i][j][k][0] = rb - 2.*(fnew[i][j][k][6]+fnew[i][j][k][11]+fnew[i][j][k][12]+fnew[i][j][k][13]+
					  fnew[i][j][k][14]+fnew[i][j][k][2]+fnew[i][j][k][4])+rvw2;
	      fnew[i][j][k][1] = 0.5*(fnew[i][j][k][2]+fnew[i][j][k][4]-rvw2);
	      fnew[i][j][k][3] = fnew[i][j][k][1];
	      fnew[i][j][k][5] = fnew[i][j][k][6]+2.*(fnew[i][j][k][11]+fnew[i][j][k][12]+fnew[i][j][k][13]+fnew[i][j][k][14])+
		fnew[i][j][k][2]+fnew[i][j][k][4]-seqyy;
	      ...
	      fnew[i][j][k][10]=0.25*(-rbvw+seqyy-2.*fnew[i][j][k][4])-fnew[i][j][k][14];
	      
	    }
	    else { // top wall modified return (ori should now be 16 or something is messed up)
	      double rb, rbvw, rvw2, seqyy, r;
	      rb = fnew[i][j][k][0]+fnew[i][j][k][1]+fnew[i][j][k][2]+fnew[i][j][k][3]+fnew[i][j][k][4]+
		fnew[i][j][k][5]+fnew[i][j][k][7]+fnew[i][j][k][8]+fnew[i][j][k][9]+fnew[i][j][k][10]+
		f_lb[i][j][k][5] + (feq[i][j][k][5]-f_lb[i][j][k][5])/tau +
		f_lb[i][j][k][7] + (feq[i][j][k][7]-f_lb[i][j][k][7])/tau +
		f_lb[i][j][k][8] + (feq[i][j][k][8]-f_lb[i][j][k][8])/tau+
		f_lb[i][j][k][9] + (feq[i][j][k][9]-f_lb[i][j][k][9])/tau+
		f_lb[i][j][k][10] + (feq[i][j][k][10]-f_lb[i][j][k][10])/tau;
	      rbvw=rb*vwtp;
	      rvw2 = rbvw*vwtp;
	      seqyy = rb*a_0 + rvw2;
	      r = rb/density_lb[i][j][k];
	      ...
	      fnew[i][j][k][2] = r*feq[i][j][k][2];
	      fnew[i][j][k][4] = r*feq[i][j][k][4];
	      fnew[i][j][k][0] = rb - 2.*(fnew[i][j][k][5]+fnew[i][j][k][7]+fnew[i][j][k][8]+fnew[i][j][k][9]+
					  fnew[i][j][k][10]+fnew[i][j][k][2]+fnew[i][j][k][4])+rvw2;
	      fnew[i][j][k][1] = 0.5*(fnew[i][j][k][2]+fnew[i][j][k][4]-rvw2);
	      fnew[i][j][k][3] = fnew[i][j][k][1];
	      fnew[i][j][k][6] = fnew[i][j][k][5]+2.*(fnew[i][j][k][7]+fnew[i][j][k][8]+fnew[i][j][k][9]+fnew[i][j][k][10])+
		fnew[i][j][k][2]+fnew[i][j][k][4]-seqyy;
	      ...
	      fnew[i][j][k][14]=0.25*(-rbvw+seqyy-2.*fnew[i][j][k][4])-fnew[i][j][k][10];
	    }
	  }
	  else {
	    for(int m=0; m<19; m++){
   ...
   if(pressure==1){
     if(comm->myloc[0]==0){
       for(int j=1; j<subNby-1; j++) {
     	 int jup = j+1; int jdwn = j-1;
     	 for(int k=1; k<subNbz-1; k++){
     	   int kup = k+1; int kdwn = k-1;
	   if(sublattice[1][j][k].type == 0) { // this works ok, but should have some case for walls and fnew from walls.
     ...
     if(comm->myloc[0]==comm->procgrid[0]-1){
	   ...
	   if(sublattice[subNbx-2][j][k].type == 0) {
	     ...
	     fnew[subNbx-2][j][k][3] = fnew[subNbx-2][j][k][3]*rhoL/density_lb[subNbx-1][j][k];
	     fnew[subNbx-2][j][k][8] = fnew[subNbx-2][j][k][8]*rhoL/density_lb[subNbx-1][jdwn][kdwn];
	     fnew[subNbx-2][j][k][9] = fnew[subNbx-2][j][k][9]*rhoL/density_lb[subNbx-1][jup][kdwn];
	     fnew[subNbx-2][j][k][12] = fnew[subNbx-2][j][k][12]*rhoL/density_lb[subNbx-1][jdwn][kup];
	     fnew[subNbx-2][j][k][13] = fnew[subNbx-2][j][k][13]*rhoL/density_lb[subNbx-1][jup][kup];
	   }
     	 }
       }
     }
  ...
  if(domain->periodicity[2]==0){
   ...
   if(pressure==1){
	 ...
	 for(k=1; k<subNbz-1; k++){
	   ...
	   fnew[subNbx-2][j][k][3] = fnew[subNbx-2][j][k][3]*rhoL/density_lb[subNbx-1][j][k];
	   fnew[subNbx-2][j][k][9] = fnew[subNbx-2][j][k][9]*rhoL/density_lb[subNbx-1][jdwn][k];
	   fnew[subNbx-2][j][k][10] = fnew[subNbx-2][j][k][10]*rhoL/density_lb[subNbx-1][jup][k];
	   fnew[subNbx-2][j][k][13] = fnew[subNbx-2][j][k][13]*rhoL/density_lb[subNbx-1][j][kdwn];
	   fnew[subNbx-2][j][k][14] = fnew[subNbx-2][j][k][14]*rhoL/density_lb[subNbx-1][j][kup];
	 }
       }
     }
   }
...
void FixLbFluid::initializeGeometry() {
  ...
  for(i=0; i<subNbx; i++) // Sweep over the sublattice including the buffer zone.
	...
	sublattice[i][j][k] = 
	  wholelattice[(boxlims[0][0]+i-1+Nbx)%Nbx][(boxlims[1][0]+j-1+Nby)%Nby][(boxlims[2][0]+k-1+Nbz)%Nbz];
	if (k==0 && comm->myloc[2]==0) { // true boundary ghost rather than neighboring processor ghost
	  Site just_inside = wholelattice[(boxlims[0][0]+i-1+Nbx)%Nbx][(boxlims[1][0]+j-1+Nby)%Nby][(boxlims[2][0]+k+1-1+Nbz)%Nbz];
  ...
  if(geodump) {
    ...
    outFile << std::endl;
    outFile << "me: " << me << " px: " << comm->myloc[0] << " py: " << comm->myloc[1] << " pz: " << comm->myloc[2] << std::endl;
    for(i=0; i<subNbx; i++) {
...
void FixLbFluid::addslit(int &x0, const int HS, const int HP, const int LE, const int SW) {
  ...
  for(int ii=0; ii<xend; ii++) {
	...
	if(kk == HP) { // bottom of slit & bottom edges
	  temp.type = 1; temp.orientation = 3;
	  if(SW==1 && jj==0) { temp.orientation = 6; }
	  else if(SW==1 && jj==Nby-1) { temp.orientation = 19; }
	}
	else if(kk == (HP + HS)) { // slit ceiling & top edges
	  temp.type = 1; temp.orientation = 16;
	  if(SW==1 && jj==0) { temp.orientation = 27; }
	  else if(SW==1 && jj==Nby-1) { temp.orientation = 28; }
	}
	else { // bulk nodes & side walls
	  temp.type = 0;
	  temp.orientation = 0;
	  if(SW==1 && jj==0) { temp.orientation = 2; temp.type = 1;}
	  else if(SW==1 && jj==Nby-1) { temp.orientation = 15; temp.type = 1;}
	}
	wholelattice[x0+ii][jj][kk] = temp;
      }
      temp.type = 2;
...
void FixLbFluid::addpit(int &x0, const int HS, const int HP, const int WP, const int LP, const int SW) {
  ...
  int p_edge1y = (Nby-WP)/2; int p_edge2y = p_edge1y+WP-1;

  if(SW==1) {p_edge1y = 0; p_edge2y=Nby-1; }
  ...
  if(SW==0) {
	...
	for(int kk=0; kk<(HS+HP+1); kk++) {
	  ...
	  // top plate
	    ...
	    temp.orientation = 16; temp.type = 1;
	  }
	  ...
	  else if((ii>x0 && ii<LP+x0) && (jj > p_edge1y && jj < p_edge2y) && kk == 0 && HP < (Nbz-1)) {
	    temp.orientation = 3;
	    temp.type = 1;
	  }
	  ...
	  // top corners at the level of slit floor WHEN THERE ARE NO SIDE WALLS
	    ...
	    if(x0 > 0 && LP < Nbx) { temp.orientation = 10; temp.type = 1; }
	    else { temp.orientation = 7; temp.type = 1;} 
	  }
	  else if(ii == (x0+LP) && jj == p_edge1y && p_edge1y > 0 && kk == HP) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx) { temp.orientation = 23; temp.type = 1; }
	    else { temp.orientation = 7; temp.type = 1; } 
	  }
	  else if(ii == x0 && jj == p_edge2y && (p_edge2y<Nby) && kk == HP) {
	    if(x0 > 0 && LP < Nbx) { temp.orientation = 11; temp.type = 1; }
	    else { temp.orientation = 20; temp.type = 1; }
	  }
	  else if(ii == (x0+LP) && jj == p_edge2y && (p_edge2y<Nby) && kk == HP) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx) { temp.orientation = 24; temp.type = 1; }
	    else { temp.orientation = 20; temp.type = 1; }
	  }

	  // UPSTREAM corners at the level of pit floor
	  else if(ii == x0 && jj == p_edge1y && (p_edge1y>0) && kk == 0) {
	    if(x0 > 0 && LP < Nbx && HP < (Nbz-1)) temp.orientation = 25;     // corner whose "normal" pointing to fluid is  (+1,+1,+1)
	    else if(x0 > 0 && LP < Nbx && HP >= (Nbz-1)) temp.orientation = 22;// VERTICAL SHUTE inside edge along z axis "normal" pointing to fluid is  (+1,+1,0)
	    else temp.orientation = 6;
	    temp.type = 1;
	  }
	  // DOWNSTREAM corners at the level of pit floor
	  else if(ii == (x0+LP) && jj == p_edge1y && (p_edge1y>0) && kk == 0) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && HP < (Nbz-1)) temp.orientation = 12; // ***redundant check
	    else if((x0+LP)<(Nbx-1) && LP < Nbx && HP >= (Nbz-1)) temp.orientation = 9;// VERTICAL SHUTE: HP >= Nbz-1 i.e. height of pit
	    else temp.orientation = 6;
	    temp.type = 1;
	  }
	  // UPSTREAM corners at the level of pit floor
	  else if(ii == x0 && jj == p_edge2y && (p_edge2y<Nby) && kk == 0) {
	    if(x0 > 0 && LP < Nbx && HP < (Nbz-1)) temp.orientation = 26;
	    else if(x0 > 0 && LP < Nbx && HP >= (Nbz-1)) temp.orientation = 21;
	    else temp.orientation = 19;
	    temp.type = 1;
	  }
	  // DOWNSTREAM corners at the level of pit floor
	  else if(ii == (x0+LP) && jj == p_edge2y && (p_edge2y<Nby) && kk == 0) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && HP < (Nbz-1)) temp.orientation = 13;
	    else if((x0+LP)<(Nbx-1) && LP < Nbx && HP >= (Nbz-1)) temp.orientation = 8;
	    else temp.orientation = 19;
	    temp.type = 1;
	  }
	  ...
	  // side walls + edges 
	    ...
	    if(x0>0 && (HP>(Nbz-1)) && LP<Nbx && kk<HP) { temp.orientation = 1; temp.type = 1;} // wall of VERTICAL SHUTE with normal (+1,0,0)
	    else if(x0 > 0 && (kk<HP) && (LP<Nbx) && kk>0) { temp.orientation = 1; temp.type = 1;} // UPSTREAM WALL with normal (+1,0,0)
	    else if(kk==0) { // bottom edge along y
	      if(x0 > 0 && LP<Nbx) { temp.orientation = 5; temp.type = 1; }
	      else { temp.orientation = 3; temp.type = 1; }  
	    }
	    else if(kk==HP) { // bottom edge along y
	      if(x0>0 && LP<Nbx) { temp.orientation = 4; temp.type = 1; }
	      else { temp.orientation = 0; temp.type = 0;} // ***redundant
	    }
	    else { temp.orientation = 0; temp.type = 0;}
	  }
	  else if(ii==(x0+LP) && (jj>p_edge1y) && (jj<p_edge2y)) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && kk<HP && HP > (Nbz-1)) { temp.orientation = 14; temp.type = 1; }
	    else if((x0+LP)<(Nbx-1) && LP < Nbx && kk<HP && kk>0) { temp.orientation = 14; temp.type = 1; }
	    else if(kk==0) { // bottom edge along y
	      if((x0+LP)<(Nbx-1) && LP<Nbx) { temp.orientation = 18; temp.type = 1; }
	      else { temp.orientation = 3; temp.type = 1; }
	    }
	    else if(kk==HP) {
	      if(LP<Nbx) { temp.orientation = 17; temp.type = 1; }
	      else { temp.orientation = 0; temp.type = 0;}
	    }
	    else { temp.orientation = 0; temp.type = 0; }
	  }
	  else if(ii>x0 && (ii<(x0+LP)) && jj==p_edge1y && (p_edge1y>0) && kk<HP) {
	    if(HP >= (Nbz-1)) { temp.orientation = 2; temp.type = 1; }
	    else if(kk>0 && HP < (Nbz-1)) { temp.orientation = 2; temp.type = 1; }
	    else if(kk==0) { temp.orientation = 6; temp.type = 1; } // edge along x axis
	    else { temp.orientation = 0; temp.type = 0; }
	  }
	  else if(ii>x0 && (ii<(x0+LP)) && jj==p_edge2y && (p_edge2y<Nby) && kk<HP) {
	    if(HP >= (Nbz-1)) { temp.orientation = 15; temp.type = 1; }
	    else if(kk>0 && HP < (Nbz-1)) {temp.orientation = 15; temp.type = 1; }    
	    else if(kk==0) { temp.orientation = 19; temp.type = 1; } // edge along x axis
	    else { temp.orientation = 0; temp.type = 0; }
	  }
	  ...
	  // top edges along x axis. If sidewalls => they become part of flat wall with (0,+/-1,0) surface normals
	    ...
	    temp.orientation = 7; temp.type = 1;
	  }
	  else if(ii>x0 && ii<(x0+LP) && jj==p_edge2y && kk==HP) {
	    temp.orientation = 20; temp.type = 1;
	  }

	  // edges along z axis
	  else if(ii==x0 && jj==p_edge1y && kk<HP) {
	    if(x0 > 0 && LP<Nbx && HP >= (Nbz-1)) { temp.orientation = 22; temp.type = 1; }
	    else if(x0 > 0 && LP < Nbx && kk>0 && HP < (Nbz-1)) { temp.orientation = 22; temp.type = 1; } 
	    else { temp.orientation = 2; temp.type = 1; }
	  }
	  else if(ii==x0 && jj==p_edge2y && kk<HP) {
	    if(x0 > 0 && LP < Nbx && HP >= (Nbz-1)) { temp.orientation = 21; temp.type = 1; }
	    else if(x0 > 0 && LP < Nbx && kk>0 && HP < (Nbz-1)) { temp.orientation = 21; temp.type = 1; }
	    else { temp.orientation = 15; temp.type = 1; }
	  }
	  else if(ii==(x0+LP) && jj==p_edge1y && kk<HP) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && HP >= (Nbz-1)) { temp.orientation = 9; temp.type = 1; }
	    else if((x0+LP)<(Nbx-1) && LP < Nbx && kk>0 && HP < (Nbz-1)) { temp.orientation = 9; temp.type = 1; }
	    else { temp.orientation = 2; temp.type = 1;}
	  }
	  else if(ii==(x0+LP) && jj==p_edge2y && kk<HP) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && HP >= (Nbz-1)) { temp.orientation = 8; temp.type = 1; }
	    else if((x0+LP)<(Nbx-1) && LP < Nbx && kk>0 && HP < (Nbz-1)) { temp.orientation = 8; temp.type = 1; }
	    else { temp.orientation = 15; temp.type = 1; }
	  }
	  else {
	    temp.orientation = 0;
  ...
  // PIT REGION IN PRESENCE OF SIDE WALLS IN SYSTEM
	...
	for(kk=0; kk<(HS+HP+1); kk++) {
	  ...
	  if(kk == HP+HS && HP < (Nbz-1)) {
	    if(jj==p_edge1y) { temp.orientation = 27; temp.type = 1; }
	    else if(jj==p_edge2y) { temp.orientation = 28; temp.type = 1; }
	    else { temp.orientation = 16; temp.type = 1; }
	  }
	  else if(kk>HP && kk<HP+HS) {
	    if(jj==p_edge1y) { temp.orientation = 2; temp.type = 1; }
	    else if(jj==p_edge2y) { temp.orientation = 15; temp.type = 1; }
	    else { temp.orientation = 0; temp.type = 0; }
	  }

	  // bottom of pit, ok for troughs, too
	  else if((ii>x0 && ii<LP+x0) && (jj > p_edge1y && jj < p_edge2y) && kk == 0 && HP < (Nbz-1)) {
	    temp.orientation = 3; temp.type = 1;
	  }

	  // top corners at the level of slit floor WHEN THERE ARE SIDE WALLS
	  else if(ii == x0 && jj == p_edge1y && kk == HP) {
	    if(x0 > 0 && LP < Nbx) { temp.orientation = 29; temp.type = 1; }
	    else { temp.orientation = 2; temp.type = 1;} 
	  }
	  else if(ii == (x0+LP) && jj == p_edge1y && kk == HP) {
	    if((x0+LP-1)<(Nbx-1) && LP < Nbx) { temp.orientation = 31; temp.type = 1; }
	    //	    else { temp.orientation = 6; temp.type = 1; } 
	  }
	  else if(ii == x0 && jj == p_edge2y && kk == HP) {
	    if(x0 > 0 && LP < Nbx) { temp.orientation = 30; temp.type = 1; }
	    else { temp.orientation = 15; temp.type = 1; }
	  }
	  else if(ii == (x0+LP) && jj == p_edge2y && kk == HP) {
	    if((x0+LP-1)<(Nbx-1) && LP < Nbx) { temp.orientation = 32; temp.type = 1; }
	    //	    else { temp.orientation = 19; temp.type = 1; }
	  }

	  // UPSTREAM corners at the bottom of the pit
	  else if(ii == x0 && jj == p_edge1y && kk == 0) {
	    if(x0 > 0 && LP < Nbx) temp.orientation = 25;        // corner whose "normal" pointing to fluid is  (+1,+1,+1)
	    else temp.orientation = 6;
	    temp.type = 1;
	  }
	  ...
	  // UPSTREAM corners at the bottom of the pit
	    ...
	    if(x0 > 0 && LP < Nbx) temp.orientation = 26;
	    else temp.orientation = 19;
	    temp.type = 1;
	  }
	  // DOWNSTREAM corners at the bottom of the pit
	  ...
	  // UPSTREAM and DOWNSTREAM pit walls + edges
	    ...
	    if(x0 > 0 && (kk<HP) && (LP<Nbx) && kk>0)
	      { temp.orientation = 1; temp.type = 1;} // UPSTREAM WALL with normal (+1,0,0)
	    else if(kk==0) { // bottom edge along y
	      if(x0 > 0 && LP<Nbx) { temp.orientation = 5; temp.type = 1; }
	      else { temp.orientation = 3; temp.type = 1; }  
	    }
	    ...
	    if((x0+LP)<(Nbx-1) && LP < Nbx && kk<HP && kk>0) { temp.orientation = 14; temp.type = 1; }
	    else if(kk==0) { // bottom edge along y
	      if((x0+LP)<(Nbx-1) && LP<Nbx) { temp.orientation = 18; temp.type = 1; }
	      else { temp.orientation = 3; temp.type = 1; }
	    }
	  ...
	  // SIDE WALLS INSIDE PIT
	    ...
	    if(kk>0 && kk<HP+HS) { temp.orientation = 2; temp.type = 1; }  // wall, surface normal (0,+1,0)
	    else if(kk==0) { temp.orientation = 6; temp.type = 1; } // edge along x axis
	    else { temp.orientation = 0; temp.type = 0; }
	  }
	  else if(ii>x0 && (ii<(x0+LP)) && jj==p_edge2y) {
	    if(kk>0 && kk<HP+HS) {temp.orientation = 15; temp.type = 1; }  // wall, surface normal (0,-1,0)
	    else if(kk==0) { temp.orientation = 19; temp.type = 1; } // edge along x axis
	    else { temp.orientation = 0; temp.type = 0; }
	  }
	  ...
	  // edges along z axis
	    ...
	    if(x0 > 0 && LP < Nbx && kk>0) { temp.orientation = 22; temp.type = 1; }
	    else { temp.orientation = 2; temp.type = 1; }
	  }
	  else if(ii==x0 && jj==p_edge2y && kk<HP) {
	    if(x0 > 0 && LP < Nbx && kk>0) { temp.orientation = 21; temp.type = 1; }
	    else { temp.orientation = 15; temp.type = 1; }
	  }
	  else if(ii==(x0+LP) && jj==p_edge1y && kk<HP) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && kk>0) { temp.orientation = 9; temp.type = 1; }
	    //	    else { temp.orientation = 2; temp.type = 1;}
	  }
	  else if(ii==(x0+LP) && jj==p_edge2y && kk<HP) {
	    if((x0+LP)<(Nbx-1) && LP < Nbx && kk>0) { temp.orientation = 8; temp.type = 1; }
	    //	    else { temp.orientation = 15; temp.type = 1; }
	  }
	  else {
	    temp.orientation = 0;
...
void FixLbFluid::calc_MPT(double& totalmass, double totalmomentum[3], double& Tave)
{
  ...
  for(int i = 1; i<subNbx-1; i++)
	...
	localTave += (u_lb[i][j][k][0]*u_lb[i][j][k][0]+u_lb[i][j][k][1]*u_lb[i][j][k][1]+u_lb[i][j][k][2]*u_lb[i][j][k][2])*density_lb[i][j][k];
      }
...
{                              /* with extra_dof variable replaced by its default domain->dimension */
  ...
  double tfactor;
  if (dof > FLT_EPSILON) tfactor = force->mvv2e / (dof * force->boltz);
  else tfactor = 0.0;
  return tfactor;
}
...
double FixLbFluid::compute_scalar() /* Based on same member of compute_temp class */
{
  ...
  if (rmass) {
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        t += (v[i][0]*v[i][0] + v[i][1]*v[i][1] + v[i][2]*v[i][2]) * (mass[type[i]]+massp[i]);
      }				  
  }
...
double FixLbFluid::compute_vector(int n)
{
  ...
  switch(n)
    ...
    case 0: return Tav;
            break;
    case 1: return totalmass;
            break;
    case 2: return totalmomentum[0];
            break;
    case 3: return totalmomentum[1];
            break;
    case 4: return totalmomentum[2];
            break;
    default: error->all(FLERR,"No fix lb/fluid variable available for that index.");
      return -1;
    }
