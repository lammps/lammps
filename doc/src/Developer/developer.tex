...
% Formatted by Rohit Goswami <r95g10 at gmail.com>
% TODO: Rewrite
...
signature: \mintinline{cpp}{FixMine(class LAMMPS *, int, char **)}.
...
\begin{itemize}
\item \mintinline{cpp}{double memory_usage} - return how much memory fix uses
\item \mintinline{cpp}{void grow_arrays(int)} - do reallocation of the per particle arrays
  in your fix
\item \mintinline{cpp}{void copy_arrays(int i, int j, int delflag)} - copy the $i^{th}$ per-particle
  information to the $j^{th}$. Used when atoms sorting is performed. If \code{delflag} is set
  and atom $j$ owns a body, move the body information to atom $i$.
\item \mintinline{cpp}{void set_arrays(int i)} - sets $i^{th}$ particle related information to zero
\end{itemize}
...
need to add \mintinline{cpp}{double ** x} to the header file. Then:
...
the constructor and implementing methods \mintinline{cpp}{void write\_restart(FILE *fp)} and \mintinline{cpp}{void restart(char *buf)}.
