...
PairEAMAlloyKokkos<Space>::PairEAMAlloyKokkos(LAMMPS *lmp) : PairEAM(lmp)
{
  respa_enable = 0;
  one_coeff = 1;
  manybody_flag = 1;
...
template<ExecutionSpace Space>
PairEAMAlloyKokkos<Space>::~PairEAMAlloyKokkos()
{
  if (!copymode) {
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (atom->nmax > nmax) {
    ...
    d_rho = DualViewHelper<Space>::view(k_rho);
    d_fp = DualViewHelper<Space>::view(k_fp);
    h_rho = k_rho.h_view;
    h_fp = k_fp.h_view;
  }
  ...
  if (neighflag == HALF || neighflag == HALFTHREAD) {
    ...
    if (newton_pair) {
      DualViewHelper<Space>::modify(k_rho);
      k_rho.sync_host();
      comm->reverse_comm_pair(this);
      k_rho.modify_host();
      DualViewHelper<Space>::sync(k_rho);
    }
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::init_style()
{
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::file2array()
{
  file2array_alloy();
  ...
  DualViewHelper<Space>::sync(k_type2frho);
  k_type2rhor.modify_host();
  DualViewHelper<Space>::sync(k_type2rhor);
  k_type2z2r.modify_host();
  DualViewHelper<Space>::sync(k_type2z2r);

  d_type2frho = DualViewHelper<Space>::view(k_type2frho);
  d_type2rhor = DualViewHelper<Space>::view(k_type2rhor);
  d_type2z2r = DualViewHelper<Space>::view(k_type2z2r);
}

/* ---------------------------------------------------------------------- */

template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::array2spline()
{
  rdr = 1.0/dr;
  rdrho = 1.0/drho;
  ...
  DualViewHelper<Space>::sync(k_frho_spline);

  for (int i = 0; i < nrhor; i++)
    interpolate(nr,dr,rhor[i],h_rhor_spline,i);
  k_rhor_spline.modify_host();
  DualViewHelper<Space>::sync(k_rhor_spline);

  for (int i = 0; i < nz2r; i++)
    interpolate(nr,dr,z2r[i],h_z2r_spline,i);
  k_z2r_spline.modify_host();
  DualViewHelper<Space>::sync(k_z2r_spline);

  d_frho_spline = DualViewHelper<Space>::view(k_frho_spline);
  d_rhor_spline = DualViewHelper<Space>::view(k_rhor_spline);
  d_z2r_spline = DualViewHelper<Space>::view(k_z2r_spline);
}
...
template<ExecutionSpace Space>
int PairEAMAlloyKokkos<Space>::pack_forward_comm_kokkos(int n, DAT::tdual_int_2d k_sendlist, int iswap_in, DAT::tdual_float_1d &buf,
...
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyPackForwardComm, const int &i) const {
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::unpack_forward_comm_kokkos(int n, int first_in, DAT::tdual_float_1d &buf)
{
  first = first_in;
  v_buf = DualViewHelper<Space>::view(buf);
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyUnpackForwardComm, const int &i) const {
...
template<ExecutionSpace Space>
int PairEAMAlloyKokkos<Space>::pack_forward_comm(int n, int *list, double *buf,
                               int pbc_flag, int *pbc)
{
  int i,j;
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::unpack_forward_comm(int n, int first, double *buf)
{
  for (int i = 0; i < n; i++) {
    h_fp[i + first] = buf[i];
  }
...
template<ExecutionSpace Space>
int PairEAMAlloyKokkos<Space>::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
template<ExecutionSpace Space>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyInitialize, const int &i) const {
  d_rho[i] = 0.0;
}
...
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelA<NEIGHFLAG,NEWTON_PAIR>, const int &ii) const {
...
template<int EFLAG>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelB<EFLAG>, const int &ii, EV_FLOAT& ev) const {
...
template<int EFLAG>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelB<EFLAG>, const int &ii) const {
  EV_FLOAT ev;
...
template<int EFLAG>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelAB<EFLAG>, const int &ii, EV_FLOAT& ev) const {
...
template<int EFLAG>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelAB<EFLAG>, const int &ii) const {
  EV_FLOAT ev;
...
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelC<NEIGHFLAG,NEWTON_PAIR,EVFLAG>, const int &ii, EV_FLOAT& ev) const {
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    if(rsq < cutforcesq) {
      ...
      const KK_FLOAT z2p = (d_z2r_spline(d_type2z2r_ij,m,0)*p + d_z2r_spline(d_type2z2r_ij,m,1))*p +
...
template<int NEIGHFLAG, int NEWTON_PAIR, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::operator()(TagPairEAMAlloyKernelC<NEIGHFLAG,NEWTON_PAIR,EVFLAG>, const int &ii) const {
  EV_FLOAT ev;
...
template<int NEIGHFLAG, int NEWTON_PAIR>
KOKKOS_INLINE_FUNCTION
void PairEAMAlloyKokkos<Space>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::coeff(int narg, char **arg)
{
  int i,j;
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::read_file(char *filename)
{
  Setfl *file = setfl;
...
template<ExecutionSpace Space>
void PairEAMAlloyKokkos<Space>::file2array_alloy()
{
  int i,j,m,n;
...
namespace LAMMPS_NS {
template class PairEAMAlloyKokkos<Device>;
template class PairEAMAlloyKokkos<Host>;
}
