...
#include "improper_sqdistharm_kokkos.h"
#include <cmath>
...
ImproperSQDistHarmKokkos<DeviceType>::ImproperSQDistHarmKokkos(LAMMPS *lmp) : ImproperSQDistHarm(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
ImproperSQDistHarmKokkos<DeviceType>::~ImproperSQDistHarmKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void ImproperSQDistHarmKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagImproperSQDistHarmCompute<1,1> >(0,nimproperlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagImproperSQDistHarmCompute<0,1> >(0,nimproperlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagImproperSQDistHarmCompute<1,0> >(0,nimproperlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagImproperSQDistHarmCompute<0,0> >(0,nimproperlist),*this);
    }
  }
...
void ImproperSQDistHarmKokkos<DeviceType>::operator()(TagImproperSQDistHarmCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const
{
  ...
  // i4 is the central atom; same geometry as distharm
  ...
  const KK_FLOAT da2 = da*da;

  KK_FLOAT eimproper = static_cast<KK_FLOAT>(0.0);
  if (EVFLAG && eflag)
    eimproper = d_k[type]*(da2 - d_chi[type])*(da2 - d_chi[type]);

  const KK_FLOAT a = static_cast<KK_FLOAT>(4.0)*da*d_k[type]*(da2 - d_chi[type]);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void ImproperSQDistHarmKokkos<DeviceType>::operator()(TagImproperSQDistHarmCompute<NEWTON_BOND,EVFLAG>, const int &n) const
{
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagImproperSQDistHarmCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void ImproperSQDistHarmKokkos<DeviceType>::allocate()
{
  ImproperSQDistHarm::allocate();
  int n = atom->nimpropertypes;
  k_k = DAT::tdual_kkfloat_1d("ImproperSQDistHarm::k",n+1);
  k_chi = DAT::tdual_kkfloat_1d("ImproperSQDistHarm::chi",n+1);
...
template<class DeviceType>
void ImproperSQDistHarmKokkos<DeviceType>::coeff(int narg, char **arg)
{
  ImproperSQDistHarm::coeff(narg, arg);
  int ilo,ihi;
...
template<class DeviceType>
void ImproperSQDistHarmKokkos<DeviceType>::read_restart(FILE *fp)
{
  ImproperSQDistHarm::read_restart(fp);
  int n = atom->nimpropertypes;
  for (int i = 1; i <= n; i++) {
    k_k.view_host()[i] = static_cast<KK_FLOAT>(k[i]);
    k_chi.view_host()[i] = static_cast<KK_FLOAT>(chi[i]);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void ImproperSQDistHarmKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i1, const int i2,
...
namespace LAMMPS_NS {
template class ImproperSQDistHarmKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class ImproperSQDistHarmKokkos<LMPHostType>;
#endif
}
