...
/* ----------------------------------------------------------------------
   the per-atom virial of a improper style has to add up to its global
   ...
   branches of the improper styles and of their accelerated variants are
...
// the units of Improper::virial before it can be compared with it
...
TEST(ImproperStyle, vatom_only_omp)
{
    if (!Info::has_package("OPENMP")) GTEST_SKIP();
    if (test_config.skip_tests.count(test_info_->name())) GTEST_SKIP();
...
TEST(ImproperStyle, vatom_only_kokkos)
{
    ...
    LAMMPS::argv args = {"ImproperStyle", "-log", "none", "-echo", "screen", "-nocite",
                         "-k", "on", "t", "1", "-sf", "kk"};
    if (kk_gpu)
        args = {"ImproperStyle", "-log", "none", "-echo", "screen", "-nocite",
                "-k", "on", "g", "1", "-sf", "kk",
...
TEST(ImproperStyle, kokkos_omp_full)
{
    if (!Info::has_package("KOKKOS")) GTEST_SKIP();
    if (test_config.skip_tests.count(test_info_->name())) GTEST_SKIP();
    // skip entries may also be qualified by the KOKKOS package precision,
...
TEST(ImproperStyle, kokkos_serial)
{
    if (!Info::has_package("KOKKOS")) GTEST_SKIP();
    if (test_config.skip_tests.count(test_info_->name())) GTEST_SKIP();
    // skip entries may also be qualified by the KOKKOS package precision,
...
TEST(ImproperStyle, kokkos_serial_full)
{
    if (!Info::has_package("KOKKOS")) GTEST_SKIP();
    if (test_config.skip_tests.count(test_info_->name())) GTEST_SKIP();
    // skip entries may also be qualified by the KOKKOS package precision,
